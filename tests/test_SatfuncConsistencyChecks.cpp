...
Opm::EclEpsScalingPointsInfo<double>
makePoints()
{
    return {};
}
...
BOOST_AUTO_TEST_CASE(All_Good)
{
    ...
    BOOST_CHECK_MESSAGE(!checker.anyFailedStandardChecks(), "There must be no failed checks");

    BOOST_CHECK_MESSAGE(!checker.anyFailedCriticalChecks(), "There must be no failed critical checks");

    auto rpt = std::string {};
    checker.reportFailures(Opm::SatfuncConsistencyChecks<double>::ViolationLevel::Standard,
                           [&rpt](std::string_view record) { rpt += fmt::format("{}\n", record); });
...
BOOST_AUTO_TEST_CASE(Standard_Violation)
{
    ...
    BOOST_CHECK_MESSAGE(checker.anyFailedStandardChecks(), "There must be at least one failed check");
...
BOOST_AUTO_TEST_CASE(Standard_Violation_ReportIJK)
{
    ...
    checker.setPointIDFormatCallback([](const std::size_t) { return std::string {"(11, 22, 33)"}; });
...
BOOST_AUTO_TEST_CASE(Critical_Violation)
{
    ...
    BOOST_CHECK_MESSAGE(!checker.anyFailedStandardChecks(), "There must be no failed standard level checks");

    BOOST_CHECK_MESSAGE(checker.anyFailedCriticalChecks(), "There must be at least one failed Critical check");
...
Opm::EclEpsScalingPointsInfo<float>
makePoints()
{
    return {};
}
...
BOOST_AUTO_TEST_CASE(Standard)
{
    ...
    checker.setPointIDFormatCallback([](const std::size_t) { return std::string {"(121, 323, 42)"}; });
