...
std::size_t
globIndex(const std::array<int, 3>& ijk, const std::array<int, 3>& dims)
{
    return ijk[0] + dims[0] * (ijk[1] + static_cast<std::size_t>(dims[1]) * ijk[2]);
}

std::array<int, 3>
cellIJK(int cell, const std::array<int, 3>& dims)
{
...
namespace Rank
{
    namespace Top
    {
        ...
        {
            return (global >= 5 * 5 * 5) ? -1 : static_cast<int>(global);
        }
        ...
        bool isInRange(const std::array<int, 3>& ijk)
        {
            ...
            return (ijk[0] >= 1) && (ijk[0] <= 3) && (ijk[1] >= 1) && (ijk[1] <= 3) && (ijk[2] >= 2) && (ijk[2] <= 4);
        }
        ...
        void cellSource(const int cell, Opm::PAvgDynamicSourceData<double>::SourceDataSpan<double> src)
        {
        ...
        Opm::ParallelWBPCalculation<double>::EvaluatorFactory connSource()
        {
            return []() {
                ...
                return [rho = std::move(rho)](const int connIx,
    ...
    namespace Bottom
    {
        int globalToLocal(const std::size_t global)
        {
            ...
            return (global < middle) ? -1 : static_cast<int>(global - middle);
        }
        ...
        bool isInRange(const std::array<int, 3>& ijk)
        {
            ...
            return (ijk[0] >= 1) && (ijk[0] <= 3) && (ijk[1] >= 1) && (ijk[1] <= 3) && (ijk[2] >= 0) && (ijk[2] <= 2);
        }
...
centreConnections(const int topConn, const int numConns)
{
    ...
    return std::make_shared<Opm::WellConnections>(Opm::Connection::Order::INPUT, i, j, conns);
}
...
{
    auto pwi = Opm::ParallelWellInfo<double> {std::pair {std::string {"P"}, true}, comm};
...
void
setCallbacksTop(Opm::ParallelWBPCalculation<double>& wbpCalcService)
{
    wbpCalcService.localCellIndex(&Rank::Top::globalToLocal).evalCellSource(&Rank::Top::cellSource);
}

void
setCallbacksBottom(Opm::ParallelWBPCalculation<double>& wbpCalcService)
{
    wbpCalcService.localCellIndex(&Rank::Bottom::globalToLocal).evalCellSource(&Rank::Bottom::cellSource);
}

void
setCallbacks(const int rank, Opm::ParallelWBPCalculation<double>& wbpCalcService)
{
    if (rank == 0) {
...
Opm::ParallelWBPCalculation<double>::EvaluatorFactory
connSource(const int rank)
{
    if (rank == 0) {
...
struct Setup {
        ...
        , pwi {parallelWellInfo(comm)}
    {
        ...
        this->wbpCalcService.createCalculator(
            producerWell(), this->pwi, localConnIdx(this->comm.rank()), connSource(this->comm.rank()));
    }
...
BOOST_AUTO_TEST_CASE(Create)
{
    ...
    auto wbpCalcService = Opm::ParallelWBPCalculation<double> {dims, comm};

    setCallbacks(comm.rank(), wbpCalcService);
    const auto pwi = parallelWellInfo(comm);

    const auto calcIdx
        = wbpCalcService.createCalculator(producerWell(), pwi, localConnIdx(comm.rank()), connSource(comm.rank()));
