...
template<class T>
using GpuB = Opm::gpuistl::GpuBuffer<T>;

template<class T>
using GpuV = Opm::gpuistl::GpuView<T>;

using GpuBufCo2Tables = Opm::CO2Tables<double, GpuB>;
using GpuBufBrineCo2Pvt = Opm::BrineCo2Pvt<double, GpuB>;
...
template <class TypeTag>
struct DummyProblem {
    using EclMaterialLawManager = typename Opm::GetPropType<TypeTag, Opm::Properties::MaterialLaw>;
    using EclThermalLawManager = typename Opm::GetPropType<TypeTag, Opm::Properties::SolidEnergyLaw>;
    using MaterialLawParams = typename EclMaterialLawManager::Params;
    struct {
        struct {
    ...
    OPM_HOST_DEVICE Evaluation rockCompPoroMultiplier(const Scalar&, std::size_t) const
    {
    ...
    template <class Scalar, class Evaluation>
    OPM_HOST_DEVICE Evaluation rockCompTransMultiplier(const Scalar&, std::size_t) const
    {
        return Evaluation(0.0);
    }
};
...
namespace Properties
{
    ...
    template <class TypeTag>
    struct Problem<TypeTag, TTag::FlowSimpleProblemGPU> {
...
testCreationGPUWithProblem(BlackOilFluidSystemView fs, ProblemType problem)
{
    ...
    return;
    Opm::BlackOilPrimaryVariables<TypeTagGPU, Opm::gpuistl::MiniVector> primaryVariables(fs);
    Opm::BlackOilIntensiveQuantities<TypeTagGPU> intensiveQuantities;
    auto& state = intensiveQuantities.fluidState();
...
BOOST_AUTO_TEST_CASE(TestPrimaryVariablesCreationGPU)
{
    ...
    std::cout << "Opm::BlackOilPrimaryVariables<TypeNacht, Opm::gpuistl::MiniVector> : "
              << typeid(Opm::BlackOilPrimaryVariables<TypeNacht, Opm::gpuistl::MiniVector>).name() << std::endl;
                  //std::cout << "Opm::BlackOilPrimaryVariables<TypeNacht, Opm::gpuistl::MiniVector>::Scalar : "
    //          << typeid(Opm::BlackOilPrimaryVariables<TypeNacht, Opm::gpuistl::MiniVector>::Scalar).name() << std::endl;
