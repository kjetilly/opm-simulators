...
public:
    ...
    TestPreconditioner(const SpMatrixFloat& matrix, const XDouble& expectedInput, const SpMatrixDouble& expectedMatrix, const XDouble& expectedOutputVector)
        : matrix(matrix)
        , expectedInput(expectedInput)
        , expectedMatrix(expectedMatrix)
        , expectedOutputVector(expectedOutputVector)
    {
    }
    ...
    virtual void apply([[maybe_unused]] XFloat& v, const XFloat& d) override
    {
        ...
        for(size_t i = 0; i < d.N(); ++i) {
            for (size_t j = 0; j < d[i].N(); ++j) {
                BOOST_CHECK_EQUAL(d[i][j], float(expectedInput[i][j]));
                v[i][j] = float(expectedOutputVector[i][j]);
            }
        }

        // make sure we get the correct matrix
        BOOST_CHECK_EQUAL(expectedMatrix.N(), matrix.N());
        BOOST_CHECK_EQUAL(expectedMatrix.nonzeroes(), matrix.nonzeroes());
            ...
            for(auto column = row->begin(); column != row->end(); ++column) {
                for (int i = 0; i < MFloat::rows; ++i) {
                    for (int j = 0; j < MFloat::cols; ++j) {
                        BOOST_CHECK_EQUAL(float(expectedMatrix[i][j][i][j]), matrix[i][j][i][j]);
                    }
                }
...
BOOST_AUTO_TEST_CASE(TestFiniteDifference1D)
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        inputVector[i][0] = 1.0;
        auto converter = Opm::cuistl::PreconditionerConvertToFloatAdapter<TestPreconditioner, SpMatrixDouble, XDouble, XDouble>(B);
        auto underlyingPreconditioner = std::make_shared<TestPreconditioner>(converter.getConvertedMatrix(),
            inputVector, B, expectedOutputVector);
        converter.setUnderlyingPreconditioner(underlyingPreconditioner);
        converter.apply(outputVector, inputVector);

        for(size_t i = 0; i < expectedOutputVector.N(); ++i) {
            for (size_t j = 0; j < expectedOutputVector[i].N(); ++j) {
                BOOST_CHECK_EQUAL(outputVector[i][j], float(expectedOutputVector[i][j]));
