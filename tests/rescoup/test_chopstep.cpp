...
public:
        ...
        , simulator_ {nullptr}
    {
        ...
        if (initialize_<Properties::TTag::FlowEarlyBird>(this->exit_code_, /*keep_keywords=*/false)) {
            this->setupVanguard();
            this->flow_main_
                = std::make_unique<FlowMain<TypeTag>>(this->argc_, this->argv_, this->outputCout_, this->outputFiles_);
...
public:
    ...
    OpmSimulatorTestCase(const std::string& prog_name,
...
struct SimulatorFixture {
    ...
    void checkEq(double a, double b) const
    {
        BOOST_CHECK_CLOSE(a, b, 1e-16);
    }
    const Opm::ReservoirCouplingMaster& getRcMaster() const
    {
        return rc_master_;
    }
...
BOOST_AUTO_TEST_CASE(Chop)
{
...

    {
        ...
        auto new_suggested_time_step = runChopSubStep(master_suggested_time_step);
        BOOST_CHECK(
            !Opm::ReservoirCoupling::Seconds::compare_gt_or_eq(new_suggested_time_step, master_suggested_time_step));
    }
}
