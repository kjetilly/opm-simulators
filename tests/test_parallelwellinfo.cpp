...
{
std::ostream&
operator<<(std::ostream& os, const std::pair<std::string, bool>& p)
{
...
{
std::ostream&
operator<<(std::ostream& os, const Opm::ParallelWellInfo<double>& w)
{
    return os << "{" << w.name() << " " << w.hasLocalCells() << " " << w.isOwner() << "}";
}
} // namespace Opm
...
BOOST_AUTO_TEST_CASE(ParallelWellComparison)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(pairs.begin(), pairs.end(), well_info.begin(), well_info.end());

    BOOST_CHECK_EQUAL_COLLECTIONS(well_info.begin(), well_info.end(), pairs.begin(), pairs.end());
...
BOOST_AUTO_TEST_CASE(CommunicateAboveBelowSelf)
{
    ...
    for (std::size_t count = 0; count < 2; ++count) {
        ...
        std::transform(eclIndex.begin(), eclIndex.end(), current.begin(), [](double v) { return 1 + 10.0 * v; });
        ...
        std::for_each(
            current.begin(), current.begin() + (current.size() - 1), [&a](double v) { BOOST_CHECK(*(a++) == v); });
        ...
        auto b = below.begin();
        std::for_each(current.begin() + 1, current.end(), [&b](double v) { BOOST_CHECK(*(b++) == v); });
    }
}
...
std::vector<int>
createGlobalEclIndex(const Opm::Parallel::Communication& comm)
{
    ...
    if (globalSize > oldSize) {
        ++lastIndex;
        for (auto entry = globalEclIndex.begin() + oldSize; entry != globalEclIndex.end(); ++entry, ++lastIndex) {
            *entry = lastIndex;
        }
    }
...
template <class C>
std::vector<double>
populateCommAbove(C& commAboveBelow,
...
BOOST_AUTO_TEST_CASE(CommunicateAboveBelowParallel)
{
    ...
    for (std::size_t count = 0; count < 2; ++count) {
        ...
        std::transform(
            globalEclIndex.begin(), globalEclIndex.end(), globalCurrent.begin(), [](double v) { return 1 + 10.0 * v; });
...
void
initRandomNumbers(Iter begin, Iter end, C comm)
{
...
BOOST_AUTO_TEST_CASE(PartialSumself)
{
    ...
    initRandomNumbers(std::begin(current), std::end(current), Opm::Parallel::Communication(comm));
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(std::begin(stdCopy), std::end(stdCopy), std::begin(current), std::end(current));
}
...
BOOST_AUTO_TEST_CASE(PartialSumParallel)
{
    ...
    auto globalEclIndex = createGlobalEclIndex(comm);
    std::vector<double> globalCurrent(globalEclIndex.size());
    initRandomNumbers(std::begin(globalCurrent), std::end(globalCurrent), Opm::Parallel::Communication(comm));

    auto localCurrent = populateCommAbove(commAboveBelow, comm, globalEclIndex, globalCurrent);
...
void
testGlobalPerfFactoryParallel(int num_component, bool local_consecutive = false)
{
    auto comm = Opm::Parallel::Communication(Dune::MPIHelper::getCommunicator());
    ...
    initRandomNumbers(std::begin(globalCurrent), std::end(globalCurrent), comm);
    initRandomNumbers(std::begin(globalAdd), std::end(globalAdd), comm);

    auto localCurrent
        = populateCommAbove(wellInfo, comm, globalEclIndex, globalCurrent, num_component, local_consecutive);

    // A hack to get local values to add.
    Opm::ParallelWellInfo<double> dummy {{"Test", true}, comm};
    auto localAdd = populateCommAbove(dummy, comm, globalEclIndex, globalAdd, num_component, local_consecutive);
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(
        std::begin(globalCurrent), std::end(globalCurrent), std::begin(globalCreated), std::end(globalCreated));

    std::transform(std::begin(globalAdd),
                   std::end(globalAdd),
                   std::begin(globalCreated),
                   std::begin(globalCreated),
                   std::plus<double>());

    auto globalSol = globalCurrent;
    std::transform(
        std::begin(globalAdd), std::end(globalAdd), std::begin(globalSol), std::begin(globalSol), std::plus<double>());

    auto localSol = localCurrent;

    std::transform(
        std::begin(localAdd), std::end(localAdd), std::begin(localSol), std::begin(localSol), std::plus<double>());
