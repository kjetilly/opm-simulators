...
sourceLocations(const std::size_t numLoc)
{
...
{
public:
    explicit LocalCellIndex(const std::size_t rank, const std::size_t size)
        : rank_ {rank}
        , size_ {size}
    {
    }
    ...
    {
        return ((i % this->size_) == this->rank_) ? static_cast<int>(i / this->size_) : -1;
    }
...
public:
    ...
    void operator()(const int i, SrcTerm source_term) const
    {
        ...
        source_term.set(Item::Pressure, this->rank_ * 314.15 - i)
                      ...
                      const Opm::PAvgDynamicSourceData<double>& source_data)
{
    ...
    for (auto srcID = 0 * num_src; srcID < num_src; ++srcID) {
        ...
        const auto ok = (src[Item::Pressure] == rank * 314.15 - locI)
            && (src[Item::PoreVol] == rank * 172.9 + 10 * locI) && (src[Item::MixtureDensity] == rank * 852.96 + locI);
...
BOOST_AUTO_TEST_CASE(Eval_and_collect)
{
    ...
    auto source_data = Opm::ParallelPAvgDynamicSourceData<double> {
        comm, sourceLocations(num_src_loc), LocalCellIndex {comm_rank, comm_size}};

    source_data.collectLocalSources(CalculateSourceTerm {comm_rank});
    source_data.synchroniseSources();

    const auto num_rank_correct = comm.sum(sourceTermsAreCorrect(comm_size, num_src_loc, source_data));
