...
void setupPattern(
    int N, M& mat, Dune::ParallelIndexSet<G, L, n>& indices, int overlapStart, int overlapEnd, int start, int end);
...
M setupAnisotropic2d(int N,
                     Dune::ParallelIndexSet<G, L, n>& indices,
                     const C& p,
                     int* nout,
                     typename M::block_type::value_type eps = 1.0);


template <class M, class G, class L, int s>
void
setupPattern(
    int N, M& mat, Dune::ParallelIndexSet<G, L, s>& indices, int overlapStart, int overlapEnd, int start, int end)
{
...
void
fillValues([[maybe_unused]] int N, M& mat, int overlapStart, int overlapEnd, int start, int end, T eps)
{
...
void
setBoundary(V& lhs, V& rhs, const G& n, Dune::ParallelIndexSet<G, L, s>& indices)
{
...
void
setBoundary(V& lhs, V& rhs, const G& N)
{
...
M
setupAnisotropic2d(
    int N, Dune::ParallelIndexSet<G, L, s>& indices, const C& p, int* nout, typename M::block_type::value_type eps)
{
...
main(int argc, char** argv)
{
    ...
    prm.put("type", "Jac"s);
    std::function<Vector()> weights
        = [&mat]() { return Opm::Amg::getQuasiImpesWeights<BCRSMat, Vector>(mat, 0, false); };
    ...
    auto ghostPreconditioner = Opm::PreconditionerFactory<PreconditionerOperatorType, Communication>::create(
        *ghostOperator, prm, weights, comm);
    Dune::BiCGSTABSolver<Vector> amgGhostBiCGSTAB(
        op, sp, *ghostPreconditioner, 10e-8, 300, (ccomm.rank() == 0) ? 2 : 0);
