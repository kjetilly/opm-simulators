...
public:
    ...
    boost::test_tools::assertion_result operator()(boost::unit_test::test_unit_id) const
    {
        ...
        response.message() << "Number of MPI processes (" << comm.size() << ") differs from expected "
                           << this->expectNP_;
...
using TestFluidSystem = Opm::BlackOilFluidSystem<double, Opm::BlackOilDefaultFluidSystemIndices>;
...
BOOST_AUTO_TEST_CASE(Single_Well)
{
    ...
    const auto comm = Opm::Parallel::Communication {Dune::MPIHelper::getCommunicator()};

    auto rftc = RFTContainer {cse.es,
                              cse.sched,
    ...
    BOOST_REQUIRE_MESSAGE(rPos != xw.end(), R"(There must be dynamic results for well "R")");

    BOOST_CHECK_EQUAL(rPos->second.connections.size(), std::size_t {3});

    // (2,2,1)
    {
        const auto* xc = rPos->second.find_connection(cse.es.getInputGrid().getGlobalIndex(1, 1, 0));

        BOOST_REQUIRE_MESSAGE(xc != nullptr, R"(Well "R" must have dynamic results in connection (2,2,1))");
    ...
    // (2,2,2)
    {
        const auto* xc = rPos->second.find_connection(cse.es.getInputGrid().getGlobalIndex(1, 1, 1));

        BOOST_REQUIRE_MESSAGE(xc != nullptr, R"(Well "R" must have dynamic results in connection (2,2,2))");
    ...
    // (2,2,3)
    {
        const auto* xc = rPos->second.find_connection(cse.es.getInputGrid().getGlobalIndex(1, 1, 2));

        BOOST_REQUIRE_MESSAGE(xc != nullptr, R"(Well "R" must have dynamic results in connection (2,2,3))");
...
BOOST_AUTO_TEST_CASE(Quarter_Five_Spot)
{
    ...
    BOOST_REQUIRE_MESSAGE(r1Pos != xw.end(), R"(There must be dynamic results for well "R1")");

    BOOST_CHECK_EQUAL(r1Pos->second.connections.size(), std::size_t {3});

    // (1,1,1)
    {
        const auto* xc = r1Pos->second.find_connection(cse.es.getInputGrid().getGlobalIndex(0, 0, 0));

        BOOST_REQUIRE_MESSAGE(xc != nullptr, R"(Well "R1" must have dynamic results in connection (1,1,1))");
    ...
    // (1,1,2)
    {
        const auto* xc = r1Pos->second.find_connection(cse.es.getInputGrid().getGlobalIndex(0, 0, 1));

        BOOST_REQUIRE_MESSAGE(xc != nullptr, R"(Well "R1" must have dynamic results in connection (1,1,2))");
    ...
    // (1,1,3)
    {
        const auto* xc = r1Pos->second.find_connection(cse.es.getInputGrid().getGlobalIndex(0, 0, 2));

        BOOST_REQUIRE_MESSAGE(xc != nullptr, R"(Well "R1" must have dynamic results in connection (1,1,3))");
    ...
    BOOST_REQUIRE_MESSAGE(r2Pos != xw.end(), R"(There must be dynamic results for well "R2")");

    BOOST_CHECK_EQUAL(r2Pos->second.connections.size(), std::size_t {3});

    // (3,3,1)
    {
        const auto* xc = r2Pos->second.find_connection(cse.es.getInputGrid().getGlobalIndex(2, 2, 0));

        BOOST_REQUIRE_MESSAGE(xc != nullptr, R"(Well "R2" must have dynamic results in connection (3,3,1))");
    ...
    // (3,3,2)
    {
        const auto* xc = r2Pos->second.find_connection(cse.es.getInputGrid().getGlobalIndex(2, 2, 1));

        BOOST_REQUIRE_MESSAGE(xc != nullptr, R"(Well "R2" must have dynamic results in connection (3,3,2))");
    ...
    // (3,3,3)
    {
        const auto* xc = r2Pos->second.find_connection(cse.es.getInputGrid().getGlobalIndex(2, 2, 2));

        BOOST_REQUIRE_MESSAGE(xc != nullptr, R"(Well "R2" must have dynamic results in connection (3,3,3))");
...
BOOST_AUTO_TEST_CASE(Quarter_Five_Spot_Non_Split_Wells)
{
    ...
    auto rftc = RFTContainer {cse.es,
                              ...
                              [r = comm.rank()](const std::string& wname) {
                                  return ((r == 0) && (wname == "R2")) || ((r == 1) && (wname == "R1"));
                              },
                              [r = comm.rank()](const std::string& wname) {
                                  return ((r == 0) && (wname == "R2")) || ((r == 1) && (wname == "R1"));
                              }};
...
BOOST_AUTO_TEST_CASE(Quarter_Five_Spot_Owned_And_Split)
{
    ...
    auto rftc = RFTContainer {cse.es,
                              cse.sched,
                              [r = comm.rank()](const std::string& wname) {
                                  return ((r == 2) && (wname == "R1")) || ((r == 1) && (wname == "R2"));
                              },
                              [r = comm.rank()](const std::string& wname) {
                                  return (((r == 0) || (r == 1)) && (wname == "R2")) || ((r == 2) && (wname == "R1"));
                              }};
...
BOOST_AUTO_TEST_CASE(Quarter_Five_Spot_Owned_And_Split)
{
    ...
    auto rftc = RFTContainer {cse.es,
                              cse.sched,
                              [r = comm.rank()](const std::string& wname) {
                                  return ((r == 2) && (wname == "R1")) || ((r == 3) && (wname == "R2"));
                              },
                              [r = comm.rank()](const std::string& wname) {
...
BOOST_AUTO_TEST_CASE(Quarter_Five_Spot_Split_Wells_Even_Odd)
{
    ...
    auto rftc = RFTContainer {cse.es,
                              ...
                              [r = comm.rank()](const std::string& wname) {
                                  return ((r % 2 == 0) && (wname == "R1")) || ((r % 2 == 1) && (wname == "R2"));
                              }};
...
BOOST_AUTO_TEST_CASE(Quarter_Five_Spot_Split_Wells_Cross)
{
    ...
    auto rftc = RFTContainer {cse.es,
                              cse.sched,
                              [r = comm.rank()](const std::string& wname) {
                                  return ((r == 2) && (wname == "R1")) || ((r == 0) && (wname == "R2"));
                              },
                              [r = comm.rank()](const std::string& wname) {
                                  return (((r == 1) || (r == 2)) && (wname == "R1")) || ((r != 1) && (wname == "R2"));
                              }};
