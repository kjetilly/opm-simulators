...
{
Opm::Connection
createConnection(int i, int j, int k, int global_index)
{
    return Opm::Connection(i,
                           j,
                           k, // i,j,k
...
Dune::cpgrid::OpmWellType
createWell(const std::string& name)
{
    using namespace Opm;
    return Dune::cpgrid::OpmWellType(name,
                                     name,
                                     0,
                                     0,
                                     0,
...
Dune::CpGrid
createTestGrid(const std::array<int, 3>& dims, const std::array<double, 3>& size)
{
    Dune::CpGrid grid;
...
Opm::ZoltanPartitioningControl<Entity>
createZoltanControl(const Dune::CpGrid& grid)
{
    Opm::ZoltanPartitioningControl<Entity> zoltan_ctrl;
    zoltan_ctrl.domain_imbalance = 1.1;
    zoltan_ctrl.index = [](const auto& element) { return element.index(); };
    zoltan_ctrl.local_to_global = [&grid](const int local_idx) { return grid.globalCell()[local_idx]; };
    return zoltan_ctrl;
}
} // namespace
...
BOOST_AUTO_TEST_CASE(PartitionCellsTest)
{
    ...
    {
        auto [part, num_part] = Opm::partitionCells("simple",
                                                    3,
                                                    grid.leafGridView(),
                                                    std::vector<Opm::Well> {},
    ...
    {
        auto [part, num_part] = Opm::partitionCells("zoltan",
                                                    3,
                                                    grid.leafGridView(),
                                                    std::vector<Opm::Well> {},
                                                    std::unordered_map<std::string, std::set<int>> {},
...
BOOST_AUTO_TEST_CASE(PartitionCellsWithWellMergeTest)
{
    ...
    auto [part, num_part] = Opm::partitionCells(
        "zoltan", 4, grid.leafGridView(), wells, std::unordered_map<std::string, std::set<int>> {}, zoltan_ctrl, 0);
...
BOOST_AUTO_TEST_CASE(PartitionCellsWithOverlappingWells3DTest)
{
    ...
    // Test Zoltan partitioning with overlapping well cells
    auto [part, num_part] = Opm::partitionCells(
        "zoltan", 10, grid.leafGridView(), wells, std::unordered_map<std::string, std::set<int>> {}, zoltan_ctrl, 0);
...
BOOST_AUTO_TEST_CASE(PartitionCellsComplexWellNetworkTest)
{
    ...
    auto [part, num_part] = Opm::partitionCells(
        "zoltan", 15, grid.leafGridView(), wells, std::unordered_map<std::string, std::set<int>> {}, zoltan_ctrl, 0);
    ...
    auto inSamePartition = [&ppart = part](int idx1, int idx2) { return ppart[idx1] == ppart[idx2]; };
...
#if HAVE_MPI && HAVE_ZOLTAN
    auto [part, num_part] = Opm::partitionCells(
        "zoltan", 5, grid.leafGridView(), wells, std::unordered_map<std::string, std::set<int>> {}, zoltan_ctrl, 0);
...
BOOST_AUTO_TEST_CASE(PartitionCellsWithNeighborConnectivityTest)
{
    ...
    auto [part, num_part] = Opm::partitionCells(
        "zoltan", 3, grid.leafGridView(), wells, std::unordered_map<std::string, std::set<int>> {}, zoltan_ctrl, 1);
