...
Opm::Well
createWell(const std::string& welspecs, const std::string& compdat)
{
...
{
    const auto wpiCalc = Opm::WellProdIndexCalculator<double> {createWell(drainRadDefaulted(), noSkinFactor_SameCF())};
...
{
    const auto wpiCalc
        = Opm::WellProdIndexCalculator<double> {createWell(drainRadDefaulted(), noSkinFactor_DifferentCF())};
...
{
    const auto wpiCalc = Opm::WellProdIndexCalculator<double> {createWell(drainRadDefaulted(), skin2_SameCF())};
...
{
    const auto wpiCalc = Opm::WellProdIndexCalculator<double> {createWell(drainRadDefaulted(), skin421_DifferentCF())};
...
{
    const auto wpiCalc = Opm::WellProdIndexCalculator<double> {createWell(explicitDrainRad(), noSkinFactor_SameCF())};
...
{
    const auto wpiCalc
        = Opm::WellProdIndexCalculator<double> {createWell(explicitDrainRad(), noSkinFactor_DifferentCF())};
...
{
    const auto wpiCalc = Opm::WellProdIndexCalculator<double> {createWell(explicitDrainRad(), skin2_SameCF())};
...
{
    const auto wpiCalc = Opm::WellProdIndexCalculator<double> {createWell(explicitDrainRad(), skin421_DifferentCF())};
...
BOOST_AUTO_TEST_CASE(allDefaulted_SameCF)
{
    ...
    const auto expectCF = 100 * cp_rm3_per_db();
    const auto expectPI = std::vector<double> {1.0 * expectCF, 2.0 * expectCF, 4.0 * expectCF};
...
BOOST_AUTO_TEST_CASE(allDefaulted_DifferentCF)
{
    ...
    const auto expectCF = 100 * cp_rm3_per_db();
    const auto expectPI = std::vector<double> {expectCF, expectCF, expectCF};
...
BOOST_AUTO_TEST_CASE(logarithmic_SameCF)
{
    ...
    const auto expectCF = 100 * cp_rm3_per_db();
    const auto expectPI = std::vector<double> {0.5 * expectCF, 1.0 * expectCF, 2.0 * expectCF};
...
BOOST_AUTO_TEST_CASE(logarithmic_DifferentCF)
{
    ...
    const auto expectCF = 100 * cp_rm3_per_db();
    const auto expectPI = std::vector<double> {0.25 * expectCF, 1.0 * expectCF, 4.0 * expectCF};
...
BOOST_AUTO_TEST_CASE(logarithmic_Skin2_SameCF)
{
    ...
    const auto expectCF = 100 * cp_rm3_per_db();
    const auto expectPI = std::vector<double> {0.75 * expectCF, 1.5 * expectCF, 3.0 * expectCF};
...
BOOST_AUTO_TEST_CASE(logarithmic_skin421_DifferentCF)
{
    ...
    const auto expectCF = 100 * cp_rm3_per_db();
    const auto expectPI = ((5.0 / 6.0) * 0.5 + 1.5 * 1.0 + (8.0 / 3.0) * 2.0) * expectCF;
