    ...
    namespace Details
    {
                            ...
                            std::array<Scalar, 2>& span)
        {
            ...
            for (const auto& cell : cells) {
                if (cellZMinMax[cell].first < span[0]) {
                    span[0] = cellZMinMax[cell].first;
                }
                if (cellZMinMax[cell].second > span[1]) {
        ...
        horizontalSubdivision(const CellID cell, const std::pair<Scalar, Scalar> topbot, const int numIntervals)
        {
            auto subdiv = std::vector<std::pair<Scalar, Scalar>> {};
            subdiv.reserve(2 * numIntervals);

            if (topbot.first > topbot.second) {
                throw std::out_of_range {"Negative thickness (inverted top/bottom faces) in cell "
                                         + std::to_string(cell)};
            }

            subdivisionCentrePoints(topbot.first, topbot.second, 2 * numIntervals, subdiv);
        ...
        RK4IVP<Scalar, RHS>::RK4IVP(const RHS& f, const std::array<Scalar, 2>& span, const Scalar y0, const int N)
            : N_(N)
            , span_(span)
        {
        ...
        template <class Scalar, class RHS>
        Scalar RK4IVP<Scalar, RHS>::operator()(const Scalar x) const
        {
        ...
        template <class Scalar, class RHS>
        Scalar RK4IVP<Scalar, RHS>::stepsize() const
        {
        ...
        namespace PhasePressODE
        {
            ...
            Water<FluidSystem>::Water(const TabulatedFunction& tempVdTable,
            ...
            template <class FluidSystem>
            typename Water<FluidSystem>::Scalar Water<FluidSystem>::operator()(const Scalar depth,
            ...
            template <class FluidSystem>
            typename Water<FluidSystem>::Scalar Water<FluidSystem>::density(const Scalar depth,
                                                                            const Scalar press) const
            {
                // The initializing algorithm can give depths outside the range due to numerical noise i.e. we
                // extrapolate
                Scalar saltConcentration = saltVdTable_.eval(depth, /*extrapolate=*/true);
                Scalar temp = tempVdTable_.eval(depth, /*extrapolate=*/true);
                Scalar rho = FluidSystem::waterPvt().inverseFormationVolumeFactor(
                    pvtRegionIdx_, temp, press, Scalar {0.0} /*=Rsw*/, saltConcentration);
                rho *= FluidSystem::referenceDensity(FluidSystem::waterPhaseIdx, pvtRegionIdx_);
                return rho;
            }

            template <class FluidSystem, class RS>
            Oil<FluidSystem, RS>::Oil(const TabulatedFunction& tempVdTable,
                                      const RS& rs,
                                      const int pvtRegionIdx,
                                      const Scalar normGrav)
                : tempVdTable_(tempVdTable)
            ...
            template <class FluidSystem, class RS>
            typename Oil<FluidSystem, RS>::Scalar Oil<FluidSystem, RS>::operator()(const Scalar depth,
                                                                                   const Scalar press) const
            {
                return this->density(depth, press) * g_;
            }
            ...
            template <class FluidSystem, class RS>
            typename Oil<FluidSystem, RS>::Scalar Oil<FluidSystem, RS>::density(const Scalar depth,
                                                                                const Scalar press) const
            {
            ...
            Gas<FluidSystem, RV, RVW>::Gas(const TabulatedFunction& tempVdTable,
                                           const RV& rv,
                                           const RVW& rvw,
                                           const int pvtRegionIdx,
                                           const Scalar normGrav)
            ...
            template <class FluidSystem, class RV, class RVW>
            typename Gas<FluidSystem, RV, RVW>::Scalar Gas<FluidSystem, RV, RVW>::operator()(const Scalar depth,
                                                                                             const Scalar press) const
            {
                return this->density(depth, press) * g_;
            }
            ...
            template <class FluidSystem, class RV, class RVW>
            typename Gas<FluidSystem, RV, RVW>::Scalar Gas<FluidSystem, RV, RVW>::density(const Scalar depth,
                                                                                          const Scalar press) const
            {
                const Scalar temp = tempVdTable_.eval(depth, /*extrapolate=*/true);
                Scalar rv = 0.0;
                ...
                if (FluidSystem::enableVaporizedOil()) {
                    if (rv >= FluidSystem::gasPvt().saturatedOilVaporizationFactor(pvtRegionIdx_, temp, press)) {
                        ...
                        bGas = FluidSystem::gasPvt().inverseFormationVolumeFactor(
                            pvtRegionIdx_, temp, press, rv, Scalar {0.0} /*=rvw*/);
                    }
                    Scalar rho = bGas * FluidSystem::referenceDensity(FluidSystem::gasPhaseIdx, pvtRegionIdx_);
                    rho += rv * bGas * FluidSystem::referenceDensity(FluidSystem::oilPhaseIdx, pvtRegionIdx_);
                    return rho;
                ...
                if (FluidSystem::enableVaporizedWater()) {
                    if (rvw >= FluidSystem::gasPvt().saturatedWaterVaporizationFactor(pvtRegionIdx_, temp, press)) {
                        ...
                        bGas = FluidSystem::gasPvt().inverseFormationVolumeFactor(
                            pvtRegionIdx_, temp, press, Scalar {0.0} /*=rv*/, rvw);
                    }
                    Scalar rho = bGas * FluidSystem::referenceDensity(FluidSystem::gasPhaseIdx, pvtRegionIdx_);
                ...
                // immiscible gas
                bGas = FluidSystem::gasPvt().inverseFormationVolumeFactor(
                    pvtRegionIdx_, temp, press, Scalar {0.0} /*=rv*/, Scalar {0.0} /*=rvw*/);
                Scalar rho = bGas * FluidSystem::referenceDensity(FluidSystem::gasPhaseIdx, pvtRegionIdx_);
        ...
        template <class ODE>
        PressureTable<FluidSystem, Region>::PressureFunction<ODE>::PressureFunction(const ODE& ode,
                                                                                    const InitCond& ic,
                                                                                    const int nsample,
                                                                                    const VSpan& span)
            : initial_(ic)
        {
            this->value_[Direction::Up]
                = std::make_unique<Distribution>(ode, VSpan {{ic.depth, span[0]}}, ic.pressure, nsample);

            this->value_[Direction::Down]
                = std::make_unique<Distribution>(ode, VSpan {{ic.depth, span[1]}}, ic.pressure, nsample);
        }

        template <class FluidSystem, class Region>
        template <class ODE>
        PressureTable<FluidSystem, Region>::PressureFunction<ODE>::PressureFunction(const PressureFunction& rhs)
            : initial_(rhs.initial_)
        {
            this->value_[Direction::Up] = std::make_unique<Distribution>(*rhs.value_[Direction::Up]);

            this->value_[Direction::Down] = std::make_unique<Distribution>(*rhs.value_[Direction::Down]);
        }

        template <class FluidSystem, class Region>
        template <class ODE>
        typename PressureTable<FluidSystem, Region>::template PressureFunction<ODE>&
        PressureTable<FluidSystem, Region>::PressureFunction<ODE>::operator=(const PressureFunction& rhs)
        {
        ...
        template <class ODE>
        typename PressureTable<FluidSystem, Region>::template PressureFunction<ODE>&
        PressureTable<FluidSystem, Region>::PressureFunction<ODE>::operator=(PressureFunction&& rhs)
        {
            this->initial_ = rhs.initial_;
        ...
        PressureTable<FluidSystem, Region>::PressureFunction<ODE>::value(const Scalar depth) const
        {
        ...
        void PressureTable<FluidSystem, Region>::checkPtr(const PressFunc* phasePress,
                                                          const std::string& phaseName) const
        {
            if (phasePress != nullptr) {
                return;
            }

            throw std::invalid_argument {"Phase pressure function for \"" + phaseName + "\" most not be null"};
        }

        template <class FluidSystem, class Region>
        typename PressureTable<FluidSystem, Region>::Strategy
        PressureTable<FluidSystem, Region>::selectEquilibrationStrategy(const Region& reg) const
        {
            if (!this->oilActive()) {
        ...
        template <class FluidSystem, class Region>
        void PressureTable<FluidSystem, Region>::copyInPointers(const PressureTable& rhs)
        {
            if (rhs.oil_ != nullptr) {
        ...
        PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::PhaseSaturations(
            MaterialLawManager& matLawMgr, const std::vector<Scalar>& swatInit)
        ...
        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::PhaseSaturations(const PhaseSaturations& rhs)
            ...
            , press_(rhs.press_)
        {
            ...
            this->setEvaluationPoint(*rhs.evalPt_.position, *rhs.evalPt_.region, *rhs.evalPt_.ptable);
        }

        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        const PhaseQuantityValue<typename FluidSystem::Scalar>&
        PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::deriveSaturations(const Position& x,
                                                                                             const Region& reg,
                                                                                             const PTable& ptable)
        {
        ...
        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        void PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::setEvaluationPoint(const Position& x,
                                                                                                   const Region& reg,
                                                                                                   const PTable& ptable)
        {
        ...
        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        void PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::initializePhaseQuantities()
        {
            this->sat_.reset();
            this->press_.reset();
        ...
        void PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::deriveGasSat()
        {
            ...
            if (this->isConstCapPress(this->gasPos())) {
                ...
                sg = this->fromDepthTable(gas_contact, this->gasPos(), isIncr);
            } else {
        ...
        void PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::deriveWaterSat()
        {
            ...
            if (!oilActive) {
                ...
                if (this->isConstCapPress(this->waterPos())) {
                    ...
                    sw = this->fromDepthTable(this->evalPt_.region->zwoc(), this->waterPos(), isIncr);
                } else {
                    // Capillary pressure curve is non-constant, meaning there is a
        ...
        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        void PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::fixUnphysicalTransition()
        {
            auto& sg = this->sat_.gas;
            auto& sw = this->sat_.water;
            ...
            sw = satFromSumOfPcs<FluidSystem>(
                this->matLawMgr_, this->waterPos(), this->gasPos(), this->evalPt_.position->cell, pcgw);
            sg = 1.0 - sw;

            this->fluidState_.setSaturation(this->oilPos(), 1.0 - sw - sg);
            this->fluidState_.setSaturation(this->gasPos(), sg);
            this->fluidState_.setSaturation(this->waterPos(), this->evalPt_.ptable->waterActive() ? sw : 0.0);
        ...
        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        void PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::accountForScaledSaturations()
        {
            ...
            const auto& scaledDrainageInfo
                = this->matLawMgr_.oilWaterScaledEpsInfoDrainage(this->evalPt_.position->cell);
        ...
        PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::applySwatInit(const Scalar pcow)
        {
        ...
        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        void PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::computeMaterialLawCapPress()
        {
            const auto& matParams = this->matLawMgr_.materialLawParams(this->evalPt_.position->cell);

            this->matLawCapPress_.fill(0.0);
            MaterialLaw::capillaryPressures(this->matLawCapPress_, matParams, this->fluidState_);
        }

        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        typename FluidSystem::Scalar
        PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::materialLawCapPressGasOil() const
        {
            return this->matLawCapPress_[this->oilPos()] + this->matLawCapPress_[this->gasPos()];
        }

        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        typename FluidSystem::Scalar
        PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::materialLawCapPressOilWater() const
        {
            return this->matLawCapPress_[this->oilPos()] - this->matLawCapPress_[this->waterPos()];
        }

        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        typename FluidSystem::Scalar
        PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::materialLawCapPressGasWater() const
        {
            return this->matLawCapPress_[this->gasPos()] - this->matLawCapPress_[this->waterPos()];
        }

        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        bool PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::isConstCapPress(
            const PhaseIdx phaseIdx) const
        {
            return isConstPc<FluidSystem>(this->matLawMgr_, phaseIdx, this->evalPt_.position->cell);
        }

        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        bool PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::isOverlappingTransition() const
        {
            return this->evalPt_.ptable->gasActive() && this->evalPt_.ptable->waterActive()
                && ((this->sat_.gas + this->sat_.water) > 1.0);
        }

        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        typename FluidSystem::Scalar PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::fromDepthTable(
            const Scalar contactdepth, const PhaseIdx phasePos, const bool isincr) const
        {
            return satFromDepth<FluidSystem>(this->matLawMgr_,
                                             this->evalPt_.position->depth,
                                             contactdepth,
                                             static_cast<int>(phasePos),
                                             this->evalPt_.position->cell,
                                             isincr);
        }

        template <class MaterialLawManager, class FluidSystem, class Region, typename CellID>
        typename FluidSystem::Scalar PhaseSaturations<MaterialLawManager, FluidSystem, Region, CellID>::invertCapPress(
            const Scalar pc, const PhaseIdx phasePos, const bool isincr) const
        {
            return satFromPc<FluidSystem>(
                this->matLawMgr_, static_cast<int>(phasePos), this->evalPt_.position->cell, pc, isincr);
        }

        template <class FluidSystem, class Region>
        PressureTable<FluidSystem, Region>::PressureTable(const Scalar gravity, const int samplePoints)
            : gravity_(gravity)
        ...
        template <class FluidSystem, class Region>
        PressureTable<FluidSystem, Region>::PressureTable(const PressureTable<FluidSystem, Region>& rhs)
        ...
        template <class FluidSystem, class Region>
        PressureTable<FluidSystem, Region>::PressureTable(PressureTable<FluidSystem, Region>&& rhs)
            : gravity_(rhs.gravity_)
            , nsample_(rhs.nsample_)
        ...
        PressureTable<FluidSystem, Region>::operator=(const PressureTable<FluidSystem, Region>& rhs)
        {
        ...
        template <class FluidSystem, class Region>
        PressureTable<FluidSystem, Region>&
        PressureTable<FluidSystem, Region>::operator=(PressureTable<FluidSystem, Region>&& rhs)
        {
            this->gravity_ = rhs.gravity_;
            this->nsample_ = rhs.nsample_;
        ...
        template <class FluidSystem, class Region>
        void PressureTable<FluidSystem, Region>::equilibrate(const Region& reg, const VSpan& span)
        {
        ...
        template <class FluidSystem, class Region>
        bool PressureTable<FluidSystem, Region>::oilActive() const
        {
            return FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx);
        }

        template <class FluidSystem, class Region>
        bool PressureTable<FluidSystem, Region>::gasActive() const
        {
            return FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx);
        }

        template <class FluidSystem, class Region>
        bool PressureTable<FluidSystem, Region>::waterActive() const
        {
            return FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx);
        }

        template <class FluidSystem, class Region>
        typename FluidSystem::Scalar PressureTable<FluidSystem, Region>::oil(const Scalar depth) const
        {
        ...
        template <class FluidSystem, class Region>
        typename FluidSystem::Scalar PressureTable<FluidSystem, Region>::gas(const Scalar depth) const
        {
        ...
        template <class FluidSystem, class Region>
        typename FluidSystem::Scalar PressureTable<FluidSystem, Region>::water(const Scalar depth) const
        {
        ...
        template <class FluidSystem, class Region>
        void PressureTable<FluidSystem, Region>::equil_WOG(const Region& reg, const VSpan& span)
        {
            // Datum depth in water zone.  Calculate phase pressure for water first,
            // followed by oil and gas if applicable.

            if (!this->waterActive()) {
                throw std::invalid_argument {"Don't know how to interpret EQUIL datum depth in "
                                             "WATER zone in model without active water phase"};
            }

            {
                const auto ic = typename WPress::InitCond {reg.datum(), reg.pressure()};
            ...
            if (this->oilActive()) {
                // Pcow = Po - Pw => Po = Pw + Pcow
                const auto ic = typename OPress::InitCond {reg.zwoc(), this->water(reg.zwoc()) + reg.pcowWoc()};
            ...
            if (this->gasActive() && this->oilActive()) {
                // Pcgo = Pg - Po => Pg = Po + Pcgo
                const auto ic = typename GPress::InitCond {reg.zgoc(), this->oil(reg.zgoc()) + reg.pcgoGoc()};
        ...
        template <class FluidSystem, class Region>
        void PressureTable<FluidSystem, Region>::equil_GOW(const Region& reg, const VSpan& span)
        {
            ...
            {
                const auto ic = typename GPress::InitCond {reg.datum(), reg.pressure()};
            ...
            if (this->oilActive()) {
                // Pcgo = Pg - Po => Po = Pg - Pcgo
                const auto ic = typename OPress::InitCond {reg.zgoc(), this->gas(reg.zgoc()) - reg.pcgoGoc()};
                this->makeOilPressure(ic, reg, span);
            }

            if (this->waterActive() && this->oilActive()) {
                // Pcow = Po - Pw => Pw = Po - Pcow
                const auto ic = typename WPress::InitCond {reg.zwoc(), this->oil(reg.zwoc()) - reg.pcowWoc()};
        ...
        template <class FluidSystem, class Region>
        void PressureTable<FluidSystem, Region>::equil_OWG(const Region& reg, const VSpan& span)
        {
            ...
            {
                const auto ic = typename OPress::InitCond {reg.datum(), reg.pressure()};
        ...
        template <class FluidSystem, class Region>
        void PressureTable<FluidSystem, Region>::makeOilPressure(const typename OPress::InitCond& ic,
                                                                 const Region& reg,
                                                                 const VSpan& span)
        {
            const auto drho
                = OilPressODE {reg.tempVdTable(), reg.dissolutionCalculator(), reg.pvtIdx(), this->gravity_};

            this->oil_ = std::make_unique<OPress>(drho, ic, this->nsample_, span);
        }

        template <class FluidSystem, class Region>
        void PressureTable<FluidSystem, Region>::makeGasPressure(const typename GPress::InitCond& ic,
                                                                 const Region& reg,
                                                                 const VSpan& span)
        {
            const auto drho = GasPressODE {reg.tempVdTable(),
        ...
        template <class FluidSystem, class Region>
        void PressureTable<FluidSystem, Region>::makeWatPressure(const typename WPress::InitCond& ic,
                                                                 const Region& reg,
                                                                 const VSpan& span)
        {
            const auto drho = WatPressODE {reg.tempVdTable(), reg.saltVdTable(), reg.pvtIdx(), this->gravity_};
    ...
    namespace DeckDependent
    {
        ...
        std::vector<int> equilnum(const EclipseState& eclipseState, const GridView& gridview)
        {
            ...
            if (std::any_of(eqlnum.begin(), eqlnum.end(), [num_regions](int n) { return n >= num_regions; })) {
                throw std::runtime_error("Values larger than maximum Equil regions " + std::to_string(num_regions)
                                         + " provided in EQLNUM");
            }
        ...
        InitialStateComputer<FluidSystem, Grid, GridView, ElementMapper, CartesianIndexMapper>::InitialStateComputer(
            ...
            , pp_(FluidSystem::numPhases, std::vector<Scalar>(grid.size(/*codim=*/0)))
            , sat_(FluidSystem::numPhases, std::vector<Scalar>(grid.size(/*codim=*/0)))
            ...
            , num_pressure_points_(num_pressure_points)
        {
                ...
                for (std::size_t i = 0; i < rec.size(); ++i) {
                    ...
                    if (!rec[i].liveOilInitConstantRs()) {
                        ...
                        if (rsvdTables.size() > 0) {
                            ...
                            rsFunc_.push_back(
                                std::make_shared<Miscibility::RsVD<FluidSystem>>(pvtIdx, depthColumn, rsColumn));
                            ...
                            rsFunc_.push_back(
                                std::make_shared<Miscibility::PBVD<FluidSystem>>(pvtIdx, depthColumn, pbubColumn));
                        ...
                        rsFunc_.push_back(
                            std::make_shared<Miscibility::RsSatAtContact<FluidSystem>>(pvtIdx, pContact, TContact));
                    }
                }
            } else {
                for (std::size_t i = 0; i < rec.size(); ++i) {
                ...
                for (std::size_t i = 0; i < rec.size(); ++i) {
                    ...
                    if (!rec[i].wetGasInitConstantRv()) {
                        ...
                        if (rvvdTables.size() > 0) {
                            ...
                            rvFunc_.push_back(
                                std::make_shared<Miscibility::RvVD<FluidSystem>>(pvtIdx, depthColumn, rvColumn));
                            ...
                            rvFunc_.push_back(
                                std::make_shared<Miscibility::PDVD<FluidSystem>>(pvtIdx, depthColumn, pdewColumn));
                        } else {
                            throw std::runtime_error("Cannot initialise: RVVD or PDCD table not available.");
                        }
                    } else {
                        if (rec[i].gasOilContactDepth() != rec[i].datumDepth()) {
                            throw std::runtime_error("Cannot initialise: when no explicit RVVD table is given, \n"
                                                     "datum depth must be at the gas-oil-contact. "
                                                     "In EQUIL region "
                                                     + std::to_string(i + 1)
                                                     + " (counting from 1), this does not hold.");
                        ...
                        const Scalar TContact = 273.15 + 20; // standard temperature for now
                        rvFunc_.push_back(
                            std::make_shared<Miscibility::RvSatAtContact<FluidSystem>>(pvtIdx, pContact, TContact));
                    }
                }
            } else {
                for (std::size_t i = 0; i < rec.size(); ++i) {
                ...
                for (std::size_t i = 0; i < rec.size(); ++i) {
                    ...
                    if (!rec[i].humidGasInitConstantRvw()) {
                        ...
                        if (rvwvdTables.size() > 0) {
                            ...
                            rvwFunc_.push_back(
                                std::make_shared<Miscibility::RvwVD<FluidSystem>>(pvtIdx, depthColumn, rvwvdColumn));
                        } else {
                        ...
                        if (oilActive) {
                            if (rec[i].gasOilContactDepth() != rec[i].datumDepth()) {
                                ...
                                const auto msg = "No explicit RVWVD table is given for EQUIL region "
                                    + std::to_string(i + 1)
                                    + ". \n"
                                ...
                                // pg = po + Pcgo = po + (pg - po)
                                // for gas-condensate with initial no oil zone: water-oil contact depth (OWC) equal
                                // gas-oil contact depth (GOC)
                                const Scalar pContact
                                    = rec[i].datumDepthPressure() + rec[i].gasOilContactCapillaryPressure();
                                const Scalar TContact = 273.15 + 20; // standard temperature for now
                                rvwFunc_.push_back(std::make_shared<Miscibility::RvwSatAtContact<FluidSystem>>(
                                    pvtIdx, pContact, TContact));
                            }
                        } else {
                            // two-phase gas-water sytem:  water-oil contact depth is taken equal to gas-water contact
                            // depth (GWC) and water-oil capillary pressure (Pcwo) is taken equal to gas-water capillary
                            // pressure (Pcgw) at GWC
                            if (rec[i].waterOilContactDepth() != rec[i].datumDepth()) {
                                ...
                                // pg = pw + Pcgw = pw + (pg - pw)
                                const Scalar pContact
                                    = rec[i].datumDepthPressure() + rec[i].waterOilContactCapillaryPressure();
                                const Scalar TContact = 273.15 + 20; // standard temperature for now
                                rvwFunc_.push_back(std::make_shared<Miscibility::RvwSatAtContact<FluidSystem>>(
                                    pvtIdx, pContact, TContact));
                            }
            ...
            applyNumericalAquifers_(
                gridView, num_aquifers, eclipseState.runspec().co2Storage() || eclipseState.runspec().h2Storage());
            ...
            updateInitialTemperature_(const EclipseState& eclState, const RMap& reg)
        {
            ...
            if (!tables.hasTables("RTEMPVD")) {
                ...
                std::vector<Scalar> y = {static_cast<Scalar>(tables.rtemp()), static_cast<Scalar>(tables.rtemp())};
        ...
        template <class FluidSystem, class Grid, class GridView, class ElementMapper, class CartesianIndexMapper>
        void InitialStateComputer<FluidSystem, Grid, GridView, ElementMapper, CartesianIndexMapper>::updateCellProps_(
            const GridView& gridView, const NumericalAquifers& aquifer)
        {
        ...
        template <class FluidSystem, class Grid, class GridView, class ElementMapper, class CartesianIndexMapper>
        void
        InitialStateComputer<FluidSystem, Grid, GridView, ElementMapper, CartesianIndexMapper>::applyNumericalAquifers_(
            const GridView& gridView, const NumericalAquifers& aquifer, const bool co2store_or_h2store)
        {
            const auto num_aqu_cells = aquifer.allAquiferCells();
            if (num_aqu_cells.empty())
                return;
        ...
        template <class FluidSystem, class Grid, class GridView, class ElementMapper, class CartesianIndexMapper>
        template <class RMap>
        void InitialStateComputer<FluidSystem, Grid, GridView, ElementMapper, CartesianIndexMapper>::setRegionPvtIdx(
            const EclipseState& eclState, const RMap& reg)
        {
        ...
        void InitialStateComputer<FluidSystem, Grid, GridView, ElementMapper, CartesianIndexMapper>::calcPressSatRsRv(
            const RMap& reg,
        ...
        {
            using PhaseSat
                = Details::PhaseSaturations<MaterialLawManager, FluidSystem, EquilReg<Scalar>, typename RMap::CellId>;
            ...
            for (std::size_t r = 0; r < rec.size(); ++r) {
                ...
                if (acc > 0) {
                    throw std::runtime_error {"Cannot initialise model: Positive item 9 is not supported "
                                              "in EQUIL keyword, record "
                                              + std::to_string(r + 1)};
                }
                ...
                const auto eqreg = EquilReg {rec[r],
                                             this->rsFunc_[r],
                                             this->rvFunc_[r],
                                             this->rvwFunc_[r],
                                             this->tempVdTable_[r],
                                             this->saltVdTable_[r],
                                             this->regionPvtIdx_[r]};
            ...
            if (comm.rank() == 0) {
                for (std::size_t r = 0; r < rec.size(); ++r) {
                    if (regionIsEmpty[r]) // region is empty on all partitions
                        OpmLog::warning("Equilibration region " + std::to_string(r + 1) + " has no active cells");
                }
            }
        }
        ...
        void InitialStateComputer<FluidSystem, Grid, GridView, ElementMapper, CartesianIndexMapper>::cellLoop(
            const CellRange& cells, EquilibrationMethod&& eqmethod)
        {
            const auto oilPos = FluidSystem::oilPhaseIdx;
            const auto gasPos = FluidSystem::gasPhaseIdx;
        ...
        void
        InitialStateComputer<FluidSystem, Grid, GridView, ElementMapper, CartesianIndexMapper>::equilibrateCellCentres(
            const CellRange& cells, const EquilReg<Scalar>& eqreg, const PressTable& ptable, PhaseSat& psat)
        {
            using CellPos = typename PhaseSat::Position;
            using CellID = std::remove_cv_t<std::remove_reference_t<decltype(std::declval<CellPos>().cell)>>;
            this->cellLoop(cells,
                           [this, &eqreg, &ptable, &psat](const CellID cell,
                               ...
                               Rs = eqreg.dissolutionCalculator()(pos.depth, pressures.oil, temp, saturations.gas);

                               Rv = eqreg.evaporationCalculator()(pos.depth, pressures.gas, temp, saturations.oil);
        ...
        template <class CellRange, class PressTable, class PhaseSat>
        void
        InitialStateComputer<FluidSystem, Grid, GridView, ElementMapper, CartesianIndexMapper>::equilibrateHorizontal(
            ...
            PhaseSat& psat)
        {
            ...
            this->cellLoop(cells,
                           [this, acc, &eqreg, &ptable, &psat](const CellID cell,
                                                               Details::PhaseQuantityValue<Scalar>& pressures,
                                                               Details::PhaseQuantityValue<Scalar>& saturations,
                                                               Scalar& Rs,
                                                               Scalar& Rv,
                               ...
                               Scalar totfrac = 0.0;
                               for (const auto& [depth, frac] :
                                    Details::horizontalSubdivision(cell, cellZSpan_[cell], acc)) {
