...
void
MechContainer<Scalar>::allocate(const std::size_t bufferSize, std::map<std::string, int>& rstKeywords)
{
    ...
    std::for_each(disp_.begin(), disp_.end(), [suffix = "XYZ", is = 0, bufferSize, &rstKeywords](auto& v) mutable {
        v.resize(bufferSize, 0.0);
        rstKeywords[std::string {"DISP"} + suffix[is++]] = 0;
    });

    auto resizeAndRegister = [&rstKeywords, bufferSize](auto& tensor, const std::string& name) {
...
template <class Scalar>
void
MechContainer<Scalar>::assignDelStress(const unsigned globalDofIdx, const Dune::FieldVector<Scalar, 6>& delStress)
{
...
template <class Scalar>
void
MechContainer<Scalar>::assignDisplacement(const unsigned globalDofIdx, const Dune::FieldVector<Scalar, 3>& disp)
{
...
template <class Scalar>
void
MechContainer<Scalar>::assignFracStress(const unsigned globalDofIdx, const Dune::FieldVector<Scalar, 6>& fracStress)
{
...
template <class Scalar>
void
MechContainer<Scalar>::assignLinStress(const unsigned globalDofIdx, const Dune::FieldVector<Scalar, 6>& linStress)
{
...
template <class Scalar>
void
MechContainer<Scalar>::assignPotentialForces(const unsigned globalDofIdx,
                                             const Scalar force,
...
template <class Scalar>
void
MechContainer<Scalar>::assignStrain(const unsigned globalDofIdx, const Dune::FieldVector<Scalar, 6>& strain)
{
...
template <class Scalar>
void
MechContainer<Scalar>::assignStress(const unsigned globalDofIdx, const Dune::FieldVector<Scalar, 6>& stress)
{
...
template <class Scalar>
void
MechContainer<Scalar>::outputRestart(data::Solution& sol)
{
    if (!allocated_) {
        return;
    }
    using DataEntry
        = std::tuple<std::string,
                     UnitSystem::measure,
                     std::variant<std::vector<Scalar>*, std::array<std::vector<Scalar>, 3>*, VoigtArray<Scalar>*>>;

    auto doInsert = [&sol](const std::string& name, const UnitSystem::measure& measure, std::vector<Scalar>& entry) {
        if (!entry.empty()) {
    ...
    std::for_each(solutionVectors.begin(), solutionVectors.end(), [&doInsert](auto& array) {
        std::visit(VisitorOverloadSet {[&array, &doInsert](std::vector<Scalar>* v) {
