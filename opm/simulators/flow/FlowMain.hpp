...
public:
    ...
    static int setupParameters_(int argc, char** argv, Parallel::Communication comm)
    {
        ...
        Parameters::Register<Parameters::DebugVerbosityLevel>(
            "Set debug verbosity level globally. Default is 1, increasing values give additional output and 0 disables "
            "most messages to the .DBG file");
    ...
    static void setMaxThreads()
    {
        ...
        if (env_var) {
            ...
            if (result.ec == std::errc() && omp_num_threads > 0) {
                ...
                if (isSet) {
                    OpmLog::warning("Environment variable OMP_NUM_THREADS takes precedence over the "
                                    "--threads-per-process cmdline argument.");
                }
            } else {
