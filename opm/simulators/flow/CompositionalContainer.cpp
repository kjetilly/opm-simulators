...
void
CompositionalContainer<FluidSystem>::allocate(const unsigned bufferSize, std::map<std::string, int>& rstKeywords)
{
...
template <class FluidSystem>
void
CompositionalContainer<FluidSystem>::assignGasFractions(const unsigned globalDofIdx, const AssignFunction& fractions)
{
    ...
    std::for_each(phaseMoleFractions_[gasPhaseIdx].begin(),
                  ...
                  [globalDofIdx, &fractions, c = 0](auto& comp) mutable { comp[globalDofIdx] = fractions(c++); });
}

template <class FluidSystem>
void
CompositionalContainer<FluidSystem>::assignMoleFractions(const unsigned globalDofIdx, const AssignFunction& fractions)
{
    if (moleFractions_.empty()) {
        return;
    }

    std::for_each(moleFractions_.begin(), moleFractions_.end(), [&fractions, globalDofIdx, c = 0](auto& comp) mutable {
...
template <class FluidSystem>
void
CompositionalContainer<FluidSystem>::assignOilFractions(const unsigned globalDofIdx, const AssignFunction& fractions)
{
...
template <class FluidSystem>
void
CompositionalContainer<FluidSystem>::outputRestart(data::Solution& sol, ScalarBuffer& oil_saturation)
{
    ...
    if (!phaseMoleFractions_[oilPhaseIdx][0].empty()) {
        for (int i = 0; i < numComponents; ++i) {
            ...
            entries.emplace_back(name, UnitSystem::measure::identity, phaseMoleFractions_[oilPhaseIdx][i]);
        }
    }
    ...
    if (!phaseMoleFractions_[gasPhaseIdx][0].empty()) {
        for (int i = 0; i < numComponents; ++i) {
            ...
            entries.emplace_back(name, UnitSystem::measure::identity, phaseMoleFractions_[gasPhaseIdx][i]);
        }
    }
