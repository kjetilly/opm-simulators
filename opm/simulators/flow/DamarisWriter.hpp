...
public:
    ...
    explicit DamarisWriter(Simulator& simulator)
        : BaseType(simulator.vanguard().schedule(),
                   ...
                   ((simulator.vanguard().grid().comm().rank() == 0) ? &simulator.vanguard().equilCartesianIndexMapper()
                                                                     : nullptr),
                   false,
                   false)
        , simulator_(simulator)
    {
        ...
        if (this->nranks_ > 1) {
            auto smryCfg = (this->rank_ == 0) ? this->eclIO_->finalSummaryConfig() : SummaryConfig {};

            eclBroadcast(this->simulator_.vanguard().grid().comm(), smryCfg);

            this->damarisOutputModule_
                = std::make_unique<OutputBlackOilModule<TypeTag>>(simulator, smryCfg, this->collectOnIORank_);
        } else {
    ...
    void writeOutput(data::Solution& localCellData, bool isSubStep)
    {
        ...
        if (!isSubStep) {
            ...
            if (damarisUpdate_ == true) {
                ...
                // this function writes the mesh data directly to Damaris shared memory using
                // Opm::DamarisOutput::DamarisVar objects.
            ...
            for (const auto& damVar : localCellData) {
                ...
                if (wanted_vars_set_.count(name) || wanted_vars_set_.empty()) {
                    ...
                    try {
                        if (dataCol.data<double>().size()
                            >= static_cast<std::vector<double>::size_type>(this->numElements_)) {
                            dam_err_ = DamarisOutput::write(name.c_str(), dataCol.data<double>().data());
                        } else {
                            OpmLog::info(fmt::format(
                                "( rank:{}) The variable \"{}\" was found to be of a different size {} (not {}).",
                                rank_,
                                name,
                                dataCol.data<double>().size(),
                                this->numElements_));
                        }
                    } catch (std::bad_variant_access const& ex) {
                        // Not a std::vector<double>, must be a std::vector<int>
                        if (dataCol.data<int>().size()
                            >= static_cast<std::vector<int>::size_type>(this->numElements_)) {
            ...
            if (!cell_data_written) {
                OpmLog::info(fmt::format(
                    "( rank:{}) No simulation data written to the Damaris server - check --damaris-limit-variables "
                    "command line option (if used) has valid variable name(s) and that the Damaris XML file contains "
                    "variable names that are available in your simulation.",
                    rank_));
            } else {
            ...
            /*
             ...
             for ( auto damVar : mybloc ) {
               ...
               std::cout << "Name of Damaris Block Varaiable : (" << rank_ << ")  "  << name  << "  part : " << part <<
             "  Value : "  << dataCol <<  std::endl ;
             }
...
private:
    ...
    bool damarisUpdate_ = false; ///< Whenever this is true writeOutput() will set up Damaris mesh information and
                                 ///< offsets of model fields
    ...
    void setGlobalIndexForDamaris()
    {
        ...
        if (this->collectOnIORank_.isParallel()) {
            const std::vector<int>& local_to_global = this->collectOnIORank_.localIdxToGlobalIdxMapping();
    ...
    void writeDamarisGridOutput()
    {
        ...
        try {
            ...
            if (hasPolyCells) {
                OpmLog::error(fmt::format("ERORR: rank {} The DUNE geometry grid has polyhedral elements - These "
            ...
            // <parameter name="n_coords_global"    type="int" value="1" comment="only needed if we need to write to
            // HDF5 in Collective mode"/> <layout    name="n_coords_layout"    type="double" dimensions="n_coords_local"
            // comment="For the individual x, y and z coordinates of the mesh vertices"  /> <group
            // name="coordset/coords/values">
            //     <variable name="x"    layout="n_coords_layout"  type="scalar"  visualizable="false"  unit="m"
            //     script="PythonConduitTest" time-varying="false" /> <variable name="y"    layout="n_coords_layout"
            //     type="scalar"  visualizable="false"  unit="m"   script="PythonConduitTest" time-varying="false" />
            //     <variable name="z"    layout="n_coords_layout"  type="scalar"  visualizable="false"  unit="m"
            //     script="PythonConduitTest" time-varying="false" />
            // </group>
            ...
            // Now we can use the shared memory area that Damaris has allocated and use it to write the x,y,z
            // coordinates
            if (geomData.writeGridPoints(var_x, var_y, var_z) < 0)
                DUNE_THROW(Dune::IOError, geomData.getError());

            //  This is the template XML model for connectivity, offsets and types, as defined in initDamarisXmlFile.cpp
            //  which is used to build the internally generated Damaris XML configuration file.
            // <parameter name="n_connectivity_ph"        type="int"  value="1" />
            // <layout    name="n_connections_layout_ph"  type="int"  dimensions="n_connectivity_ph"   comment="Layout
            // for connectivities "  /> <parameter name="n_offsets_types_ph"       type="int"  value="1" /> <layout
            // name="n_offsets_layout_ph"      type="int"  dimensions="n_offsets_types_ph+1"  comment="Layout for the
            // offsets_ph"  /> <layout    name="n_types_layout_ph"        type="char" dimensions="n_offsets_types_ph"
            // comment="Layout for the types_ph "  /> <group name="topologies/topo/elements">
            //     <variable name="connectivity" layout="n_connections_layout_ph"  type="scalar"  visualizable="false"
            //     unit=""   script="PythonConduitTest" time-varying="false" /> <variable name="offsets"
            //     layout="n_offsets_layout_ph"    type="scalar"  visualizable="false"  unit=""
            //     script="PythonConduitTest" time-varying="false" /> <variable name="types" layout="n_types_layout_ph"
            //     type="scalar"  visualizable="false"  unit=""   script="PythonConduitTest" time-varying="false" />
            // </group>

            DamarisVarInt var_connectivity(1, {"n_connectivity_ph"}, "topologies/topo/elements/connectivity", rank_);
            var_connectivity.setDamarisParameterAndShmem({geomData.getNCorners()});
            DamarisVarInt var_offsets(1, {"n_offsets_types_ph"}, "topologies/topo/elements/offsets", rank_);
            var_offsets.setDamarisParameterAndShmem({geomData.getNCells() + 1});
            DamarisVarChar var_types(1, {"n_offsets_types_ph"}, "topologies/topo/elements/types", rank_);
    ...
    void prepareLocalCellData(const bool isSubStep, const int reportStepNum)
    {
        ...
        const int num_interior = detail::countLocalInteriorCellsGridView(gridView);
        const bool log = this->collectOnIORank_.isIORank();

        damarisOutputModule_->allocBuffers(num_interior, reportStepNum, isSubStep, log, /*isRestart*/ false);
