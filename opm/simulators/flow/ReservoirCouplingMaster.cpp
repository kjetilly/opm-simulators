...
ReservoirCouplingMaster::ReservoirCouplingMaster(const Parallel::Communication& comm,
...
bool
ReservoirCouplingMaster::isMasterGroup(const std::string& group_name) const
{
    return this->master_group_slave_names_.find(group_name) != this->master_group_slave_names_.end();
}

std::size_t
ReservoirCouplingMaster::getMasterGroupPotIdx(const std::string& slave_name, const std::string& master_group_name) const
{
    return this->master_group_name_order_.at(slave_name).at(master_group_name);
}

const ReservoirCoupling::Potentials&
ReservoirCouplingMaster::getSlaveGroupPotentials(const std::string& master_group_name)
{
    ...
    if (it != this->master_group_slave_names_.end()) {
        ...
        OPM_THROW(
            std::runtime_error,
            fmt::format("Master group name {} not found in master-to-slave-group-name mapping", master_group_name));
    }
}

void
ReservoirCouplingMaster::maybeActivate(int report_step)
{
    if (!this->activated()) {
...
void
ReservoirCouplingMaster::maybeSpawnSlaveProcesses(int report_step)
{
...
double
ReservoirCouplingMaster::maybeChopSubStep(double suggested_timestep_original, double elapsed_time) const
{
...
void
ReservoirCouplingMaster::maybeReceiveActivationHandshakeFromSlaves(double current_time)
{
        ...
        for (unsigned int i = 0; i < this->numSlavesStarted(); i++) {
            ...
            if (Seconds::compare_eq(slave_activation_date, current_date)) {
                ...
                if (!flag) {
                    ...
                    OpmLog::info(fmt::format("Waiting for activation handshake from slave {}", this->slave_names_[i]));
                }
                ...
                OpmLog::info(fmt::format("Received activation handshake from slave {}", this->slave_names_[i]));
            }
        }
    }
    // Broadcast status to all ranks
    comm_.broadcast(this->slave_activation_status_.data(), /*count=*/this->numSlavesStarted(), /*emitter_rank=*/0);
}

std::size_t
ReservoirCouplingMaster::numSlaveGroups(unsigned int index)
{
    return this->master_group_name_order_[this->slave_names_[index]].size();
}

std::size_t
ReservoirCouplingMaster::numSlavesStarted() const
{
    return this->slave_names_.size();
}

void
ReservoirCouplingMaster::sendNextTimeStepToSlaves(double timestep)
{
    if (this->comm_.rank() == 0) {
        for (unsigned int i = 0; i < this->master_slave_comm_.size(); i++) {
            if (!this->slaveIsActivated(i)) {
                OpmLog::info(fmt::format("Slave {} has not activated yet, skipping sending next time step to slave",
                                         this->slave_names_[i]));
                continue;
            }
            ...
            MPI_Send(&timestep,
                     ...
                     this->getSlaveComm(i));
            OpmLog::info(fmt::format("Sent next time step {} from master process rank 0 to slave process "
                                     "rank 0 with name: {}",
                                     timestep,
                                     this->slave_names_[i]));
        }
...
void
ReservoirCouplingMaster::receiveNextReportDateFromSlaves()
{
    auto num_slaves = this->numSlavesStarted();
    if (this->comm_.rank() == 0) {
        ...
        for (unsigned int i = 0; i < num_slaves; i++) {
            ...
            // NOTE: See comment about error handling at the top of this file.
            MPI_Recv(&slave_next_report_time_offset,
                     /*count=*/1,
                     /*datatype=*/MPI_DOUBLE,
                     /*source_rank=*/0,
            ...
            OpmLog::info(fmt::format("Received simulation slave next report date from slave process with name: {}. "
                                     "Next report date: {}",
                                     this->slave_names_[i],
                                     slave_next_report_time_offset));
        }
    }
    this->comm_.broadcast(this->slave_next_report_time_offsets_.data(), /*count=*/num_slaves, /*emitter_rank=*/0);
    OpmLog::info("Broadcasted slave next report dates to all ranks");
}

void
ReservoirCouplingMaster::receivePotentialsFromSlaves()
{
    auto num_slaves = this->numSlavesStarted();
        ...
        if (this->comm_.rank() == 0) {
            if (this->slaveIsActivated(i)) {
                ...
                MPI_Recv(potentials.data(),
                         ...
                         this->getSlaveComm(i),
                         MPI_STATUS_IGNORE);
                this->logger_.info(fmt::format("Received potentials from slave process with name: {}. "
                                               "Number of slave groups: {}",
                                               this->slave_names_[i],
                                               num_slave_groups));
            } else {
                this->logger_.info(fmt::format(
                    "Slave {} has not activated yet, skipping receiving potentials from slave", this->slave_names_[i]));
...
void
ReservoirCouplingMaster::updateMasterGroupNameOrderMap(const std::string& slave_name,
                                                       const std::map<std::string, std::size_t>& master_group_name_map)
{
...
double
ReservoirCouplingMaster::getMasterActivationDate_() const
{
    ...
    OPM_THROW(std::runtime_error,
              "Reservoir coupling: Failed to find master activation time: "
