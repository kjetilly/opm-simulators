...
template <class TypeTag>
PyBaseSimulator<TypeTag>::PyBaseSimulator(std::shared_ptr<Deck> deck,
...
template <class TypeTag>
void
PyBaseSimulator<TypeTag>::setPorosity(PyCArray array)
{
...
template <class TypeTag>
void
PyBaseSimulator<TypeTag>::setPrimaryVariable(const std::string& variable, PyCArray array)
{
    std::size_t size_ = array.size();
...
PyBaseSimulator<TypeTag>::stepInit()
{
    ...
    if (this->deck_) {
        this->main_ = std::make_unique<PyMain<TypeTag>>(this->deck_->getDataFile(),
                                                        this->eclipse_state_,
                                                        this->schedule_,
        ...
        this->main_ = std::make_unique<PyMain<TypeTag>>(this->deck_filename_, this->mpi_init_, this->mpi_finalize_);
    }
