...
std::string
get_error_report(const std::vector<ValidationError>& errors,
    ...
    if (treat_critical_as_noncritical) {
        ...
        if (!error_report.empty()) {
            OpmLog::info("\nOPM Flow will terminate due to unsupported critical keywords.\n"
                         ...
                         "If you need to override this behaviour, you can use the command line argument "
...
void
KeywordValidator::validateDeckKeyword(const DeckKeyword& keyword, std::vector<ValidationError>& errors) const
{
...
void
KeywordValidator::validateKeywordItems(const DeckKeyword& keyword,
...
template <typename T>
void
KeywordValidator::validateKeywordItem(const DeckKeyword& keyword,
