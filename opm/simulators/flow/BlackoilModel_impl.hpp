...
BlackoilModel<TypeTag>::BlackoilModel(Simulator& simulator,
...
BlackoilModel<TypeTag>::prepareStep(const SimulatorTimerInterface& timer)
{
...
template <class TypeTag>
void
BlackoilModel<TypeTag>::initialLinearization(SimulatorReportSingle& report,
                                             const int iteration,
                                             const int minIter,
                                             const int maxIter,
    ...
    // the step is not considered converged until at least minIter iterations is done
    {
        ...
        if (severity == ConvergenceReport::Severity::NotANumber) {
            ...
            OPM_THROW_PROBLEM(
                ...
                fmt::format("Total penalty count exceeded cut-off-limit of {}", param_.monitor_params_.cutoff_));
        }
    }
...
SimulatorReportSingle
BlackoilModel<TypeTag>::nonlinearIteration(const int iteration,
                                           const SimulatorTimerInterface& timer,
...
template <class NonlinearSolverType>
SimulatorReportSingle
BlackoilModel<TypeTag>::nonlinearIterationNewton(const int iteration,
                                                 const SimulatorTimerInterface& timer,
                                                 NonlinearSolverType& nonlinear_solver)
{
    // -----------   Set up reports and timer   -----------
    ...
    this->initialLinearization(report, iteration, this->param_.newton_min_iter_, this->param_.newton_max_iter_, timer);
        ...
        if (param_.use_update_stabilization_) {
            ...
            if (isOscillate) {
                ...
                if (terminalOutputEnabled()) {
                    std::string msg
                        = "    Oscillating behavior detected: Relaxation set to " + std::to_string(current_relaxation_);
                    OpmLog::info(msg);
                }
            }
...
template <class TypeTag>
SimulatorReportSingle
BlackoilModel<TypeTag>::afterStep(const SimulatorTimerInterface&)
{
    SimulatorReportSingle report;
    Dune::Timer perfTimer;
    perfTimer.start();
...
template <class TypeTag>
SimulatorReportSingle
BlackoilModel<TypeTag>::assembleReservoir(const SimulatorTimerInterface& /* timer */, const int iterationIdx)
{
...
BlackoilModel<TypeTag>::relativeChange() const
{
...
template <class TypeTag>
void
BlackoilModel<TypeTag>::solveJacobianSystem(BVector& x)
{
...
template <class TypeTag>
void
BlackoilModel<TypeTag>::updateSolution(const BVector& dx)
{
...
template <class TypeTag>
std::tuple<typename BlackoilModel<TypeTag>::Scalar, typename BlackoilModel<TypeTag>::Scalar>
BlackoilModel<TypeTag>::convergenceReduction(Parallel::Communication comm,
...
template <class TypeTag>
std::pair<typename BlackoilModel<TypeTag>::Scalar, typename BlackoilModel<TypeTag>::Scalar>
BlackoilModel<TypeTag>::localConvergenceData(std::vector<Scalar>& R_sum,
                                             std::vector<Scalar>& maxCoeff,
                                             std::vector<Scalar>& B_avg,
                                             std::vector<int>& maxCoeffCell)
{
    ...
    for (const auto& elem : elements(gridView, Dune::Partitions::interior)) {
        ...
        const auto pvValue = problem.referencePorosity(cell_idx, /*timeIdx=*/0) * model.dofTotalVolume(cell_idx);
        pvSumLocal += pvValue;
        ...
        this->getMaxCoeff(cell_idx, intQuants, fs, residual, pvValue, B_avg, R_sum, maxCoeff, maxCoeffCell);
    }
...
BlackoilModel<TypeTag>::characteriseCnvPvSplit(const std::vector<Scalar>& B_avg, const double dt)
{
    ...
    auto cnvPvSplit = std::pair<std::vector<double>, std::vector<int>> {
        std::piecewise_construct, std::forward_as_tuple(numPvGroups), std::forward_as_tuple(numPvGroups)};
    ...
    for (const auto& elem : elements(gridView, Dune::Partitions::interior)) {
        ...
        const auto ix = (maxCnv > this->param_.tolerance_cnv_) + (maxCnv > this->param_.tolerance_cnv_relaxed_);

        splitPV[ix] += static_cast<double>(pvValue);
        ++cellCntPV[ix];
    }

    OPM_END_PARALLEL_TRY_CATCH("BlackoilModel::characteriseCnvPvSplit() failed: ", this->grid_.comm());
...
template <class TypeTag>
void
BlackoilModel<TypeTag>::updateTUNING(const Tuning& tuning)
{
...
template <class TypeTag>
ConvergenceReport
BlackoilModel<TypeTag>::getReservoirConvergence(const double reportTime,
                                                const double dt,
                                                const int iteration,
                                                const int maxIter,
                                                std::vector<Scalar>& B_avg,
                                                std::vector<Scalar>& residual_norms)
{
    ...
    const auto [pvSumLocal, numAquiferPvSumLocal] = this->localConvergenceData(R_sum, maxCoeff, B_avg, maxCoeffCell);

    // compute global sum and max of quantities
    const auto& [pvSum, numAquiferPvSum]
        = this->convergenceReduction(this->grid_.comm(), pvSumLocal, numAquiferPvSumLocal, R_sum, maxCoeff, B_avg);

    report.setCnvPoreVolSplit(this->characteriseCnvPvSplit(B_avg, dt), pvSum - numAquiferPvSum);
    ...
    const bool relax_final_iteration_mb = this->param_.min_strict_mb_iter_ < 0 && iteration == maxIter;

    const bool use_relaxed_mb = relax_final_iteration_mb
        || (this->param_.min_strict_mb_iter_ >= 0 && iteration >= this->param_.min_strict_mb_iter_);
    ...
    const bool relax_final_iteration_cnv = this->param_.min_strict_cnv_iter_ < 0 && iteration == maxIter;

    const bool relax_iter_cnv
        = this->param_.min_strict_cnv_iter_ >= 0 && iteration >= this->param_.min_strict_cnv_iter_;
    ...
    const auto relax_pv_fraction_cnv = [&report, this, eligible = pvSum - numAquiferPvSum]() {
        const auto& cnvPvSplit = report.cnvPvSplit().first;

        // [1]: tol < cnv <= relaxed
        // [2]: relaxed < cnv
        return static_cast<Scalar>(cnvPvSplit[1] + cnvPvSplit[2]) < this->param_.relaxed_max_pv_fraction_ * eligible;
    }();

    const bool use_relaxed_cnv = relax_final_iteration_cnv || relax_pv_fraction_cnv || relax_iter_cnv;

    if ((relax_final_iteration_mb || relax_final_iteration_cnv) && this->terminal_output_) {
        std::string message = "Number of newton iterations reached its maximum "
...
template <class TypeTag>
void
BlackoilModel<TypeTag>::convergencePerCell(const std::vector<Scalar>& B_avg,
                                           const double dt,
                                           ...
                                           const int iteration)
{
    ...
    OPM_END_PARALLEL_TRY_CATCH("BlackoilModel::convergencePerCell() failed: ", this->grid_.comm());
    rst_conv.updateNewton(convNewt);
}

template <class TypeTag>
ConvergenceReport
BlackoilModel<TypeTag>::getConvergence(const SimulatorTimerInterface& timer,
                                       const int iteration,
                                       const int maxIter,
                                       std::vector<Scalar>& residual_norms)
{
    ...
    auto report = getReservoirConvergence(
        timer.simulationTimeElapsed(), timer.currentStepLength(), iteration, maxIter, B_avg, residual_norms);
    {
...
BlackoilModel<TypeTag>::computeFluidInPlace(const std::vector<int>& /*fipnum*/) const
{
...
BlackoilModel<TypeTag>::localAccumulatedReports() const
{
    if (!hasNlddSolver()) {
...
BlackoilModel<TypeTag>::domainAccumulatedReports() const
{
    if (!nlddSolver_)
...
template <class TypeTag>
void
BlackoilModel<TypeTag>::writeNonlinearIterationsPerCell(const std::filesystem::path& odir) const
{
    if (hasNlddSolver()) {
...
template <class TypeTag>
void
BlackoilModel<TypeTag>::writePartitions(const std::filesystem::path& odir) const
{
    if (hasNlddSolver()) {
...
void
BlackoilModel<TypeTag>::getMaxCoeff(const unsigned cell_idx,
                                    ...
                                    std::vector<int>& maxCoeffCell)
{
    ...
    if constexpr (has_energy_) {
        B_avg[contiEnergyEqIdx]
            += 1.0 / (4.182e1); // converting J -> RM3 (entalpy / (cp * deltaK * rho) assuming change of 1e-5K of water
    ...
    if constexpr (has_bioeffects_) {
        ...
        maxCoeff[contiBiofilmEqIdx] = std::max(maxCoeff[contiBiofilmEqIdx], std::abs(R2) / pvValue);
        if constexpr (has_micp_) {
            ...
            maxCoeff[contiOxygenEqIdx] = std::max(maxCoeff[contiOxygenEqIdx], std::abs(R3) / pvValue);
            ...
            maxCoeff[contiUreaEqIdx] = std::max(maxCoeff[contiUreaEqIdx], std::abs(R4) / pvValue);
