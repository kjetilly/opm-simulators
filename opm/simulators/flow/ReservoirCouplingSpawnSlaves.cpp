...
ReservoirCouplingSpawnSlaves::ReservoirCouplingSpawnSlaves(ReservoirCouplingMaster& master,
...
void
ReservoirCouplingSpawnSlaves::spawn()
{
...
void
ReservoirCouplingSpawnSlaves::createMasterGroupNameOrder_()
{
    ...
    for (unsigned int i = 0; i < num_slaves; i++) {
        ...
        std::sort(slave_group_names.begin(), slave_group_names.end(), [](const auto& lhs, const auto& rhs) {
...
void
ReservoirCouplingSpawnSlaves::createMasterGroupToSlaveNameMap_()
{
    // When the slaves send master/slave group potentials to us, we need to know
...
std::vector<char*>
ReservoirCouplingSpawnSlaves::getSlaveArgv_(const std::filesystem::path& data_file,
...
ReservoirCouplingSpawnSlaves::getMasterGroupNamesForSlave_(const std::string& slave_name) const
{
...
void
ReservoirCouplingSpawnSlaves::prepareTimeStepping_()
{
...
ReservoirCouplingSpawnSlaves::receiveActivationDateFromSlaves_()
{
    ...
    if (this->comm_.rank() == 0) {
        for (unsigned int i = 0; i < num_slaves; i++) {
            ...
            MPI_Recv(&slave_activation_date,
                     /*count=*/1,
                     /*datatype=*/MPI_DOUBLE,
                     /*source_rank=*/0,
            ...
            if (slave_activation_date < this->master_.getActivationDate()) {
                OPM_THROW(std::runtime_error,
                          "Slave process activation date is earlier than "
                          "the master process' activation date");
            }
            this->master_.addSlaveActivationDate(slave_activation_date);
            this->logger_.info(fmt::format("Received activation date from slave process with name: {}. "
                                           "Activation date: {}",
...
void
ReservoirCouplingSpawnSlaves::receiveSimulationStartDateFromSlaves_()
{
    auto num_slaves = this->master_.numSlavesStarted();
    if (this->comm_.rank() == 0) {
        for (unsigned int i = 0; i < num_slaves; i++) {
            ...
            this->logger_.info(fmt::format("Received start date from slave process with name: {}. "
                                           "Start date: {}",
                                           this->master_.getSlaveName(i),
                                           slave_start_date));
        }
...
void
ReservoirCouplingSpawnSlaves::sendMasterGroupNamesToSlaves_()
{
    if (this->comm_.rank() == 0) {
        auto num_slaves = this->master_.numSlavesStarted();
        for (unsigned int i = 0; i < num_slaves; i++) {
            ...
            MPI_Send(group_names.data(),
                     ...
                     this->master_.getSlaveComm(i));
            this->logger_.info(
                fmt::format("Sent master group names to slave process rank 0 with name: {}", slave_name));
        }
    }
}

void
ReservoirCouplingSpawnSlaves::sendSlaveNamesToSlaves_()
{
    if (this->comm_.rank() == 0) {
        auto num_slaves = this->master_.numSlavesStarted();
        for (unsigned int i = 0; i < num_slaves; i++) {
            ...
            auto MPI_SIZE_T_TYPE = Dune::MPITraits<std::size_t>::getType();
            // NOTE: See comment about error handling at the top of this file.
            MPI_Send(&slave_name_size,
                     /*count=*/1,
                     /*datatype=*/MPI_SIZE_T_TYPE,
                     /*dest_rank=*/0,
                     /*tag=*/static_cast<int>(MessageTag::SlaveNameSize),
                     this->master_.getSlaveComm(i));
            MPI_Send(slave_name.c_str(),
                     /*count=*/slave_name_size,
                     /*datatype=*/MPI_CHAR,
                     /*dest_rank=*/0,
                     /*tag=*/static_cast<int>(MessageTag::SlaveName),
                     this->master_.getSlaveComm(i));
            this->logger_.info(fmt::format("Sent slave name to slave process rank 0 with name: {}", slave_name));
        }
    }
}
...
//   the slave processes
void
ReservoirCouplingSpawnSlaves::spawnSlaveProcesses_()
{
    ...
    for (const auto& [slave_name, slave] : this->rescoup_.slaves()) {
        ...
        std::vector<char*> slave_argv = this->getSlaveArgv_(full_path, slave_name, log_filename);
        ...
        int spawn_result = MPI_Comm_spawn(flow_program_name,
                                          slave_argv.data(),
                                          /*maxprocs=*/num_procs,
                                          /*info=*/MPI_INFO_NULL,
        ...
        this->logger_.info(fmt::format("Spawned reservoir coupling slave simulation for slave with name: "
                                       "{}. Standard output logfile name: {}.log",
                                       slave_name,
                                       slave_name));
