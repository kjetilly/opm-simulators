...
void
logUniqueFailedCells(const std::string& messageTag,
                     ...
                     IJKString&& ijkString)
{
    ...
    const auto numFailed = static_cast<std::size_t>(std::distance(sorted.begin(), u));

    std::ostringstream errlog;
    errlog << prefix << " failed for " << numFailed << " cell" << ((numFailed != std::size_t {1}) ? "s" : "") << " ["
           << ijkString(cells[0]);
...
std::string
injectorCModeToString(const Opm::WellInjectorCMode cmode)
{
...
std::string
producerCModeToString(const Opm::WellProducerCMode cmode)
{
    auto cmodeStr = std::string {};

    // NONE && CMODE_UNDEFINED => ""
    if ((cmode != Opm::WellProducerCMode::NONE) && (cmode != Opm::WellProducerCMode::CMODE_UNDEFINED)) {
...
std::string
formatBorder(const Array& widths)
{
    std::string ss;
    ss = " ";
    std::for_each(widths.begin(), widths.end(), [&ss](const auto w) { ss += fmt::format(":{:->{}}", "", w); });
    ss += ':';
...
std::string
formatTextRow(const std::array<int, size>& widths, const std::array<std::string_view, size>& entries)
{
    std::string ss = " ";
    std::for_each(widths.begin(), widths.end(), [&entries, &ss, i = 0](const auto w) mutable {
...
double
conn_value_or_zero(const Opm::SummaryState& st,
                   const std::string& wname,
                   const std::string& name,
                   const std::size_t gindex)
{
    return st.has_conn_var(wname, name, gindex) ? st.get_conn_var(wname, name, gindex) : 0.0;
}
...
template <class Scalar>
void
LogOutputHelper<Scalar>::cumulative(const std::size_t reportStepNum, const bool withConns) const
{
        ...
        if (well.isInjector()) {
            ...
            if (ctlMode != Well::InjectorCMode::RATE) {
                ...
                if (injType == InjectorType::OIL) {
                    names[Ix::WellCTRL] = "ORAT";
                } else if (injType == InjectorType::WATER) {
                    names[Ix::WellCTRL] = "WRAT";
                } else if (injType == InjectorType::GAS) {
                    names[Ix::WellCTRL] = "GRAT";
                }
            }
...
template <class Scalar>
void
LogOutputHelper<Scalar>::error(const std::vector<int>& failedCellsPbub, const std::vector<int>& failedCellsPdew) const
{
...
template <class Scalar>
void
LogOutputHelper<Scalar>::fip(const Inplace& inplace, const Inplace& initialInplace, const std::string& name) const
{
    auto iget = [&name](const Inplace& ip, Inplace::Phase phase, std::size_t idx) {
        if (name.empty()) {
            return ip.get(phase);
        }
    ...
    for (std::size_t reg = 1; reg <= (name.empty() ? 1 : inplace.max_region(name)); ++reg) {
        ...
        current_values[Inplace::Phase::DynamicPoreVolume] = iget(inplace, Inplace::Phase::DynamicPoreVolume, reg);
...
template <class Scalar>
void
LogOutputHelper<Scalar>::fipResv(const Inplace& inplace, const std::string& name) const
{
    {
        std::unordered_map<Inplace::Phase, Scalar> current_values;
...
template <class Scalar>
void
LogOutputHelper<Scalar>::csv_header(std::ostringstream& ss) const
{
    ss << "RegName,RegNum"
...
template <class Scalar>
void
LogOutputHelper<Scalar>::fip_csv(std::ostringstream& ss, const Inplace& initial_inplace, const std::string& name) const
{
...
template <class Scalar>
void
LogOutputHelper<Scalar>::timeStamp(const std::string& lbl,
                                   double elapsed,
                                   int rstep,
...
template <class Scalar>
void
LogOutputHelper<Scalar>::injection(const std::size_t reportStepNum,
                                   const std::map<std::pair<std::string, int>, double>& block_pressures) const
{
    ...
    for (const auto& wname : this->schedule_[reportStepNum].well_order()) {
        ...
        if (injType == InjectorType::OIL) {
            names[Ix::CTRLModeOil] = isRate ? "ORAT" : injectorCModeToString(ctlMode);
        } else if (injType == InjectorType::WATER) {
            names[Ix::CTRLModeWat] = isRate ? "WRAT" : injectorCModeToString(ctlMode);
        } else if (injType == InjectorType::GAS) {
            names[Ix::CTRLModeGas] = isRate ? "GRAT" : injectorCModeToString(ctlMode);
        }
        ...
        if (!block_pressures.empty()) {
            ...
            for (const auto& connection : well.getConnections()) {
                ...
                const auto bpr = bpr_it == block_pressures.end()
                    ? 0.0
...
template <class Scalar>
void
LogOutputHelper<Scalar>::msw(const std::size_t reportStepNum) const
{
    ...
    std::for_each(msWells.begin(), msWells.end(), [this](const Well& well) { this->outputMSWReportRecord_(well); });
...
template <class Scalar>
void
LogOutputHelper<Scalar>::production(const std::size_t reportStepNum,
                                    const std::map<std::pair<std::string, int>, double>& block_pressures) const
{
...
template <class Scalar>
void
LogOutputHelper<Scalar>::wellSpecification(const std::vector<std::string>& changedWells,
                                           const std::size_t reportStepNum) const
{
    std::ostringstream ss;

    auto blockDepth = PrtFile::Reports::BlockDepthCallback {
        [&grid = this->eclState_.getInputGrid()](const std::size_t globalCellIndex) {
            return grid.getCellDepth(globalCellIndex);
        }};

    PrtFile::Reports::wellSpecification(changedWells, reportStepNum, this->schedule_, blockDepth, ss);
...
void
LogOutputHelper<Scalar>::beginCumulativeReport_() const
{
    const auto& units = this->eclState_.getUnits();
    ...
    auto oil_unit = [utype = units.getType()]() {
        switch (utype) {
    ...
    auto wat_unit = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "MSCM"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    auto gas_unit = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "MMSCM"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    auto res_unit = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "MRCM"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    ss << fmt::format("\n{:=^132}\n", " CUMULATIVE PRODUCTION/INJECTION TOTALS ")
       ...
       << formatTextRow(widths, unitStrings) << fmt::format("{:=>132}", "");
...
template <typename Scalar>
void
LogOutputHelper<Scalar>::endCumulativeReport_() const
{
    const auto widths = std::array {8, 11, 8, 4, 11, 11, 11, 11, 11, 11, 11, 11};
...
template <class Scalar>
void
LogOutputHelper<Scalar>::outputCumulativeReportRecord_(const std::vector<Scalar>& wellCum,
                                                       ...
                                                       const std::vector<ConnData>& connData) const
{
    ...
    if (wellCum[WellCumDataType::WellLocationi] < 1) {
        ...
        ss << fmt::format(
            "{:>3},{:>3} {:>3}:", wellCum[WellCumDataType::WellLocationi], wellCum[WellCumDataType::WellLocationj], "");
    }
    ...
    for (const auto& conn : connData) {
        ss << fmt::format("\n:  BLOCK :{0:>3},{1:>3},{2:>3}:{3:>8}:{3:>4}:", conn.I, conn.J, conn.K, "")
...
template <typename Scalar>
void
LogOutputHelper<Scalar>::beginInjectionReport_() const
{
    const auto& units = this->eclState_.getUnits();
    ...
    auto oil_rate = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "SCM/DAY"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    auto wat_rate = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "SCM/DAY"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    auto gas_rate = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "SCM/DAY"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    auto res_vol = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "RCM/DAY"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    ss << fmt::format("\n{:=^109}\n", " INJECTION REPORT ")
       ...
       << formatTextRow(widths, unitStrings) << fmt::format("{:=>109}", "");
...
template <typename Scalar>
void
LogOutputHelper<Scalar>::endInjectionReport_() const
{
    const auto widths = std::array {8, 11, 6, 6, 6, 11, 11, 11, 11, 8, 8};
    OpmLog::note(formatBorder(widths));
}
...
template <class Scalar>
void
LogOutputHelper<Scalar>::outputInjectionReportRecord_(const std::vector<Scalar>& wellInj,
                                                      const std::vector<std::string>& wellInjNames,
                                                      const std::vector<ConnData>& connData) const
{
    const auto isWellRecord = wellInj[WellProdDataType::WellLocationi] >= 1;
    ...
    if (!isWellRecord) {
        ...
        ss << fmt::format(
            "{:>3},{:>3} {:>3}:", wellInj[WellInjDataType::WellLocationi], wellInj[WellInjDataType::WellLocationj], "");
    }
    ...
    if (!isWellRecord) {
        ...
        ss << fmt::format("{:>8.1f}:{:>8.1f}:", wellInj[WellInjDataType::BHP], wellInj[WellInjDataType::THP]);
    }
...
template <typename Scalar>
void
LogOutputHelper<Scalar>::beginMSWReport_() const
{
    const auto& units = this->eclState_.getUnits();
...
template <typename Scalar>
void
LogOutputHelper<Scalar>::endMSWReport_() const
{
    const auto widths = std::array {10, 5, 5, 11, 11, 11, 9, 16, 9, 8, 8, 8};
    OpmLog::note(formatBorder(widths));
}
...
template <class Scalar>
void
LogOutputHelper<Scalar>::outputMSWReportRecord_(const Well& well) const
{
    const auto& st = this->summaryState_;

    using namespace std::string_view_literals;

    auto clampToZero = [](const auto& number) { return std::fabs(number) > 1e-6 ? number : 0.0; };

    auto get_phase_values = [&st, &clampToZero, &wname = well.name()](const int segmentNumber,
                                                                      const std::array<std::string_view, 3>& fields) {
        std::array<Scalar, 3> result;
        for (std::size_t p = 0; p < 3; ++p) {
            const auto value = st.has_segment_var(wname, std::string {fields[p]}, segmentNumber)
    ...
    for (int i = 1; i <= well.maxBranchID(); ++i) {
        ...
        for (const auto& segment : segments) {
            ...
            const auto rates = get_phase_values(segment.segmentNumber(), std::array {"SOFR"sv, "SWFR"sv, "SGFR"sv});
            const auto holdups = get_phase_values(segment.segmentNumber(), std::array {"SOHF"sv, "SWHF"sv, "SGHF"sv});
            const auto velocities
                = get_phase_values(segment.segmentNumber(), std::array {"SOFV"sv, "SWFV"sv, "SGFV"sv});
            const auto press_drop
                = get_phase_values(segment.segmentNumber(), std::array {"SPRDH"sv, "SPRDF"sv, "SPRDA"sv});

            const auto mixture_vel = std::inner_product(holdups.begin(), holdups.end(), velocities.begin(), 0.0);
            ...
            ss << fmt::format("{:^5}:{:>11.1f}:{:>11.1f}:{:>11.1f}:{:>9.3f}:"
                              ...
                              holdups[1],
                              holdups[2],
                              clampToZero(st.has_segment_var(well.name(), "SPR", segment.segmentNumber())
                                              ? st.get_segment_var(well.name(), "SPR", segment.segmentNumber())
                                              : 0.0),
                              press_drop[0],
                              press_drop[1],
                              press_drop[2]);
...
template <typename Scalar>
void
LogOutputHelper<Scalar>::beginProductionReport_() const
{
    const auto& units = this->eclState_.getUnits();
    ...
    auto water_cut = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "SCM/SCM"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    auto gas_oil_rate = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "SCM/SCM"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    auto wat_gas_rate = [utype = units.getType()]() {
        switch (utype) {
        case UnitSystem::UnitType::UNIT_TYPE_METRIC:
            return "SCM/SCM"sv;
        case UnitSystem::UnitType::UNIT_TYPE_FIELD:
    ...
    ss << fmt::format("\n{:=^129}\n", " PRODUCTION REPORT ")
       ...
       << formatTextRow(widths, unitStrings) << fmt::format("{:=>129}", "");
...
template <typename Scalar>
void
LogOutputHelper<Scalar>::endProductionReport_() const
{
    const auto widths = std::array {8, 11, 4, 11, 11, 11, 11, 11, 10, 12, 8, 8};
    OpmLog::note(formatBorder(widths));
}
...
template <class Scalar>
void
LogOutputHelper<Scalar>::outputProductionReportRecord_(const std::vector<Scalar>& wellProd,
                                                       const std::vector<std::string>& wellProdNames,
                                                       const std::vector<ConnData>& connData) const
{
    const auto isWellRecord = wellProd[WellProdDataType::WellLocationi] >= 1;
    ...
    if (!isWellRecord) {
        ...
        ss << fmt::format("{:>8.1f}:{:>8.1f}:", wellProd[WellProdDataType::BHP], wellProd[WellProdDataType::THP]);
    }

    for (const auto& conn : connData) {
        ss << fmt::format("\n:  BLOCK :{0:>3},{1:>3},{2:>3}:{3:>4}:", conn.I, conn.J, conn.K, "")
           << fmt::format("{:>11.1f}:{:>11.1f}:{:>11.1f}:{:>11.1f}:"
...
template <class Scalar>
void
LogOutputHelper<Scalar>::outputRegionFluidInPlace_(std::unordered_map<Inplace::Phase, Scalar> oip,
                                                   ...
                                                   const int reg) const
{
    ...
    auto topEntry = [&units](const char* text, const Scalar value, const UnitSystem::measure m) {
        return fmt::format(" {0: >52}:{0: >8}{1:<5}={2:>14.0f}  {3:<18}:\n", "", text, value, units.name(m));
    };

    auto topLabel = [](const char* text) { return fmt::format(" {: >52}: {:^47}:\n", "", text); };
    ...
    ss << fmt::format(" {0: >26}:{0:->15} OIL {1:>4} {0:->18}:"
       ...
       << fmt::format(" {0: >26}:{1:^15}{2:^14}{3:^14}:{3:^16}:{4:^15}{5:^14}{3:^14}:\n",
                      ...
                      "DISSOLVED")
       << formatBorder(widths) << '\n'
       << formatEntry(cip, "CURRENTLY IN PLACE") << formatBorder(widths) << '\n'
...
template <class Scalar>
void
LogOutputHelper<Scalar>::outputResvFluidInPlace_(std::unordered_map<Inplace::Phase, Scalar> cipr, const int reg) const
{
    const UnitSystem& units = eclState_.getUnits();
    using namespace std::string_view_literals;
...
template <class Scalar>
void
LogOutputHelper<Scalar>::fipUnitConvert_(std::unordered_map<Inplace::Phase, Scalar>& fip) const
{
    const UnitSystem& units = eclState_.getUnits();
...
template <class Scalar>
void
LogOutputHelper<Scalar>::pressureUnitConvert_(Scalar& pav) const
{
    pav = eclState_.getUnits().from_si(UnitSystem::measure::pressure, pav);
}
