...
public:
    ...
    using matrix_type_to= typename Dune::BCRSMatrix<Dune::FieldMatrix<scalar_field_type_to, block_type::dimension, block_type::dimension>> ;
    ...
    PreconditionerConvertToFloatAdapter(const M& matrix)
        : matrix(matrix), convertedMatrix(createConvertedMatrix())
    {
        ...
        for(auto row = matrix.begin(); row!=matrix.end(); ++row)
        {
            for (auto column = row->begin(); column != row->end(); ++column) {
                for (size_t i = 0; i < block_type::dimension; ++i) {
                    for (size_t j = 0; j < block_type::dimension; ++j) {
                        matrixBuilder[row.index()][column.index()][i][j] = scalar_field_type_to(matrix[row.index()][column.index()][i][j]);
                    }
                }
            }
        }
