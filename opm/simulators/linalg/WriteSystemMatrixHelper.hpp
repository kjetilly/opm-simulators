...
namespace detail
{
                                 ...
                                 [[maybe_unused]] const Communicator* comm)
    {
        ...
        oss << "prob_" << simulator.episodeIndex() << "_time_" << std::setprecision(15) << std::setw(12)
            << std::setfill('0') << simulator.time() << "_nit_" << simulator.model().newtonMethod().numIterations()
            << '_' << objName << "_istl";
...
void
writeVector(const SimulatorType& simulator, const VectorType& rhs, const std::string& sysName, const Communicator* comm)
{
...
void
writeMatrix(const SimulatorType& simulator,
...
void
writeSystem(const SimulatorType& simulator,
            const MatrixType& matrix,
...
template <class SimulatorType, class MatrixType, class VectorType, class Communicator>
void
writeSystem(const SimulatorType& simulator, const MatrixType& matrix, const VectorType& rhs, const Communicator* comm)
{
...
template <class SimulatorType, class MatrixType, class VectorType, class Communicator>
void
writeMechSystem(const SimulatorType& simulator,
                const MatrixType& matrix,
                const VectorType& rhs,
                const Communicator* comm)
{
