...
template <class Scalar, unsigned int block_size>
bool
openclBILU0<Scalar, block_size>::analyze_matrix(BlockedMatrix<Scalar>* mat)
{
...
template <class Scalar, unsigned int block_size>
bool
openclBILU0<Scalar, block_size>::analyze_matrix(BlockedMatrix<Scalar>* mat, BlockedMatrix<Scalar>* jacMat)
{
    ...
    if (opencl_ilu_parallel) {
        ...
        Timer t_convert;
        csrPatternToCsc(
            matToDecompose->colIndices, matToDecompose->rowPointers, CSCRowIndices.data(), CSCColPointers.data(), Nb);
        if (verbosity >= 3) {
    ...
    if (opencl_ilu_parallel) {
        ...
        findLevelScheduling(matToDecompose->colIndices,
                            matToDecompose->rowPointers,
                            CSCRowIndices.data(),
                            CSCColPointers.data(),
                            Nb,
                            &numColors,
    ...
    err = queue->enqueueWriteBuffer(
        s.invDiagVals, CL_FALSE, 0, mat->Nb * sizeof(Scalar) * bs * bs, invDiagVals.data(), nullptr, &events[0]);
    ...
    err |= queue->enqueueWriteBuffer(
        s.rowsPerColor, CL_FALSE, 0, (numColors + 1) * sizeof(int), rowsPerColorPrefix.data(), nullptr, &events[1]);

    if (opencl_ilu_parallel) {
        err |= queue->enqueueWriteBuffer(
            s.rowIndices, CL_FALSE, 0, Nb * sizeof(unsigned), fromOrder.data(), nullptr, &events[2]);
    } else {
        ...
        err |= queue->enqueueWriteBuffer(
            s.rowIndices, CL_FALSE, 0, Nb * sizeof(unsigned), rowsPerColorPrefix.data(), nullptr, &events[2]);
    }
...
template <class Scalar, unsigned int block_size>
bool
openclBILU0<Scalar, block_size>::create_preconditioner(BlockedMatrix<Scalar>* mat)
{
...
template <class Scalar, unsigned int block_size>
bool
openclBILU0<Scalar, block_size>::create_preconditioner(BlockedMatrix<Scalar>* mat, BlockedMatrix<Scalar>* jacMat)
{
    const unsigned int bs = block_size;
...
#if CHOW_PATEL
    chowPatelIlu.decomposition(queue.get(),
                               context.get(),
                               LUmat.get(),
                               Lmat.get(),
                               Umat.get(),
    ...
    queue->enqueueWriteBuffer(
        s.LUvals, CL_FALSE, 0, LUmat->nnzbs * bs * bs * sizeof(Scalar), LUmat->nnzValues, nullptr, &events[0]);
    ...
    std::call_once(pattern_uploaded, [&]() {
        ...
        queue->enqueueWriteBuffer(
            s.LUcols, CL_FALSE, 0, LUmat->nnzbs * sizeof(int), LUmat->colIndices, nullptr, &events[2]);
        queue->enqueueWriteBuffer(
            s.LUrows, CL_FALSE, 0, (LUmat->Nb + 1) * sizeof(int), LUmat->rowPointers, nullptr, &events[3]);
    });
    ...
    for (int color = 0; color < numColors; ++color) {
        ...
        OpenclKernels<Scalar>::ILU_decomp(firstRow,
                                          lastRow,
                                          s.rowIndices,
                                          s.LUvals,
                                          s.LUcols,
...
template <class Scalar, unsigned int block_size>
void
openclBILU0<Scalar, block_size>::apply(const cl::Buffer& y,
                                       cl::Buffer& x,
                                       [[maybe_unused]] WellContributions<Scalar>& wellContribs)
{
    ...
    for (int color = 0; color < numColors; ++color) {
#if CHOW_PATEL
        OpenclKernels<Scalar>::ILU_apply1(s.rowIndices,
                                          s.Lvals,
                                          s.Lcols,
                                          s.Lrows,
                                          s.diagIndex,
...
#if CHOW_PATEL
        OpenclKernels<Scalar>::ILU_apply2(s.rowIndices,
                                          s.Uvals,
                                          s.Ucols,
                                          s.Urows,
                                          s.diagIndex,
