...
rocsparseBILU0<Scalar, block_size>::rocsparseBILU0(int verbosity_)
    : Base(verbosity_)
{
}

template <class Scalar, unsigned int block_size>
rocsparseBILU0<Scalar, block_size>::~rocsparseBILU0()
{
...
template <class Scalar, unsigned int block_size>
bool
rocsparseBILU0<Scalar, block_size>::initialize(std::shared_ptr<BlockedMatrix<Scalar>> matrix,
...
template <class Scalar, unsigned int block_size>
bool
rocsparseBILU0<Scalar, block_size>::analyze_matrix()
{
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrilu0_buffer_size(this->handle,
                                                       this->dir,
                                                       Nb,
                                                       this->nnzbs_prec,
                                                       descr_M,
        ...
        ROCSPARSE_CHECK(rocsparse_sbsrsv_buffer_size(this->handle,
                                                     this->dir,
                                                     this->operation,
                                                     Nb,
                                                     this->nnzbs_prec,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrilu0_buffer_size(this->handle,
                                                       this->dir,
                                                       Nb,
                                                       this->nnzbs_prec,
                                                       descr_M,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrsv_buffer_size(this->handle,
                                                     this->dir,
                                                     this->operation,
                                                     Nb,
                                                     this->nnzbs_prec,
    ...
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrilu0_analysis(this->handle,
                                                    this->dir,
                                                    Nb,
                                                    this->nnzbs_prec,
                                                    descr_M,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrilu0_analysis(this->handle,
                                                    this->dir,
                                                    Nb,
                                                    this->nnzbs_prec,
                                                    descr_M,
    ...
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrsv_analysis(this->handle,
                                                  this->dir,
                                                  this->operation,
                                                  Nb,
                                                  this->nnzbs_prec,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrsv_analysis(this->handle,
                                                  this->dir,
                                                  this->operation,
                                                  Nb,
                                                  this->nnzbs_prec,
...
template <class Scalar, unsigned int block_size>
bool
rocsparseBILU0<Scalar, block_size>::analyze_matrix(BlockedMatrix<Scalar>* mat)
{
...
template <class Scalar, unsigned int block_size>
bool
rocsparseBILU0<Scalar, block_size>::analyze_matrix(BlockedMatrix<Scalar>*, BlockedMatrix<Scalar>*)
{
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrilu0_buffer_size(this->handle,
                                                       this->dir,
...
template <class Scalar, unsigned int block_size>
bool
rocsparseBILU0<Scalar, block_size>::create_preconditioner(BlockedMatrix<Scalar>* mat)
{
...
template <class Scalar, unsigned int block_size>
bool
rocsparseBILU0<Scalar, block_size>::create_preconditioner(BlockedMatrix<Scalar>*, BlockedMatrix<Scalar>*)
{
    bool result = true;

    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrilu0(this->handle,
                                           this->dir,
                                           Nb,
                                           this->nnzbs_prec,
                                           descr_M,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrilu0(this->handle,
                                           this->dir,
                                           Nb,
                                           this->nnzbs_prec,
                                           descr_M,
...
template <class Scalar, unsigned int block_size>
void
rocsparseBILU0<Scalar, block_size>::copy_system_to_gpu(Scalar* d_Avals)
{
    ...
    if (this->useJacMatrix) {
        ...
        HIP_CHECK(hipMemcpyAsync(
            d_Mrows, this->jacMat->rowPointers, sizeof(rocsparse_int) * (Nb + 1), hipMemcpyHostToDevice, this->stream));
        HIP_CHECK(hipMemcpyAsync(d_Mcols,
                                 this->jacMat->colIndices,
                                 sizeof(rocsparse_int) * this->nnzbs_prec,
                                 hipMemcpyHostToDevice,
                                 this->stream));
        HIP_CHECK(hipMemcpyAsync(d_Mvals,
                                 this->jacMat->nnzValues,
                                 sizeof(Scalar) * this->nnzbs_prec * block_size * block_size,
                                 hipMemcpyHostToDevice,
                                 this->stream));
    } else {
...
template <class Scalar, unsigned int block_size>
void
rocsparseBILU0<Scalar, block_size>::copy_values_to_gpu(Scalar* h_vals, int* h_rows, int* h_cols, bool reuse)
{
    if (!reuse) {
        HIP_CHECK(
            hipMemcpyAsync(d_Mrows, h_rows, sizeof(rocsparse_int) * (Nb + 1), hipMemcpyHostToDevice, this->stream));
        HIP_CHECK(hipMemcpyAsync(
            d_Mcols, h_cols, sizeof(rocsparse_int) * this->nnzbs_prec, hipMemcpyHostToDevice, this->stream));
    }
...
template <class Scalar, unsigned int block_size>
void
rocsparseBILU0<Scalar, block_size>::update_system_on_gpu(Scalar*, Scalar* d_Avals)
{
    bool use_multithreading = true;
...
template <class Scalar, unsigned int block_size>
void
rocsparseBILU0<Scalar, block_size>::apply(const Scalar& y,
                                          Scalar& x,
                                          [[maybe_unused]] WellContributions<Scalar>& wellContribs)
{
    ...
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrsv_solve(this->handle,
                                               this->dir,
                                               this->operation,
                                               Nb,
                                               this->nnzbs_prec,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrsv_solve(this->handle,
                                               this->dir,
                                               this->operation,
                                               Nb,
                                               this->nnzbs_prec,
