...
template <class Scalar, unsigned int block_size>
bool
rocsparseCPR<Scalar, block_size>::initialize(std::shared_ptr<BlockedMatrix<Scalar>> matrix,
...
template <class Scalar, unsigned int block_size>
void
rocsparseCPR<Scalar, block_size>::copy_system_to_gpu(Scalar* b)
{
...
template <class Scalar, unsigned int block_size>
void
rocsparseCPR<Scalar, block_size>::update_system_on_gpu(Scalar* h_vals, Scalar* vals)
{
...
template <class Scalar, unsigned int block_size>
bool
rocsparseCPR<Scalar, block_size>::analyze_matrix(BlockedMatrix<Scalar>* mat_)
{
    this->Nb = mat_->Nb;
...
template <class Scalar, unsigned int block_size>
bool
rocsparseCPR<Scalar, block_size>::analyze_matrix(BlockedMatrix<Scalar>* mat_, BlockedMatrix<Scalar>* jacMat_)
{
    this->Nb = mat_->Nb;
    this->nnzb = mat_->nnzbs;
    this->N = Nb * block_size;
...
template <class Scalar, unsigned int block_size>
bool
rocsparseCPR<Scalar, block_size>::create_preconditioner([[maybe_unused]] BlockedMatrix<Scalar>* mat_,
...
template <class Scalar, unsigned int block_size>
bool
rocsparseCPR<Scalar, block_size>::create_preconditioner(BlockedMatrix<Scalar>* mat_)
{
    ...
    for (int i = 0; i < this->num_levels - 1; i++) {
        ...
        auto levelMat = std::make_shared<BlockedMatrix<Scalar>>(this->Amatrices[i].N,
                                                                this->Amatrices[i].nnzs,
                                                                1,
                                                                &(this->Amatrices[i].nnzValues[0]),
                                                                &(this->Amatrices[i].colIndices[0]),
                                                                &(this->Amatrices[i].rowPointers[0]));
        ...
        if (amgcreated) {
            ...
            smoother->copy_values_to_gpu(&(this->Amatrices[i].nnzValues[0]),
                                         &(this->Amatrices[i].rowPointers[0]),
                                         &(this->Amatrices[i].colIndices[0]),
                                         false);
    ...
    if (amgcreated) {
        if (!createfirsttime)
            clear_rocm_buffers();
        init_rocm_buffers();
        HIP_CHECK(hipMemcpyAsync(d_weights.data()->nnzValues,
                                 this->weights.data(),
                                 sizeof(Scalar) * this->N,
...
template <class Scalar, unsigned int block_size>
void
rocsparseCPR<Scalar, block_size>::init_rocm_buffers_fineMatrix()
{
...
template <class Scalar, unsigned int block_size>
void
rocsparseCPR<Scalar, block_size>::init_rocm_buffers()
{
...
template <class Scalar, unsigned int block_size>
void
rocsparseCPR<Scalar, block_size>::clear_rocm_buffers()
{
    d_Amatrices.clear();
    d_Rmatrices.clear();
    d_invDiags.clear();
...
template <class Scalar, unsigned int block_size>
void
rocsparseCPR<Scalar, block_size>::rocm_upload()
{
    ...
    for (unsigned int i = 0; i < this->Rmatrices.size(); ++i) {
        ...
        HIP_CHECK(hipMemcpyAsync(d_invDiags[i].nnzValues,
                                 this->invDiags[i].data(),
                                 sizeof(Scalar) * this->Amatrices[i].N,
                                 hipMemcpyHostToDevice,
                                 this->stream));
        HIP_CHECK(hipMemcpyAsync(d_PcolIndices[i].nnzValues,
...
template <class Scalar, unsigned int block_size>
void
rocsparseCPR<Scalar, block_size>::amg_cycle_gpu(const int level,
                                                Scalar& y,
                                                Scalar& x,
                                                [[maybe_unused]] WellContributions<Scalar>& wellContribs)
{
    ...
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_scsrmv_analysis(this->handle,
                                                  this->operation,
                                                  R->Nb,
                                                  R->Mb,
                                                  R->nnzbs,
        ...
        ROCSPARSE_CHECK(rocsparse_dcsrmv_analysis(this->handle,
                                                  this->operation,
                                                  R->Nb,
                                                  R->Mb,
                                                  R->nnzbs,
    ...
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_scsrmv(this->handle,
                                         this->operation,
                                         R->Nb,
                                         R->Mb,
                                         R->nnzbs,
        ...
        ROCSPARSE_CHECK(rocsparse_dcsrmv(this->handle,
                                         this->operation,
                                         R->Nb,
                                         R->Mb,
                                         R->nnzbs,
    ...
    if (level < this->num_levels - 2) {
        HipKernels<Scalar>::prolongate_vector(
            d_t[level + 1].nnzValues, &x, d_PcolIndices[level].nnzValues, Ncur, this->stream);
    } else {
    ...
    for (unsigned i = 0; i < this->num_post_smooth_steps; ++i) {
        // update defect
        HipKernels<Scalar>::residual(A->nnzValues,
                                     A->colIndices,
                                     A->rowPointers,
                                     &x,
                                     &y,
                                     ...
                                     Ncur,
                                     1,
                                     this->stream); // params: x@in, y@in, y@out = (A.., x, rhs, out, Nb, block_size,
                                                    // stream) -> computes out = rhs - A * x <==> y -= Ax
...
template <class Scalar, unsigned int block_size>
void
rocsparseCPR<Scalar, block_size>::apply_amg(const Scalar& y,
                                            Scalar& x,
                                            [[maybe_unused]] WellContributions<Scalar>& wellContribs)
{
    ...
    HipKernels<Scalar>::full_to_pressure_restriction(
        &y, d_weights.data()->nnzValues, d_coarse_y.data()->nnzValues, Nb, this->stream);

    // NOTE: input b=y is is overwritten, so duplicate into d_yinput
    HIP_CHECK(hipMemcpyAsync(d_yinput.data()->nnzValues,
                             d_coarse_y.data()->nnzValues,
                             sizeof(Scalar) * d_coarse_y.data()->size,
    ...
    // update defect
    HipKernels<Scalar>::residual(
        A->nnzValues,
        A->colIndices,
        A->rowPointers,
        d_t[0].nnzValues,
        d_yinput.data()->nnzValues,
        d_yinput.data()->nnzValues,
        d_yinput.data()->size,
        1,
        this->stream); // params: x@in, y@in, y@out = (A.., x, rhs, out, Nb, block_size, stream)
...
template <class Scalar, unsigned int block_size>
void
rocsparseCPR<Scalar, block_size>::apply(const Scalar& y,
                                        Scalar& x,
                                        [[maybe_unused]] WellContributions<Scalar>& wellContribs)
{
    Scalar one = 1.0;
    ...
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrmv_analysis(this->handle,
                                                  this->dir,
                                                  this->operation,
                                                  Nb,
                                                  Nb,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrmv_analysis(this->handle,
                                                  this->dir,
                                                  this->operation,
                                                  Nb,
                                                  Nb,
    ...
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrmv_ex_analysis(this->handle,
                                                     this->dir,
                                                     this->operation,
                                                     Nb,
                                                     Nb,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrmv_ex_analysis(this->handle,
                                                     this->dir,
                                                     this->operation,
                                                     Nb,
                                                     Nb,
...
#if HIP_VERSION >= 60000000
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrmv(this->handle,
                                         this->dir,
                                         this->operation,
                                         Nb,
                                         Nb,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrmv(this->handle,
                                         this->dir,
                                         this->operation,
                                         Nb,
                                         Nb,
...
#elif HIP_VERSION >= 50400000
    if constexpr (std::is_same_v<Scalar, float>) {
        ROCSPARSE_CHECK(rocsparse_sbsrmv_ex(this->handle,
                                            this->dir,
                                            this->operation,
                                            Nb,
                                            Nb,
        ...
        ROCSPARSE_CHECK(rocsparse_dbsrmv_ex(this->handle,
                                            this->dir,
                                            this->operation,
                                            Nb,
                                            Nb,
