...
template <class Scalar>
__global__ void
vmul_k(const Scalar alpha, Scalar const* in1, Scalar const* in2, Scalar* out, const int N)
{
...
template <class Scalar>
__global__ void
full_to_pressure_restriction_k(const Scalar* fine_y, const Scalar* weights, Scalar* coarse_y, const unsigned int Nb)
{
...
template <class Scalar>
__global__ void
add_coarse_pressure_correction_k(const Scalar* coarse_x,
                                 Scalar* fine_x,
...
template <class Scalar>
__global__ void
prolongate_vector_k(const Scalar* in, Scalar* out, const int* cols, const unsigned int N)
{
    const unsigned int NUM_THREADS = gridDim.x;
...
template <class Scalar>
__global__ void
residual_blocked_k(const Scalar* vals,
                   const int* cols,
                   const int* rows,
                   const int Nb,
                   const Scalar* x,
...
__global__ void
residual_k(
    const Scalar* vals, const int* cols, const int* rows, const int N, const Scalar* x, const Scalar* rhs, Scalar* out)
{
    extern __shared__ Scalar tmp[];
    const unsigned int bsize = blockDim.x;
    const unsigned int gid = blockDim.x * blockIdx.x + threadIdx.x;
...
template <class Scalar>
__global__ void
spmv_k(const Scalar* vals, const int* cols, const int* rows, const int N, const Scalar* x, Scalar* out)
{
    extern __shared__ Scalar tmp[];
    const unsigned int bsize = blockDim.x;
    const unsigned int gid = blockDim.x * blockIdx.x + threadIdx.x;
...
template <class Scalar>
void
HipKernels<Scalar>::init(int verbosity_)
{
    if (initialized) {
...
template <class Scalar>
void
HipKernels<Scalar>::vmul([[maybe_unused]] const Scalar alpha,
...
template <class Scalar>
void
HipKernels<Scalar>::full_to_pressure_restriction([[maybe_unused]] const Scalar* fine_y,
                                                 ...
                                                 [[maybe_unused]] hipStream_t stream)
{
    ...
    OPM_THROW(std::logic_error,
              "Error full_to_pressure_restriction for rocsparse only supported when compiling with hipcc");
#endif
...
template <class Scalar>
void
HipKernels<Scalar>::add_coarse_pressure_correction([[maybe_unused]] Scalar* coarse_x,
...
#else
    OPM_THROW(std::logic_error,
              "Error add_coarse_pressure_correction for rocsparse only supported when compiling with hipcc");
#endif
...
template <class Scalar>
void
HipKernels<Scalar>::prolongate_vector([[maybe_unused]] const Scalar* in,
                                      [[maybe_unused]] Scalar* out,
...
template <class Scalar>
void
HipKernels<Scalar>::residual([[maybe_unused]] Scalar* vals,
                             ...
                             [[maybe_unused]] hipStream_t stream)
{
    ...
    if (block_size > 1) {
        ...
        residual_blocked_k<<<dim3(gridDim), dim3(blockDim), shared_mem_size, stream>>>(
            vals, cols, rows, Nb, x, rhs, out, block_size);
    } else {
...
template <class Scalar>
void
HipKernels<Scalar>::spmv([[maybe_unused]] Scalar* vals,
                         [[maybe_unused]] int* cols,
                         [[maybe_unused]] int* rows,
                         [[maybe_unused]] Scalar* x,
