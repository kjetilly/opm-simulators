...
__global__ void
stdwell_apply(const Scalar* Cnnzs,
              const Scalar* Dnnzs,
              const Scalar* Bnnzs,
...
template <class Scalar>
void
WellContributionsRocsparse<Scalar>::apply_stdwells([[maybe_unused]] Scalar* d_x, [[maybe_unused]] Scalar* d_y)
{
#ifdef __HIP__
    unsigned gridDim = this->num_std_wells;
    unsigned blockDim = 64;
    unsigned shared_mem_size
        = (blockDim + 2 * this->dim_wells) * sizeof(Scalar); // shared memory for localSum, z1 and z2
    ...
    OPM_THROW(std::logic_error,
              "Error separate wellcontributions for rocsparse only supported when compiling with hipcc");
#endif
}

template <class Scalar>
void
WellContributionsRocsparse<Scalar>::apply_mswells(Scalar* d_x, Scalar* d_y)
{
    if (h_x.empty()) {
        h_x.resize(this->N);
        h_y.resize(this->N);
...
template <class Scalar>
void
WellContributionsRocsparse<Scalar>::apply(Scalar* d_x, Scalar* d_y)
{
...
template <class Scalar>
void
WellContributionsRocsparse<Scalar>::setStream(hipStream_t stream_)
{
    stream = stream_;
}
...
template <class Scalar>
void
WellContributionsRocsparse<Scalar>::APIaddMatrix(MatrixType type,
                                                 int* colIndices,
                                                 Scalar* values,
                                                 unsigned int val_size)
{
    ...
    case MatrixType::C:
        HIP_CHECK(hipMemcpyAsync(d_Cnnzs_hip + this->num_blocks_so_far * this->dim * this->dim_wells,
                                 values,
                                 sizeof(d_Cnnzs_hip) * val_size * this->dim * this->dim_wells,
                                 hipMemcpyHostToDevice,
                                 stream));
        HIP_CHECK(hipMemcpyAsync(d_Ccols_hip + this->num_blocks_so_far,
                                 colIndices,
    ...
    case MatrixType::D:
        HIP_CHECK(hipMemcpyAsync(d_Dnnzs_hip + this->num_std_wells_so_far * this->dim_wells * this->dim_wells,
                                 values,
                                 sizeof(d_Dnnzs_hip) * this->dim_wells * this->dim_wells,
                                 hipMemcpyHostToDevice,
                                 stream));
        break;
    ...
    case MatrixType::B:
        HIP_CHECK(hipMemcpyAsync(d_Bnnzs_hip + this->num_blocks_so_far * this->dim * this->dim_wells,
                                 values,
                                 sizeof(d_Bnnzs_hip) * val_size * this->dim * this->dim_wells,
                                 hipMemcpyHostToDevice,
                                 stream));
        HIP_CHECK(hipMemcpyAsync(d_Bcols_hip + this->num_blocks_so_far,
                                 colIndices,
        ...
        if (this->num_std_wells_so_far == this->num_std_wells - 1) {
            ...
            HIP_CHECK(hipMemcpyAsync(d_val_pointers_hip,
...
template <class Scalar>
void
WellContributionsRocsparse<Scalar>::APIalloc()
{
    HIP_CHECK(hipMalloc((void**)&d_Cnnzs_hip, sizeof(d_Cnnzs_hip) * this->num_blocks * this->dim * this->dim_wells));
    HIP_CHECK(
        hipMalloc((void**)&d_Dnnzs_hip, sizeof(d_Dnnzs_hip) * this->num_std_wells * this->dim_wells * this->dim_wells));
