                                                          ...
                                                          [[maybe_unused]] Comm* comm)
    {
        ...
        if (parallel) {
            ...
            if (!wellOperator_) {
                ...
                auto sol = std::make_unique<FlexibleSolverType>(*pop, *comm, prm, weightsCalculator, pressureIndex);
                ...
                auto pop = std::make_unique<ParOperatorType>(matrix, *wellOperator_, interiorCellNum_);
                using FlexibleSolverType = Dune::FlexibleSolver<ParOperatorType>;
                auto sol = std::make_unique<FlexibleSolverType>(*pop, *comm, prm, weightsCalculator, pressureIndex);
                this->pre_ = &sol->preconditioner();
                this->op_ = std::move(pop);
            ...
            if (!wellOperator_) {
                ...
                auto sol = std::make_unique<FlexibleSolverType>(*sop, prm, weightsCalculator, pressureIndex);
                this->pre_ = &sol->preconditioner();
