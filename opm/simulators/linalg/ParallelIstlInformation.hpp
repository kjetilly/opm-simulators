...
public:
    ...
    void copyValuesTo(ParallelIndexSet& indexSet,
    ...
    void computeReduction(const Container& container, BinaryOperator binaryOperator, T& value) const;
...
namespace Reduction
{
    ...
    struct MaskIDOperator {
        ...
        typedef
            typename std::remove_cv<typename std::remove_reference<typename BinaryOperator::result_type>::type>::type
                Result;
    ...
    struct MaskToMinOperator {
        ...
        typedef
            typename std::remove_reference<typename std::remove_const<typename BinaryOperator::result_type>::type>::type
                Result;
...
template <class T1>
auto accumulateMaskedValues(const T1& container, const std::vector<double>* maskContainer)
