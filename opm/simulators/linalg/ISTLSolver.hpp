...
namespace detail
{
    ...
    void
    copyParValues(std::any& parallelInformation, std::size_t size, Dune::OwnerOverlapCopyCommunication<int, int>& comm);
#endif
...
public:
    ...
    void initialize()
    {
        ...
        element_chunks_ = std::make_unique<ElementChunksType>(
            simulator_.vanguard().gridView(), Dune::Partitions::all, ThreadManager::maxThreads());
    }
...
protected:
        ...
        if (shouldCreateSolver()) {
            if (!useWellConn_) {
                if (isNlddLocalSolver()) {
                    auto wellOp = std::make_unique<DomainWellModelAsLinearOperator<WellModel, Vector, Vector>>(
                        simulator_.problem().wellModel());
            ...
            std::function<Vector()> weightCalculator
                = this->getWeightsCalculator(prm_[activeSolverNum_], getMatrix(), pressureIndex);
    ...
    std::function<Vector()>
    getWeightsCalculator(const PropertyTree& prm, const Matrix& matrix, std::size_t pressIndex) const
    {
        ...
        if (preconditionerType == "cpr" || preconditionerType == "cprt" || preconditionerType == "cprw"
            ...
            if (weightsType == "quasiimpes") {
                ...
                weightsCalculator = [matrix, transpose, pressIndex]() {
                    return Amg::getQuasiImpesWeights<Matrix, Vector>(matrix, pressIndex, transpose);
                };
            } else if (weightsType == "trueimpes") {
                weightsCalculator = [this, pressIndex] {
                    Vector weights(rhs_->size());
                    ElementContext elemCtx(simulator_);
                    Amg::getTrueImpesWeights(pressIndex, weights, elemCtx, simulator_.model(), *element_chunks_);
                    return weights;
                };
                ...
                weightsCalculator = [this, pressIndex] {
                    Vector weights(rhs_->size());
                    ElementContext elemCtx(simulator_);
                    Amg::getTrueImpesWeightsAnalytic(
                        pressIndex, weights, elemCtx, simulator_.model(), *element_chunks_);
                    return weights;
                };
            } else {
