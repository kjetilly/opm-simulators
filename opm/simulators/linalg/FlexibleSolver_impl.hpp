...
FlexibleSolver<Operator>::FlexibleSolver(Operator& op,
                                         const Opm::PropertyTree& prm,
                                         const std::function<VectorType()>& weightsCalculator,
                                         std::size_t pressureIndex)
{
    init(op, Dune::Amg::SequentialInformation(), prm, weightsCalculator, pressureIndex);
}
...
template <class Operator>
void
FlexibleSolver<Operator>::apply(VectorType& x, VectorType& rhs, Dune::InverseOperatorResult& res)
{
    if (direct_solver_) {
...
template <class Operator>
void
FlexibleSolver<Operator>::apply(VectorType& x, VectorType& rhs, double reduction, Dune::InverseOperatorResult& res)
{
    if (direct_solver_) {
        recreateDirectSolver();
    }
...
template <class Operator>
auto
FlexibleSolver<Operator>::preconditioner() -> AbstractPrecondType&
{
...
FlexibleSolver<Operator>::category() const
{
...
template <class Comm>
void
FlexibleSolver<Operator>::initOpPrecSp(Operator& op,
                                       const Opm::PropertyTree& prm,
                                       const std::function<VectorType()> weightsCalculator,
                                       const Comm& comm,
                                       std::size_t pressureIndex)
{
    ...
    preconditioner_ = Opm::PreconditionerFactory<Operator, Comm>::create(
        op, child ? *child : Opm::PropertyTree(), weightsCalculator, comm, pressureIndex);
                                       ...
                                       std::size_t pressureIndex)
{
    ...
    linearoperator_for_solver_ = &op;
    auto child = prm.get_child_optional("preconditioner");
    preconditioner_ = Opm::PreconditionerFactory<Operator, Dune::Amg::SequentialInformation>::create(
        op, child ? *child : Opm::PropertyTree(), weightsCalculator, pressureIndex);
...
template <class Comm>
void
FlexibleSolver<Operator>::initSolver(const Opm::PropertyTree& prm, const Comm& comm)
{
    ...
    if (solver_type == "bicgstab") {
        ...
        if constexpr (!Opm::is_gpu_operator_v<Operator>) {
            if (solver_type == "flexgmres") {
                int restart = prm.get<int>("restart", 15);
                linsolver_ = std::make_shared<Dune::RestartedFlexibleGMResSolver<VectorType>>(
                ...
                if constexpr (std::is_same_v<typename VectorType::field_type, float>) {
                    ...
                    using MatrixType
                        = std::remove_const_t<std::remove_reference_t<decltype(linearoperator_for_solver_->getmat())>>;
                    linsolver_ = std::make_shared<Dune::UMFPack<MatrixType>>(
                        linearoperator_for_solver_->getmat(), verbosity, false);
                    direct_solver_ = true;
                }
#endif
#if HAVE_CUDA
...
template <class Operator>
void
FlexibleSolver<Operator>::recreateDirectSolver()
{
#if HAVE_SUITESPARSE_UMFPACK
    if constexpr (!Opm::is_gpu_operator_v<Operator>) {
        if constexpr (std::is_same_v<typename VectorType::field_type, float>) {
...
template <class Comm>
void
FlexibleSolver<Operator>::init(Operator& op,
                               const Comm& comm,
                               const Opm::PropertyTree& prm,
                               const std::function<VectorType()> weightsCalculator,
                               std::size_t pressureIndex)
...
#define INSTANTIATE_FLEXIBLESOLVER_OP(...)                                                                             \
    ...
    template Dune::FlexibleSolver<__VA_ARGS__>::FlexibleSolver(                                                        \
        __VA_ARGS__& op,                                                                                               \
        const Comm& comm,                                                                                              \
        const Opm::PropertyTree& prm,                                                                                  \
...
#define INSTANTIATE_FLEXIBLESOLVER_OP(...) template class Dune::FlexibleSolver<__VA_ARGS__>;
