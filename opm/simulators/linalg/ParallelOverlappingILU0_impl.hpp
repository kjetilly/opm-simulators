...
{
    return std::is_same_v<ParallelInfoT, Dune::Amg::SequentialInformation> ? Dune::SolverCategory::sequential
                                                                           : Dune::SolverCategory::overlapping;
}

template <class Matrix, class Domain, class Range, class ParallelInfoT>
ParallelOverlappingILU0<Matrix, Domain, Range, ParallelInfoT>::ParallelOverlappingILU0(
    const Matrix& A, const int n, const field_type w, MILU_VARIANT milu, bool redblack, bool reorder_sphere)
    : lower_()
    , upper_()
    , inv_()
    , comm_(nullptr)
    ...
    , A_(&reinterpret_cast<const Matrix&>(A))
    , iluIteration_(n)
    , milu_(milu)
    , redBlack_(redblack)
    , reorderSphere_(reorder_sphere)
{
    interiorSize_ = A.N();
...
template <class Matrix, class Domain, class Range, class ParallelInfoT>
ParallelOverlappingILU0<Matrix, Domain, Range, ParallelInfoT>::ParallelOverlappingILU0(const Matrix& A,
...
template <class Matrix, class Domain, class Range, class ParallelInfoT>
ParallelOverlappingILU0<Matrix, Domain, Range, ParallelInfoT>::ParallelOverlappingILU0(
    const Matrix& A, const field_type w, MILU_VARIANT milu, bool redblack, bool reorder_sphere)
...
template <class Matrix, class Domain, class Range, class ParallelInfoT>
void
ParallelOverlappingILU0<Matrix, Domain, Range, ParallelInfoT>::apply(Domain& v, const Range& d)
{
    OPM_TIMEBLOCK(apply);
    Range& md = reorderD(d);
...
template <class V>
void
ParallelOverlappingILU0<Matrix, Domain, Range, ParallelInfoT>::copyOwnerToAll(V& v) const
{
    if (comm_) {
...
template <class Matrix, class Domain, class Range, class ParallelInfoT>
void
ParallelOverlappingILU0<Matrix, Domain, Range, ParallelInfoT>::update()
{
    ...
    if (redBlack_) {
        ...
        if (reorderSphere_) {
            ordering_ = reorderVerticesSpheres(colors, noColors, verticesPerColor, graph, 0);
        } else {
            ordering_ = reorderVerticesPreserving(colors, noColors, verticesPerColor, graph);
        }
    }
    ...
    try {
        ...
        if (iluIteration_ == 0) {
            ...
            if (ordering_.empty()) {
                ...
                ILU_ = std::make_unique<Matrix>(A_->N(), A_->M(), A_->nonzeroes(), Matrix::row_wise);
                auto& newA = *ILU_;
            ...
            case MILU_VARIANT::MILU_2:
                detail::milu0_decomposition(*ILU_,
                                            detail::identityFunctor<typename Matrix::field_type>,
                                            detail::signFunctor<typename Matrix::field_type>);
                break;
            case MILU_VARIANT::MILU_3:
                detail::milu0_decomposition(*ILU_,
                                            detail::absFunctor<typename Matrix::field_type>,
                                            detail::signFunctor<typename Matrix::field_type>);
                break;
        ...
        std::cerr << "Exception occurred on process " << rank << " during "
                  << "setup of ILU0 preconditioner with message: " << message << std::endl;
...
template <class Matrix, class Domain, class Range, class ParallelInfoT>
Range&
ParallelOverlappingILU0<Matrix, Domain, Range, ParallelInfoT>::reorderD(const Range& d)
{
    if (ordering_.empty()) {
...
template <class Matrix, class Domain, class Range, class ParallelInfoT>
Domain&
ParallelOverlappingILU0<Matrix, Domain, Range, ParallelInfoT>::reorderV(Domain& v)
{
    if (ordering_.empty()) {
        return v;
    } else {
...
template <class Matrix, class Domain, class Range, class ParallelInfoT>
void
ParallelOverlappingILU0<Matrix, Domain, Range, ParallelInfoT>::reorderBack(const Range& reorderedV, Range& v)
{
    if (!ordering_.empty()) {
        std::size_t i = 0;
        for (const auto index : ordering_) {
