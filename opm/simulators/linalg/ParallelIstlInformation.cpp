    ...
    void operator()(const typename ParallelIndexSet::IndexPair& pair)
    {
        for (std::size_t i = 0; i < num_components_; i++)
            indexSet_->add(i * component_size_ + pair.global(),
                           LocalIndex(i * local_component_size_ + pair.local(), pair.local().attribute()));
    }
...
ParallelISTLInformation::ParallelISTLInformation(const std::shared_ptr<ParallelIndexSet>& indexSet,
...
ParallelISTLInformation::ParallelISTLInformation(const ParallelISTLInformation& other)
    ...
    , remoteIndices_(other.remoteIndices_)
    , communicator_(other.communicator_)
{
}

void
ParallelISTLInformation::copyValuesTo(ParallelIndexSet& indexSet,
                                      ...
                                      std::size_t num_components) const
{
    ...
    IndexSetInserter<ParallelIndexSet> inserter(indexSet, global_component_size, local_component_size, num_components);
...
template <class T>
void
ParallelISTLInformation::copyOwnerToAll(const T& source, T& dest) const
{
...
void
ParallelISTLInformation::computeReduction(const Container& container, BinaryOperator binaryOperator, T& value) const
{
...
void
ParallelISTLInformation::computeTupleReduction(const std::tuple<Containers...>& containers,
                                               std::tuple<BinaryOperators...>& operators,
    ...
    -> decltype(container[0] * (*maskContainer)[0])
{
    ...
    if (maskContainer) {
        return std::inner_product(container.begin(), container.end(), maskContainer->begin(), initial);
    } else {
...
template <class T>
using C1 = std::vector<T>;
template <class T>
using Ops1 = Reduction::MaskIDOperator<std::plus<T>>;

template <class T>
using C2 = std::tuple<std::vector<T>, std::vector<T>, std::vector<T>, std::vector<T>, std::vector<T>>;
template <class T>
...
#define INSTANCE2(T)                                                                                                   \
    template void ParallelISTLInformation::computeReduction<C2<T>, Ops2<T>, Vals2<T>>(                                 \
        const C2<T>&, Ops2<T>, Vals2<T>&) const;
