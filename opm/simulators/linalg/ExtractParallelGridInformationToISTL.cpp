...
void
extractParallelGridInformationToISTL(const Dune::CpGrid& grid, std::any& anyComm)
{
    if (grid.comm().size() > 1) {
        ...
        anyComm = std::any(Opm::ParallelISTLInformation(
            Dune::stackobject_to_shared_ptr(idx), Dune::stackobject_to_shared_ptr(ridx), grid.comm()));
    }
}
#else
// Missing support for MPI or dune-istl -> do nothing.
void
extractParallelGridInformationToISTL(const Dune::CpGrid&, std::any&)
{
}
