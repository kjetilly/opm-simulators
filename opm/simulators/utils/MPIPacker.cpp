...
std::size_t
mpi_buffer_size(const std::size_t bufsize, const std::size_t position)
{
    if (bufsize < position) {
        throw std::invalid_argument("Buffer size should never be less than position!");
    }

    return std::min(bufsize - position, static_cast<std::size_t>(std::numeric_limits<int>::max()));
}

template <std::size_t Size>
std::size_t
Packing<false, std::bitset<Size>>::packSize(const std::bitset<Size>& data, Parallel::MPIComm comm)
{
...
template <std::size_t Size>
void
Packing<false, std::bitset<Size>>::pack(const std::bitset<Size>& data,
...
template <std::size_t Size>
void
Packing<false, std::bitset<Size>>::unpack(std::bitset<Size>& data,
...
std::size_t
Packing<false, std::string>::packSize(const std::string& data, Parallel::MPIComm comm)
{
    int size;
...
void
Packing<false, std::string>::pack(const std::string& data,
                                  std::vector<char>& buffer,
                                  std::size_t& position,
                                  Parallel::MPIComm comm)
{
    ...
    MPI_Pack(&length,
             ...
             mpi_buffer_size(buffer.size(), position),
             &int_position,
             comm);
    MPI_Pack(data.data(),
             length,
...
void
Packing<false, std::string>::unpack(std::string& data,
                                    const std::vector<char>& buffer,
                                    std::size_t& position,
                                    Opm::Parallel::MPIComm comm)
{
    std::size_t length = 0;
    int int_position = 0;
    MPI_Unpack(buffer.data() + position,
               mpi_buffer_size(buffer.size(), position),
               &int_position,
               &length,
               1,
...
std::size_t
Packing<false, time_point>::packSize(const time_point&, Opm::Parallel::MPIComm comm)
{
    return Packing<true, time_point::duration::rep>::packSize(time_point::duration::rep(), comm);
}

void
Packing<false, time_point>::pack(const time_point& data,
                                 std::vector<char>& buffer,
                                 std::size_t& position,
                                 Parallel::MPIComm comm)
{
...
void
Packing<false, time_point>::unpack(time_point& data,
                                   const std::vector<char>& buffer,
                                   std::size_t& position,
                                   Parallel::MPIComm comm)
{
