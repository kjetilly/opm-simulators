...
bool
FileExists(const std::string& filename_in, const Parallel::Communication& comm)
{
...
void
DamarisSettings::SetRandString(void)
{
...
DamarisSettings::getKeywords([[maybe_unused]] const Parallel::Communication& comm, const std::string& OutputDir)
{
    ...
    if ((pythonFilename_.size() > 0) && (paraviewPythonFilename_.size() > 0)) {
        ...
        // The Python script will issue: ERROR: Initialiazation via BOOST_PYTHON_MODULE(server) failed at
        // import_mpi4py()
        OPM_THROW(std::runtime_error,
                  "ERROR: Both the Python (--damaris-python-script command line argument) and Paraview Python "
    ...
    OpmLog::info(fmt::format("Opm::DamarisOutput::DamarisKeywords() : <simulation name={} ", simName_str));
    ...
    OpmLog::info(fmt::format(
        "Opm::DamarisOutput::DamarisKeywords() : <buffer name={} size={} ", shmemName_str, shmemSizeBytes_str));
    ...
    OpmLog::info(fmt::format(
        "Opm::DamarisOutput::DamarisKeywords() : <dedicated cores={} nodes={} ", nDamarisCores_str, nDamarisNodes_str));
    ...
    OpmLog::info(
        fmt::format("Opm::DamarisOutput::DamarisKeywords() : <log FileName={}/damaris_log/{} Flush={}  LogLevel={} ",
                    OutputDir,
                    simName_str,
                    logFlush_str,
                    logLevel_str));
    ...
    std::map<std::string, std::string> damaris_keywords = {
        ...
        {"_PATH_REGEX_",
         OutputDir}, /* Do Not change the string "_PATH_REGEX_" as it is used to search for the output path */
        ...
        {"_PYTHON_SCRIPT_",
         pythonFilename_}, /* if a Python script is specified then assume that we want to publish the data to Python */
        {"_PRESSURE_UNIT_", "Pa"},
