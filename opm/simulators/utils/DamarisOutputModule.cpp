 ...
 */
void
initializeDamaris(const Parallel::Communication comm,
                  const int mpiRank,
                  const std::map<std::string, std::string>& find_replace_map)
{
    ...
    if (cs_damaris_xml_file != NULL) {
        OpmLog::info(std::string("Initializing Damaris from environment variable FLOW_DAMARIS_XML_FILE: ")
                     + cs_damaris_xml_file);
        ...
        // std::map<std::string, std::string> find_replace_map = DamarisKeywords(outputDir,
        ...
        if (dam_err != DAMARIS_OK) {
            OpmLog::error(
                fmt::format("damariswriter::initializeDamaris()       : ( rank:{}) "
