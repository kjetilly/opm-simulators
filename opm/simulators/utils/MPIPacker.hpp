...
namespace detail
{
    ...
    struct Packing<true, T> {
        ...
        //! \param comm The communicator to use
        static void
        pack(const T* data, std::size_t n, std::vector<char>& buffer, std::size_t& position, Parallel::MPIComm comm)
        {
            int int_position = 0;
            MPI_Pack(data,
                     n,
                     Dune::MPITraits<T>::getType(),
        ...
        //! \param comm The communicator to use
        static void
        unpack(T* data, std::size_t n, const std::vector<char>& buffer, std::size_t& position, Parallel::MPIComm comm)
        {
            int int_position = 0;
