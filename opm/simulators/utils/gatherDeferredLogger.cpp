...
void
packMessages(const std::vector<Opm::DeferredLogger::Message>& local_messages,
             std::vector<char>& buf,
             int& offset,
             const Opm::Parallel::Communication mpi_communicator)
{
    ...
    for (const auto& lm : local_messages) {
        MPI_Pack(static_cast<void*>(const_cast<std::int64_t*>(&lm.flag)),
                 1,
                 MPI_INT64_T,
                 buf.data(),
                 buf.size(),
...
unpackSingleMessage(const std::vector<char>& recv_buffer, int& offset, const MPI_Comm mpi_communicator)
{
    int64_t flag;
...
unpackMessages(const std::vector<char>& recv_buffer, const std::vector<int>& displ, const MPI_Comm mpi_communicator)
{
...
Opm::DeferredLogger
gatherDeferredLogger(const Opm::DeferredLogger& local_deferredlogger, Opm::Parallel::Communication mpi_communicator)
{
