...
void
setZoltanParameters(const Opm::ParallelNLDDPartitioningZoltan::ZoltanParamMap& params, Zoltan_Struct* zz)
{
...
public:
    ...
    explicit EnumerateSeenVertices(const std::size_t numVertices, const std::vector<Edge>& edges)
...
int
numVertices(void* graphPtr, int* ierr)
{
    *ierr = ZOLTAN_OK;
           ...
           int* ierr)
{
    ...
    std::transform(&localIds[0],
                   ...
                   &globalIds[0],
                   [graph](const int localCell) -> ZOLTAN_ID_TYPE { return graph->globalId(localCell); });
         ...
         int* ierr)
{
    ...
    if ((numElmsPerGid != numElmsPerLid) || (numElmsPerLid != 1) || (numCells != graph->numVertices())) {
        *ierr = ZOLTAN_FATAL;
    }
    ...
    for (auto cell = 0 * numCells, ix = 0; cell < numCells; ++cell) {
        ...
        for (auto neighIx = ia[localCell], end = ia[localCell + 1]; neighIx != end; ++neighIx, ++ix) {
...
{
    Zoltan_LB_Free_Part(&this->globalId, &this->localId, &this->process, &this->block);
}
...
Partitioner::operator()(void* graphPtr, const std::size_t numCells)
{
    ...
    auto numRecv = 0;
    auto numSend = 0;
    const auto rc = Zoltan_LB_Partition(this->zz_,
                                        &partitionChanged,
                                        &numElmPerGid,
                                        &numElmPerLid,
                                        &numRecv,
...
void
Opm::ParallelNLDDPartitioningZoltan::addVertexGroup(const std::vector<int>& vertices)
{
...
Opm::ParallelNLDDPartitioningZoltan::partitionElements(const ZoltanParamMap& params) const
{
    ...
    auto graph = VertexGraph {
        this->comm_.rank(), this->numElements_, this->conns_, vertexId, reachableVertexGroups, this->globalCell_};

    const auto partsForReachableCells
        = Partitioner {this->comm_, params}(static_cast<void*>(&graph), graph.numVertices());
