 ...
 */
template <class Grid>
class PropsDataHandle : public Dune::CommDataHandleIF<PropsDataHandle<Grid>, double>
{
public:
        ...
        , m_distributed_fieldProps(eclState.m_fieldProps)
    {
        ...
        if (comm.rank() == 0) {
            ...
            using ElementMapper = Dune::MultipleCodimMultipleGeomTypeMapper<typename Grid::LevelGridView>;
            ...
            for (const auto& element : elements(gridView, Dune::Partitions::interiorBorder)) {
                ...
                for (const auto& intKey : m_intKeys) {
                    ...
                    data.emplace_back(fieldData.data[index], static_cast<unsigned char>(fieldData.value_status[index]));
                }
    ...
    ~PropsDataHandle()
    {
        ...
        for (const auto& element : elements(gridView, Dune::Partitions::all)) {
            ...
            for (const auto& intKey : m_intKeys) {
                ...
                m_distributed_fieldProps.m_intProps[intKey].value_status[index]
                    = static_cast<value::status>(pair.second);
            }

            for (const auto& doubleKey : m_doubleKeys) {
                const auto& pair = data->second[counter++];
                m_distributed_fieldProps.m_doubleProps[doubleKey].data[index] = pair.first;
                m_distributed_fieldProps.m_doubleProps[doubleKey].value_status[index]
                    = static_cast<value::status>(pair.second);
            }
        }
    }
