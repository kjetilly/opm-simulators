...
bool
RelpermDiagnostics::phaseCheck_(const EclipseState& es)
{
...
void
RelpermDiagnostics::satFamilyCheck_(const EclipseState& eclState)
{
...
void
RelpermDiagnostics::tableCheck_(const EclipseState& eclState)
{
...
void
RelpermDiagnostics::swofTableCheck_(const SwofTable& swofTables, const int satnumIdx)
{
...
void
RelpermDiagnostics::sgofTableCheck_(const SgofTable& sgofTables, const int satnumIdx)
{
...
void
RelpermDiagnostics::slgofTableCheck_(const SlgofTable& slgofTables, const int satnumIdx)
{
...
void
RelpermDiagnostics::swfnTableCheck_(const SwfnTable& swfnTables, const int satnumIdx)
{
...
void
RelpermDiagnostics::wsfTableCheck_(const WsfTable& wsfTables, const int satnumIdx)
{
...
void
RelpermDiagnostics::sgfnTableCheck_(const SgfnTable& sgfnTables, const int satnumIdx)
{
...
void
RelpermDiagnostics::gsfTableCheck_(const GsfTable& gsfTables, const int satnumIdx)
{
...
void
RelpermDiagnostics::sof3TableCheck_(const Sof3Table& sof3Tables, const int satnumIdx)
{
    ...
    if (krog.back() != krow.back()) {
        const std::string msg
            = "In SOF3 table SATNUM = " + regionIdx + ", max value of krog and krow should be the same.";
        OpmLog::error(msg);
    }
}
...
void
RelpermDiagnostics::sof2TableCheck_(const Sof2Table& sof2Tables, const int satnumIdx)
{
...
void
RelpermDiagnostics::sgwfnTableCheck_(const SgwfnTable& sgwfnTables, const int satnumIdx)
{
...
void
RelpermDiagnostics::sgcwmisTableCheck_(const SgcwmisTable& sgcwmisTables, const int satnumIdx)
{
    ...
    if (sgc.front() < 0.0 || sgc.back() > 1.0) {
        const std::string msg
            = "In SGCWMIS table SATNUM = " + regionIdx + ", critical gas saturation should be in range [0,1].";
        OpmLog::error(msg);
    }
}
...
void
RelpermDiagnostics::sorwmisTableCheck_(const SorwmisTable& sorwmisTables, const int satnumIdx)
{
    ...
    if (sor.front() < 0.0 || sor.back() > 1.0) {
        const std::string msg
            = "In SORWMIS table SATNUM = " + regionIdx + ", critical oil saturation should be in range [0,1].";
        OpmLog::error(msg);
    }
}
...
void
RelpermDiagnostics::ssfnTableCheck_(const SsfnTable& ssfnTables, const int satnumIdx)
{
    ...
    if (krgm.front() < 0.0 || krgm.back() > 1.0) {
        const std::string msg = "In SSFN table SATNUM = " + regionIdx
            + ", gas relative permeability multiplier should be in range [0,1].";
        OpmLog::error(msg);
    }
    ...
    if (krsm.front() < 0.0 || krsm.back() > 1.0) {
        const std::string msg = "In SSFN table SATNUM = " + regionIdx
            + ", solvent relative permeability multiplier should be in range [0,1].";
        OpmLog::error(msg);
    }
}
...
void
RelpermDiagnostics::miscTableCheck_(const MiscTable& miscTables, const int miscnumIdx)
{
...
void
RelpermDiagnostics::msfnTableCheck_(const MsfnTable& msfnTables, const int satnumIdx)
{
    ...
    if (frac.front() < 0.0 || frac.back() > 1.0) {
        const std::string msg
            = "In MSFN table SATNUM = " + regionIdx + ", total gas fraction should be in range [0,1].";
        OpmLog::error(msg);
    }

    // Check gas_solvent relperm multiplier column.
    if (krgsm.front() < 0.0 || krgsm.back() > 1.0) {
        const std::string msg = "In MSFN table SATNUM = " + regionIdx
            + ", gas+solvent relative permeability multiplier should be in range [0,1].";
        OpmLog::error(msg);
    }
    ...
    if (krom.front() > 1.0 || krom.back() < 0.0) {
        const std::string msg = "In MSFN table SATNUM = " + regionIdx
            + ", oil relative permeability multiplier should be in range [0,1].";
        OpmLog::error(msg);
    }
}
...
void
RelpermDiagnostics::unscaledEndPointsCheck_(const EclipseState& eclState)
{
    ...
    const auto tolcrit = runspec.saturationFunctionControls().minimumRelpermMobilityThreshold();

    const auto rtep = satfunc::getRawTableEndpoints(tables, phases, tolcrit);

    const auto rfunc = satfunc::getRawFunctionValues(tables, phases, rtep);
    ...
    for (int satnumIdx = 0; satnumIdx < numSatRegions; ++satnumIdx) {
        this->unscaledEpsInfo_[satnumIdx].extractUnscaled(rtep, rfunc, satnumIdx);
        ...
        if (fluidSystem_ == FluidSystem::BlackOil) {
            ...
            if (krow_value != krog_value) {
                const std::string msg
                    = "In saturation table SATNUM = " + regionIdx + ", Krow(Somax) should be equal to Krog(Somax).";
                OpmLog::warning(msg);
            }
        }
        ...
        if (((unscaledEpsInfo_[satnumIdx].Sowcr + unscaledEpsInfo_[satnumIdx].Swcr) - 1) >= 0) {
            const std::string msg
                = "In saturation table SATNUM = " + regionIdx + ", Sowcr + Swcr should be less than 1.";
            OpmLog::warning(msg);
        }
        if (((unscaledEpsInfo_[satnumIdx].Sogcr + unscaledEpsInfo_[satnumIdx].Sgcr + unscaledEpsInfo_[satnumIdx].Swl)
             - 1)
            > 0) {
            const std::string msg
                = "In saturation table SATNUM = " + regionIdx + ", Sogcr + Sgcr + Swco should be less than 1.";
            OpmLog::warning(msg);
        }
    }
}
...
void
RelpermDiagnostics::diagnosis(const EclipseState& eclState, const LevelCartesianIndexMapper& levelCartesianIndexMapper)
{
...
template <class LevelCartesianIndexMapper>
void
RelpermDiagnostics::scaledEndPointsCheck_(const EclipseState& eclState,
                                          const LevelCartesianIndexMapper& levelCartesianIndexMapper)
{
    ...
    for (int c = 0; c < nc; ++c) {
        ...
        std::string cellIdx;
        {
            ...
            cellIdx
                = "(" + std::to_string(ijk[0]) + ", " + std::to_string(ijk[1]) + ", " + std::to_string(ijk[2]) + ")";
        }
        ...
        if (scaledEpsInfo_[c].Sgu > (1.0 - scaledEpsInfo_[c].Swl + tolerance)) {
            const std::string msg
                = "For scaled endpoints input, cell" + cellIdx + " SATNUM = " + satnumIdx + ", SGU exceed 1.0 - SWL";
        ...
        if (scaledEpsInfo_[c].Sgl > (1.0 - scaledEpsInfo_[c].Swu + tolerance)) {
            const std::string msg
                = "For scaled endpoints input, cell" + cellIdx + " SATNUM = " + satnumIdx + ", SGL exceed 1.0 - SWU";
            OpmLog::warning(tag, msg);
        }
        ...
        if (threepoint && fluidSystem_ == FluidSystem::BlackOil) {
            ...
            if ((scaledEpsInfo_[c].Sowcr + scaledEpsInfo_[c].Swcr) >= (1.0 + tolerance)) {
                const std::string msg = "For scaled endpoints input, cell" + cellIdx + " SATNUM = " + satnumIdx
...
#define INSTANCE_DIAGNOSIS(...)                                                                                        \
    template void RelpermDiagnostics::diagnosis<Opm::LevelCartesianIndexMapper<__VA_ARGS__>>(                          \
        const EclipseState&, const Opm::LevelCartesianIndexMapper<__VA_ARGS__>&);                                      \
    template void RelpermDiagnostics::scaledEndPointsCheck_<Opm::LevelCartesianIndexMapper<__VA_ARGS__>>(              \
        const EclipseState&, const Opm::LevelCartesianIndexMapper<__VA_ARGS__>&);
