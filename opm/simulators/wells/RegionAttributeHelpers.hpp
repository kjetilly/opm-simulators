...
namespace RegionAttributeHelpers
{
    ...
    struct AverageIncrementCalculator {
         ...
         */
        std::tuple<Scalar, Scalar, Scalar, Scalar, int> operator()(const std::vector<Scalar>& pressure,
        ...
        {
            if (ownership[cell]) {
                return std::make_tuple(pressure[cell], temperature[cell], rs[cell], rv[cell], 1);
            } else {
