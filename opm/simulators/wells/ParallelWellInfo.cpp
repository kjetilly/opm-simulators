...
GlobalPerfContainerFactory<Scalar>::GlobalPerfContainerFactory(const IndexSet& local_indices,
                                                               const Parallel::Communication comm,
                                                               const int num_local_perfs)
    : local_indices_(local_indices)
    , comm_(comm)
{
    if (comm_.size() > 1) {
        ...
        std::sort(global_pairs.begin(), global_pairs.end(), [](const Pair& p1, const Pair& p2) {
            return p1.first < p2.first;
        });
        map_received_.resize(global_pairs.size());
        std::transform(global_pairs.begin(), global_pairs.end(), map_received_.begin(), [](const Pair& pair) {
            return pair.second;
        });
        perf_ecl_index_.resize(global_pairs.size());
        std::transform(global_pairs.begin(), global_pairs.end(), perf_ecl_index_.begin(), [](const Pair& pair) {
            return pair.first;
        });
...
template <class Scalar>
void
GlobalPerfContainerFactory<Scalar>::buildLocalToGlobalMap() const
{
...
template <class Scalar>
int
GlobalPerfContainerFactory<Scalar>::localToGlobal(std::size_t localIndex) const
{
    if (comm_.size() == 1)
        return localIndex;
    if (!l2g_map_built_)
...
template <class Scalar>
void
GlobalPerfContainerFactory<Scalar>::buildGlobalToLocalMap() const
{
...
template <class Scalar>
int
GlobalPerfContainerFactory<Scalar>::globalToLocal(const int globalIndex) const
{
    if (comm_.size() == 1)
        return globalIndex;
    if (!g2l_map_built_) {
...
template <class Scalar>
std::vector<Scalar>
GlobalPerfContainerFactory<Scalar>::createGlobal(const std::vector<Scalar>& local_perf_container,
                                                 std::size_t num_quantities) const
{
    ...
    if (comm_.size() > 1) {
        ...
        if (num_quantities == 1) {
            comm_.allgatherv(local_perf_container.data(),
                             local_perf_container.size(),
                             global_recv.data(),
                             const_cast<int*>(sizes_.data()),
...
template <class Scalar>
void
GlobalPerfContainerFactory<Scalar>::copyGlobalToLocal(const std::vector<Scalar>& global,
...
template <class Scalar>
int
GlobalPerfContainerFactory<Scalar>::numGlobalPerfs() const
{
    return num_global_perfs_;
}

template <class Scalar>
CommunicateAboveBelow<Scalar>::CommunicateAboveBelow([[maybe_unused]] const Parallel::Communication& comm)
#if HAVE_MPI
    : comm_(comm)
    , interface_(comm_)
#endif
...
template <class Scalar>
void
CommunicateAboveBelow<Scalar>::clear()
{
#if HAVE_MPI
    above_indices_ = {};
    current_indices_ = {};
...
template <class Scalar>
void
CommunicateAboveBelow<Scalar>::beginReset()
{
    clear();
#if HAVE_MPI
    if (comm_.size() > 1) {
...
template <class Scalar>
int
CommunicateAboveBelow<Scalar>::endReset()
{
#if HAVE_MPI
    if (comm_.size() > 1) {
...
void
CommunicateAboveBelow<Scalar>::partialSumPerfValues(RAIterator begin, RAIterator end) const
{
    if (this->comm_.size() < 2) {
        ...
        std::transform(global_pairs.begin(), global_pairs.end(), sums.begin(), [](const Pair& p) { return p.second; });
        std::partial_sum(sums.begin(), sums.end(), sums.begin());
        // assign the values (both ranges are sorted by the ecl index)
        auto global_pair = global_pairs.begin();
        for (const auto& pair : current_indices_) {
            global_pair = std::lower_bound(global_pair,
                                           global_pairs.end(),
                                           pair.global(),
                                           [](const Pair& val1, const GlobalIndex& val2) { return val1.first < val2; });
...
template <class Scalar>
std::vector<Scalar>
CommunicateAboveBelow<Scalar>::communicateAbove(Scalar first_above, const Scalar* current, std::size_t size)
{
...
template <class Scalar>
std::vector<Scalar>
CommunicateAboveBelow<Scalar>::communicateBelow(Scalar last_below, const Scalar* current, std::size_t size)
{
...
template <class Scalar>
void
CommunicateAboveBelow<Scalar>::pushBackEclIndex([[maybe_unused]] int above,
...
template <class Scalar>
void
ParallelWellInfo<Scalar>::DestroyComm::operator()(Parallel::Communication* comm)
{
#if HAVE_MPI
    // Only delete custom communicators.
    bool del = comm && (*comm != Dune::MPIHelper::getLocalCommunicator())
...
template <class Scalar>
int
CommunicateAboveBelow<Scalar>::numLocalPerfs() const
{
    return num_local_perfs_;
}

template <class Scalar>
ParallelWellInfo<Scalar>::ParallelWellInfo(const std::string& name, bool hasLocalCells)
    : name_(name)
    , hasLocalCells_(hasLocalCells)
    , isOwner_(true)
...
ParallelWellInfo<Scalar>::ParallelWellInfo(const std::pair<std::string, bool>& well_info,
    ...
    : name_(well_info.first)
    , hasLocalCells_(well_info.second)
    , rankWithFirstPerf_(-1)
{
#if HAVE_MPI
    MPI_Comm newComm;
...
template <class Scalar>
void
ParallelWellInfo<Scalar>::setActivePerfToLocalPerfMap(const std::unordered_map<int, int> active_to_local_map) const
{
...
template <class Scalar>
int
ParallelWellInfo<Scalar>::localPerfToActivePerf(std::size_t localIndex) const
{
    if (comm_->size() == 1)
        return localIndex;
...
template <class Scalar>
int
ParallelWellInfo<Scalar>::activePerfToLocalPerf(const int activeIndex) const
{
    if (comm_->size() == 1)
        return activeIndex;
...
template <class Scalar>
int
ParallelWellInfo<Scalar>::localPerfToGlobalPerf(std::size_t localIndex) const
{
    if (globalPerfCont_)
...
template <class Scalar>
int
ParallelWellInfo<Scalar>::globalPerfToLocalPerf(const int globalIndex) const
{
    if (globalPerfCont_)
...
template <class Scalar>
void
ParallelWellInfo<Scalar>::communicateFirstPerforation(bool hasFirst)
{
    int first = hasFirst;
    std::vector<int> firstVec(comm_->size());
    comm_->allgather(&first, 1, firstVec.data());
    auto found = std::find_if(firstVec.begin(), firstVec.end(), [](int i) -> bool { return i; });
...
template <class Scalar>
void
ParallelWellInfo<Scalar>::pushBackEclIndex(int above, int current)
{
...
template <class Scalar>
void
ParallelWellInfo<Scalar>::beginReset()
{
...
template <class Scalar>
void
ParallelWellInfo<Scalar>::endReset()
{
    int local_num_perfs = commAboveBelow_->endReset();
    globalPerfCont_.reset(
        new GlobalPerfContainerFactory<Scalar>(commAboveBelow_->getIndexSet(), *comm_, local_num_perfs));
}
...
template <class T>
T
ParallelWellInfo<Scalar>::broadcastFirstPerforationValue(const T& t) const
{
#if HAVE_MPI
    T res = t;
...
template <class Scalar>
std::vector<Scalar>
ParallelWellInfo<Scalar>::communicateAboveValues(Scalar zero_value,
                                                 const Scalar* current_values,
                                                 std::size_t size) const
{
    return commAboveBelow_->communicateAbove(zero_value, current_values, size);
}

template <class Scalar>
std::vector<Scalar>
ParallelWellInfo<Scalar>::communicateAboveValues(Scalar zero_value, const std::vector<Scalar>& current_values) const
{
    return commAboveBelow_->communicateAbove(zero_value, current_values.data(), current_values.size());
}

template <class Scalar>
std::vector<Scalar>
ParallelWellInfo<Scalar>::communicateBelowValues(Scalar last_value,
                                                 const Scalar* current_values,
                                                 std::size_t size) const
{
    return commAboveBelow_->communicateBelow(last_value, current_values, size);
}

template <class Scalar>
std::vector<Scalar>
ParallelWellInfo<Scalar>::communicateBelowValues(Scalar last_value, const std::vector<Scalar>& current_values) const
{
    return commAboveBelow_->communicateBelow(last_value, current_values.data(), current_values.size());
}
...
ParallelWellInfo<Scalar>::getGlobalPerfContainerFactory() const
{
    if (globalPerfCont_)
        ...
        OPM_THROW(std::logic_error, "No ecl indices have been added via beginReset, pushBackEclIndex, endReset");
}

template <class Scalar>
bool
operator<(const ParallelWellInfo<Scalar>& well1, const ParallelWellInfo<Scalar>& well2)
{
    return well1.name() < well2.name()
        || (!(well2.name() < well1.name()) && well1.hasLocalCells() < well2.hasLocalCells());
}
...
operator==(const ParallelWellInfo<Scalar>& well1, const ParallelWellInfo<Scalar>& well2)
{
    ...
    ret = ret && static_cast<MPIComm>(well1.communication()) == static_cast<MPIComm>(well2.communication());
#endif
    return ret;
}
...
template <class Scalar>
bool
operator<(const std::pair<std::string, bool>& pair, const ParallelWellInfo<Scalar>& well)
{
...
template <class Scalar>
bool
operator<(const ParallelWellInfo<Scalar>& well, const std::pair<std::string, bool>& pair)
{
...
template <class Scalar>
CheckDistributedWellConnections<Scalar>::CheckDistributedWellConnections(const Well& well,
...
template <class Scalar>
void
CheckDistributedWellConnections<Scalar>::connectionFound(std::size_t i)
{
...
template <class Scalar>
bool
CheckDistributedWellConnections<Scalar>::checkAllConnectionsFound()
{
    ...
    pwinfo_.communication().sum(foundConnections_.data(), foundConnections_.size());

    std::string msg
        = std::string("Cells with these i,j,k indices were not found ") + "in grid (well = " + pwinfo_.name() + "):";
    ...
    for (auto conFound = start; conFound != foundConnections_.end(); ++conFound) {
        if (*conFound == 0) {
            ...
            msg = msg + " " + std::to_string(completion.getI()) + "," + std::to_string(completion.getJ()) + ","
...
#define INSTANTIATE_BROADCAST_FIRST_PERF_DENSEAD_EVALUATION(T, DIM)                                                    \
    template std::tuple<T, Opm::DenseAd::Evaluation<T, DIM, 0u>, int> Opm::ParallelWellInfo<T>::                       \
        broadcastFirstPerforationValue<std::tuple<T, Opm::DenseAd::Evaluation<T, DIM, 0u>, int>>(                      \
            std::tuple<T, Opm::DenseAd::Evaluation<T, DIM, 0u>, int> const&) const;
...
#define INSTANTIATE_TYPE(T)                                                                                            \
    ...
    template typename cdIter<T>::value_type ParallelWellInfo<T>::sumPerfValues<cdIter<T>>(cdIter<T>, cdIter<T>) const; \
    template typename dIter<T>::value_type ParallelWellInfo<T>::sumPerfValues<dIter<T>>(dIter<T>, dIter<T>) const;     \
    template int ParallelWellInfo<T>::broadcastFirstPerforationValue<int>(const int&) const;                           \
    template T ParallelWellInfo<T>::broadcastFirstPerforationValue<T>(const T&) const;                                 \
