...
public:
    ...
    explicit PackUnpackXConn(const bool isOwner, const XConn& local, XConn& global);

    // Pack all data associated with link.
    void pack(const int link, MessageBufferType& buffer);

    // Unpack all data associated with link.
    void unpack([[maybe_unused]] const int link, MessageBufferType& buffer);
...
PackUnpackXConn::PackUnpackXConn(const bool isOwner, const XConn& local, XConn& global)
    : local_(local)
    , global_(global)
{
    if (!isOwner) {
    ...
    this->global_.insert(this->global_.end(), this->local_.begin(), this->local_.end());
}

void
PackUnpackXConn::pack(const int link, MessageBufferType& buffer)
{
    // We should only get one link
    if (link != 0) {
        throw std::logic_error {"link in pack() does not match expected value 0"};
    }
...
void
PackUnpackXConn::unpack([[maybe_unused]] const int link, MessageBufferType& buffer)
{
...
WellState<Scalar, IndexTraits>::WellState(const ParallelWellInfo<Scalar>& pinfo)
{
    ...
    wells_.add("test4", SingleWellState<Scalar, IndexTraits> {"dummy", pinfo, pu, false, 0.0, {}, 0.0});
}

template <typename Scalar, typename IndexTraits>
WellState<Scalar, IndexTraits>
WellState<Scalar, IndexTraits>::serializationTestObject(const ParallelWellInfo<Scalar>& pinfo)
{
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::base_init(
    ...
    const SummaryState& summary_state)
{
    ...
    this->wells_.clear();
    {
        ...
        for (int w = 0; w < nw; ++w) {
            ...
            initSingleWell(
                cellPressures, cellTemperatures, well, well_perf_data[w], parallel_well_info[w], summary_state);
        }
    }
}
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::initSingleProducer(const Well& well,
                                                   ...
                                                   const SummaryState& summary_state)
{
    ...
    auto& ws = this->wells_.add(
        well.name(),
        SingleWellState<Scalar, IndexTraits> {
            well.name(), well_info, this->phaseUsageInfo(), true, pressure_first_connection, well_perf_data, temp});
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::initSingleInjector(const Well& well,
                                                   const ParallelWellInfo<Scalar>& well_info,
                                                   Scalar pressure_first_connection,
                                                   ...
                                                   const SummaryState& summary_state)
{
    ...
    auto& ws = this->wells_.add(
        well.name(),
        SingleWellState<Scalar, IndexTraits> {
            well.name(), well_info, this->phaseUsageInfo(), false, pressure_first_connection, well_perf_data, temp});
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::initSingleWell(const std::vector<Scalar>& cellPressures,
                                               const std::vector<Scalar>& cellTemperatures,
                                               const Well& well,
                                               const std::vector<PerforationData<Scalar>>& well_perf_data,
                                               const ParallelWellInfo<Scalar>& well_info,
                                               const SummaryState& summary_state)
{
    ...
    // The following call is necessary to ensure that processes that do not contain the first perforation get the
    // correct value
    ...
    if (well.isInjector()) {
        ...
        this->initSingleInjector(
            well, well_info, pressure_first_connection, temperature_first_connection, well_perf_data, summary_state);
    } else {
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::init(
    const std::vector<Scalar>& cellPressures,
    const std::vector<Scalar>& cellTemperatures,
    ...
    const bool enableDistributedWells)
{
    ...
    this->base_init(cellPressures, cellTemperatures, wells_ecl, parallel_well_info, well_perf_data, summary_state);
        ...
        for (int perf = 0; perf < num_perf_this_well; ++perf) {
            if (wells_ecl[w].getStatus() == Well::Status::OPEN) {
                for (int p = 0; p < this->numPhases(); ++p) {
                    perf_data.phase_rates[this->numPhases() * perf + p]
                        = ws.surface_rates[p] / Scalar(global_num_perf_this_well);
                }
            }
    ...
    if ((prevState != nullptr) && (prevState->size() > 0)) {
        for (int w = 0; w < nw; ++w) {
            ...
            if (const auto num_perf_this_well = new_well.perf_data.size();
                ...
                const auto global_num_perf_this_well = static_cast<Scalar>(wells_ecl[w].getConnections().num_open());
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::resize(
    const std::vector<Well>& wells_ecl,
    const std::vector<std::reference_wrapper<ParallelWellInfo<Scalar>>>& parallel_well_info,
    const Schedule& schedule,
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::gatherVectorsOnRoot(const std::vector<data::Connection>& from_connections,
...
template <typename Scalar, typename IndexTraits>
data::Wells
WellState<Scalar, IndexTraits>::report(const int* globalCellIdxMap,
                                       const std::function<bool(const int)>& wasDynamicallyClosed) const
{
    ...
    for (std::size_t well_index = 0; well_index < this->size(); ++well_index) {
        ...
        well.rates.set(rt::dissolved_gas,
                       ws.phase_mixing_rates[ws.dissolved_gas] + ws.phase_mixing_rates[ws.dissolved_gas_in_water]);
        ...
        if (const auto& pwinfo = ws.parallel_info.get(); pwinfo.communication().size() == 1) {
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::reportConnections(std::vector<data::Connection>& connections,
                                                  ...
                                                  const int* globalCellIdxMap) const
{
    ...
    if (!perf_data.connFracStatistics.empty()) {
        this->reportFractureStatistics(perf_data.connFracStatistics, connections);
    }
}

template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::initWellStateMSWell(const std::vector<Well>& wells_ecl,
                                                    const WellState* prev_well_state)
{
    ...
    for (int w = 0; w < nw; ++w) {
        ...
        if (well_ecl.isMultiSegment()) {
            ...
            // we need to know for each segment, how many perforation it has and how many segments using it as
            // outlet_segment that is why I think we should use a well model to initialize the WellState here
                    ...
                    if (segment_index == -1) {
                        if (!connectionWithoutAssociatedSegmentsLocalErrorMessage) {
                            connectionWithoutAssociatedSegmentsLocalErrorMessage
                                = ""; // First well with error: initialize the string
                        }
            ...
            if (static_cast<int>(ws.perf_data.size()) != n_activeperf) {
                ...
                // Append to the existing error message for errors in further wells
                *distributedMSWellLocalErrorMessage
                    += fmt::format("Distributed multi-segment well {} detected on rank {}.\n"
                                   "The number of active perforations on rank {} is {}, but the total number of active "
                                   "perforations is {}.\n\n",
                                   well_ecl.name(),
            ...
            for (size_t perf = 0; perf < perf_data.size(); ++perf) {
                if (auto candidate = active_perf_index_local_to_global.find(perf);
                    ...
                    if (!initializePerforationMismatchLocalErrorMessage) {
                        initializePerforationMismatchLocalErrorMessage
                            = ""; // First well with error: initialize the string
                    }

                    // Append to the existing error message for errors in further wells
                    *initializePerforationMismatchLocalErrorMessage
                        += fmt::format("Error when initializing MS Well state of well {}, there is no active "
                                       "perforation index for the local index {}.\n",
                                       well_ecl.name(),
                                       perf);
                }
            }
            ...
            calculateSegmentRates(ws.parallel_info,
                                  segment_inlets,
            ...
            // for the segment pressure, the segment pressure is the same with the first perforation belongs to the
            // segment if there is no perforation associated with this segment, it uses the pressure from the outlet
            // segment which requres the ordering is successful Not sure what is the best way to handle the
            // initialization, hopefully, the bad initialization can be improved during the solveWellEq process
            {
...

    {
        ...
        if (!this->enableDistributedWells_ && distributedMSWellLocalErrorMessage) {
            *distributedMSWellLocalErrorMessage
                += "Either adjust the partitioning or set the flag --allow-distributed-wells to true.\n";
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::calculateSegmentRatesBeforeSum(
    ...
    std::vector<Scalar>& segment_rates)
{
    ...
    for (const int& inlet_seg : segment_inlets[segment]) {
        calculateSegmentRatesBeforeSum(
            pw_info, segment_inlets, segment_perforations, perforation_rates, np, inlet_seg, segment_rates);
        for (int p = 0; p < np; ++p) {
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::calculateSegmentRates(const ParallelWellInfo<Scalar>& pw_info,
                                                      const std::vector<std::vector<int>>& segment_inlets,
                                                      const std::vector<std::vector<int>>& segment_perforations,
                                                      const std::vector<Scalar>& perforation_rates,
                                                      const int np,
...
{
    calculateSegmentRatesBeforeSum(
        pw_info, segment_inlets, segment_perforations, perforation_rates, np, segment, segment_rates);
    pw_info.communication().sum(segment_rates.data(), segment_rates.size());
}

template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::stopWell(int well_index)
{
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::openWell(int well_index)
{
    auto& ws = this->well(well_index);
    ws.open();
}
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::shutWell(int well_index)
{
    auto& ws = this->well(well_index);
    ws.shut();
}
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::updateStatus(int well_index, WellStatus status)
{
    auto& ws = this->well(well_index);
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::communicateGroupRates(const Parallel::Communication& comm)
{
    // Compute the size of the data.
    std::size_t sz
        = std::accumulate(this->well_rates.begin(),
                          this->well_rates.end(),
                          std::size_t {0},
                          [](const std::size_t acc, const auto& rates) { return acc + rates.second.second.size(); });

    // Make a vector and collect all data into it.
    std::vector<Scalar> data(sz);
    auto pos = data.begin();
    std::for_each(this->well_rates.begin(), this->well_rates.end(), [&pos](const auto& input) {
    ...
    pos = data.begin();
    std::for_each(this->well_rates.begin(), this->well_rates.end(), [&pos](auto& input) {
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::updateGlobalIsGrup(const Parallel::Communication& comm,
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::updateEfficiencyScalingFactor(const std::string& wellName, const Scalar value)
{
...
template <typename Scalar, typename IndexTraits>
data::Segment
WellState<Scalar, IndexTraits>::reportSegmentResults(const int well_id, const int seg_ix, const int seg_no) const
{
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::reportConnectionFactors(const std::size_t well_index,
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::reportConnectionPressuresAndRates(const std::size_t well_index,
                                                                  std::vector<data::Connection>& connections) const
{
    using rt = data::Rates::opt;
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::reportConnectionFilterCake(const std::size_t well_index,
                                                           std::vector<data::Connection>& connections) const
{
    const auto& perf_data = this->well(well_index).perf_data;
    const auto num_perf_well = perf_data.size();
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::reportFractureStatistics(const std::vector<ConnFracStatistics<Scalar>>& stats,
                                                         std::vector<data::Connection>& connections) const
{
...
template <typename Scalar, typename IndexTraits>
bool
WellState<Scalar, IndexTraits>::wellIsOwned(std::size_t well_index, [[maybe_unused]] const std::string& wellName) const
{
...
template <typename Scalar, typename IndexTraits>
bool
WellState<Scalar, IndexTraits>::wellIsOwned(const std::string& wellName) const
{
    const auto& well_index = this->index(wellName);
    if (!well_index.has_value()) {
        OPM_THROW(std::logic_error, fmt::format("Could not find well {} in well map", wellName));
    }
...
template <typename Scalar, typename IndexTraits>
void
WellState<Scalar, IndexTraits>::updateWellsDefaultALQ(const Schedule& schedule,
                                                      const int report_step,
                                                      const SummaryState& summary_state)
{
...
template <typename Scalar, typename IndexTraits>
bool
WellState<Scalar, IndexTraits>::operator==(const WellState& rhs) const
{
    return this->well_rates == rhs.well_rates && this->wells_ == rhs.wells_
