...
bool
WellBhpThpCalculator<Scalar, IndexTraits>::wellHasTHPConstraints(const SummaryState& summaryState) const
{
...
template <typename Scalar, typename IndexTraits>
Scalar
WellBhpThpCalculator<Scalar, IndexTraits>::getTHPConstraint(const SummaryState& summaryState) const
{
    const auto& well_ecl = well_.wellEcl();
    if (well_ecl.isInjector()) {
        const auto& controls = well_ecl.injectionControls(summaryState);
...
template <typename Scalar, typename IndexTraits>
Scalar
WellBhpThpCalculator<Scalar, IndexTraits>::mostStrictBhpFromBhpLimits(const SummaryState& summaryState) const
{
    const auto& well_ecl = well_.wellEcl();
    if (well_ecl.isInjector()) {
        const auto& controls = well_ecl.injectionControls(summaryState);
...
template <typename Scalar, typename IndexTraits>
Scalar
WellBhpThpCalculator<Scalar, IndexTraits>::calculateThpFromBhp(const std::vector<Scalar>& rates,
                                                               const Scalar bhp,
                                                               const Scalar rho,
                                                               ...
                                                               DeferredLogger& deferred_logger) const
{
    ...
    if (well_.isInjector()) {
        ...
        const Scalar dp
            = wellhelpers::computeHydrostaticCorrection(well_.refDepth(), vfp_ref_depth, rho, well_.gravity());
        auto thp_func = [this, table_id, aqua, liquid, vapour, dp](const Scalar bhp_value, const Scalar pressure_loss) {
        ...
        auto thp_func = [this, table_id, aqua, liquid, vapour, dp, &alq, wfr, gfr, use_vfpexp](
...
template <typename Scalar, typename IndexTraits>
Scalar
WellBhpThpCalculator<Scalar, IndexTraits>::findThpFromBhpIteratively(
    const std::function<Scalar(const Scalar, const Scalar)>& thp_func,
    const Scalar bhp,
    const Scalar thp_limit,
    const Scalar dp,
    DeferredLogger& deferred_logger) const
{
    ...
    while (true) {
        ...
        if (extraThpFromBhpOutput) {
            const std::string msg = fmt::format("findThpFromBhpIteratively(): iteration {}, thp = {}, bhp = {}, "
                                                "pressure_loss = {}, error = {}",
                                                it,
                                                thp,
                                                bhp + dp - pressure_loss,
                                                pressure_loss,
                                                error);
...
template <typename Scalar, typename IndexTraits>
std::optional<Scalar>
WellBhpThpCalculator<Scalar, IndexTraits>::computeBhpAtThpLimitProd(
    const std::function<std::vector<Scalar>(const Scalar)>& frates,
    ...
    DeferredLogger& deferred_logger) const
{
    ...
    if (!bhp_max.has_value()) {
        deferred_logger.debug("FAILED_ROBUST_BHP_THP_SOLVE_INOPERABLE",
                              ...
                              "find bhp-point where production becomes non-zero for well "
                                  + well_.name());
        return std::nullopt;
    }
...
template <typename Scalar, typename IndexTraits>
std::optional<Scalar>
WellBhpThpCalculator<Scalar, IndexTraits>::computeBhpAtThpLimitInj(
    const std::function<std::vector<Scalar>(const Scalar)>& frates,
    const SummaryState& summary_state,
    const Scalar rho,
...
{
    if (throwOnError) {
        return computeBhpAtThpLimitInjImpl<ThrowOnError>(
            frates, summary_state, rho, flo_rel_tol, max_iteration, deferred_logger);
    } else {
        return computeBhpAtThpLimitInjImpl<WarnAndContinueOnError>(
            frates, summary_state, rho, flo_rel_tol, max_iteration, deferred_logger);
    }
}
...
template <typename Scalar, typename IndexTraits>
void
WellBhpThpCalculator<Scalar, IndexTraits>::updateThp(const Scalar rho,
...
EvalWell
WellBhpThpCalculator<Scalar, IndexTraits>::calculateBhpFromThp(const WellState<Scalar, IndexTraits>& well_state,
                                                               ...
                                                               DeferredLogger& deferred_logger) const
{
    ...
    if (well_.isInjector()) {
        ...
        bhp_tab = well_.vfpProperties()->getInj()->bhp(controls.vfp_table_number, aqua, liquid, vapour, thp_limit);
    } else if (well_.isProducer()) {
...
template <typename Scalar, typename IndexTraits>
Scalar
WellBhpThpCalculator<Scalar, IndexTraits>::calculateMinimumBhpFromThp(const WellState<Scalar, IndexTraits>& well_state,
                                                                      const Well& well,
                                                                      const SummaryState& summaryState,
                                                                      const Scalar rho) const
{
    ...
    const Scalar bhp_min = well_.vfpProperties()->getProd()->minimumBHP(
        controls.vfp_table_number, thp_limit, wfr, gfr, well_.getALQ(well_state));
...
template <typename Scalar, typename IndexTraits>
Scalar
WellBhpThpCalculator<Scalar, IndexTraits>::getVfpBhpAdjustment(const Scalar bhp_tab, const Scalar thp_limit) const
{
...
std::optional<Scalar>
WellBhpThpCalculator<Scalar, IndexTraits>::computeBhpAtThpLimitInjImpl(
    const std::function<std::vector<Scalar>(const Scalar)>& frates,
    const SummaryState& summary_state,
    const Scalar rho,
    const Scalar flo_rel_tol,
    ...
    DeferredLogger& deferred_logger) const
{
    ...
    auto fbhp = [this, &controls, thp_limit, dp](const std::vector<Scalar>& rates) {
        assert(rates.size() == 3);
        const auto bhp = well_.vfpProperties()->getInj()->bhp(
            controls.vfp_table_number, rates[Water], rates[Oil], rates[Gas], thp_limit);
        return bhp - dp + getVfpBhpAdjustment(bhp, thp_limit);
    };
    ...
    for (Scalar flo_sample : flo_samples) {
        ...
        auto eq = [&flo, &frates, flo_sample](Scalar bhp) { return flo(frates(bhp)) - flo_sample; };
        ...
        try {
            int iteration = 0;
            const Scalar solved_bhp
                = RegulaFalsiBisection<ErrorPolicy>::solve(eq, low, high, max_iteration, flo_tolerance, iteration);
            ...
            deferred_logger.debug("FAILED_ROBUST_BHP_THP_SOLVE_EXTRACT_SAMPLES",
                                  "Robust bhp(thp) solve failed extracting bhp values at flo samples for well "
                                      + well_.name());
        }
    }
    ...
    auto eq = [&fbhp, &frates](Scalar bhp) { return fbhp(frates(bhp)) - bhp; };
    // TODO: replace hardcoded low/high limits.
    ...
    if (low == high) {
        ...
        deferred_logger.debug("FAILED_ROBUST_BHP_THP_SOLVE", "Robust bhp(thp) solve failed for well " + well_.name());
        return std::nullopt;
    }
    try {
        int iteration = 0;
        const Scalar solved_bhp
            = RegulaFalsiBisection<ErrorPolicy>::solve(eq, low, high, max_iteration, bhp_tolerance, iteration);
        if constexpr (extraBhpAtThpLimitOutput) {
...
template <typename Scalar, typename IndexTraits>
std::optional<Scalar>
WellBhpThpCalculator<Scalar, IndexTraits>::bhpMax(const std::function<Scalar(const Scalar)>& fflo,
                                                  ...
                                                  DeferredLogger& deferred_logger) const
{
    ...
    if constexpr (extraBhpAtThpLimitOutput) {
        deferred_logger.debug("computeBhpAtThpLimitProd(): well = " + well_.name() + "  low = " + std::to_string(low)
                              + "  high = " + std::to_string(high) + "  f(low) = " + std::to_string(f_low)
    ...
    if constexpr (extraBhpAtThpLimitOutput) {
        deferred_logger.debug("computeBhpAtThpLimitProd(): well = " + well_.name() + "  low = " + std::to_string(low)
                              + "  high = " + std::to_string(high) + "  f(low) = " + std::to_string(f_low)
                              + "  f(high) = " + std::to_string(f_high) + "  bhp_max = " + std::to_string(bhp_max));
    }
    return bhp_max;
}
...
template <typename Scalar, typename IndexTraits>
std::optional<Scalar>
WellBhpThpCalculator<Scalar, IndexTraits>::computeBhpAtThpLimit(
    const std::function<std::vector<Scalar>(const Scalar)>& frates,
    ...
    DeferredLogger& deferred_logger) const
{
    ...
    if (!finding_bracket) {
        deferred_logger.debug("FAILED_ROBUST_BHP_THP_SOLVE_INOPERABLE",
                              "Robust bhp(thp) solve failed due to not being able to "
                              "bracket the bhp solution with the brute force search for "
                                  + well_.name());
        return std::nullopt;
    }
    ...
    try {
        int iteration = 0;
        const Scalar solved_bhp
            = RegulaFalsiBisection<ThrowOnError>::solve(eq, low, high, max_iteration, bhp_tolerance, iteration);
        return solved_bhp;
    } catch (...) {
        deferred_logger.debug("FAILED_ROBUST_BHP_THP_SOLVE", "Robust bhp(thp) solve failed for well " + well_.name());
        return std::nullopt;
...
template <typename Scalar, typename IndexTraits>
bool
WellBhpThpCalculator<Scalar, IndexTraits>::bisectBracket(const std::function<Scalar(const Scalar)>& eq,
                                                         const std::array<Scalar, 2>& range,
                                                         Scalar& low,
                                                         Scalar& high,
                                                         std::optional<Scalar>& approximate_solution,
                                                         DeferredLogger& deferred_logger) const
{
    ...
    if constexpr (extraBhpAtThpLimitOutput) {
        deferred_logger.debug("computeBhpAtThpLimitProd(): well = " + well_.name() + "  low = " + std::to_string(low)
                              + "  high = " + std::to_string(high) + "  eq(low) = " + std::to_string(eq_low)
...
template <typename Scalar, typename IndexTraits>
bool
WellBhpThpCalculator<Scalar, IndexTraits>::bruteForceBracket(const std::function<Scalar(const Scalar)>& eq,
                                                             const std::array<Scalar, 2>& range,
                                                             Scalar& low,
                                                             Scalar& high,
                                                             DeferredLogger& deferred_logger)
{
    ...
    if (bracket_found) {
        deferred_logger.debug(" brute force solve found low " + std::to_string(low) + " with eq_low "
                              + std::to_string(eq_low) + " high " + std::to_string(high) + " with eq_high "
                              + std::to_string(eq_high));
    }
...
template <typename Scalar, typename IndexTraits>
bool
WellBhpThpCalculator<Scalar, IndexTraits>::isStableSolution(const WellState<Scalar, IndexTraits>& well_state,
                                                            const Well& well,
                                                            ...
                                                            const SummaryState& summaryState) const
{
    ...
    auto bhp = VFPHelpers<double>::bhp(
        table, aqua, liquid, vapour, thp, well_.getALQ(well_state), wfr, gfr, use_vfpexplicit);
...
template <typename Scalar, typename IndexTraits>
std::optional<Scalar>
WellBhpThpCalculator<Scalar, IndexTraits>::estimateStableBhp(const WellState<Scalar, IndexTraits>& well_state,
                                                             const Well& well,
                                                             const std::vector<Scalar>& rates,
                                                             const Scalar rho,
                                                             const SummaryState& summaryState) const
{
    ...
    const Scalar dp_hydro
        = wellhelpers::computeHydrostaticCorrection(well_.refDepth(), vfp_ref_depth, rho, well_.gravity());
    auto bhp_adjusted
        = [this, &thp, &dp_hydro](const Scalar bhp) { return bhp - dp_hydro + getVfpBhpAdjustment(bhp, thp); };
    const auto retval = VFPHelpers<double>::intersectWithIPR(
        table, thp, wfr, gfr, well_.getALQ(well_state), ipr.first, ipr.second, bhp_adjusted);
    if (retval.has_value()) {
...
WellBhpThpCalculator<Scalar, IndexTraits>::getFloIPR(const WellState<Scalar, IndexTraits>& well_state,
                                                     const Well& well,
                                                     const SummaryState& summary_state) const
{
    ...
    const Scalar& aqua_a = pu.phaseIsActive(IndexTraits::waterPhaseIdx)
        ? ipr_a[pu.canonicalToActivePhaseIdx(IndexTraits::waterPhaseIdx)]
        : 0.0;
    const Scalar& liquid_a = pu.phaseIsActive(IndexTraits::oilPhaseIdx)
        ? ipr_a[pu.canonicalToActivePhaseIdx(IndexTraits::oilPhaseIdx)]
        : 0.0;
    const Scalar& vapour_a = pu.phaseIsActive(IndexTraits::gasPhaseIdx)
        ? ipr_a[pu.canonicalToActivePhaseIdx(IndexTraits::gasPhaseIdx)]
        : 0.0;
    ...
    const Scalar& aqua_b = pu.phaseIsActive(IndexTraits::waterPhaseIdx)
        ? ipr_b[pu.canonicalToActivePhaseIdx(IndexTraits::waterPhaseIdx)]
        : 0.0;
    const Scalar& liquid_b = pu.phaseIsActive(IndexTraits::oilPhaseIdx)
        ? ipr_b[pu.canonicalToActivePhaseIdx(IndexTraits::oilPhaseIdx)]
        : 0.0;
    const Scalar& vapour_b = pu.phaseIsActive(IndexTraits::gasPhaseIdx)
        ? ipr_b[pu.canonicalToActivePhaseIdx(IndexTraits::gasPhaseIdx)]
        : 0.0;
...
template <typename Scalar, typename IndexTraits>
bool
WellBhpThpCalculator<Scalar, IndexTraits>::bruteForceBracketCommonTHP(const std::function<Scalar(const Scalar)>& eq,
                                                                      const std::array<Scalar, 2>& range,
                                                                      Scalar& low,
                                                                      Scalar& high,
                                                                      std::optional<Scalar>& approximate_solution,
...
#define INSTANTIATE(T, ...)                                                                                            \
    template __VA_ARGS__ WellBhpThpCalculator<T, BlackOilDefaultFluidSystemIndices>::calculateBhpFromThp(              \
