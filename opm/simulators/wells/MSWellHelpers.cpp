...
ValueType
haalandFormular(const ValueType& re, const Scalar diameter, const Scalar roughness)
{
...
template <typename ValueType, typename Scalar>
ValueType
WIOEmulsionViscosity(const ValueType& oil_viscosity,
...
template <typename ValueType, typename Scalar>
ValueType
OIWEmulsionViscosity(const ValueType& water_viscosity,
                     const ValueType& water_liquid_fraction,
                     const Scalar max_visco_ratio)
{
...
ParallellMSWellB<MatrixType>::ParallellMSWellB(const MatrixType& B, const ParallelWellInfo<Scalar>& parallel_well_info)
    : B_(B)
...
void
ParallellMSWellB<MatrixType>::mv(const X& x, Y& y) const
{
...
template <class X, class Y>
void
ParallellMSWellB<MatrixType>::mmv(const X& x, Y& y) const
{
    if (this->parallel_well_info_.communication().size() == 1) {
...
template <typename ValueType, typename Scalar>
ValueType
frictionPressureLoss(const Scalar l,
...
template <typename ValueType, typename Scalar>
ValueType
valveContrictionPressureLoss(const ValueType& mass_rate,
                             const ValueType& density,
...
template <typename ValueType, typename Scalar>
ValueType
velocityHead(const Scalar area, const ValueType& mass_rate, const ValueType& density)
{
...
template <typename ValueType, typename Scalar>
ValueType
emulsionViscosity(const ValueType& water_fraction,
                  ...
                  const SICD& sicd)
{
    ...
    if (water_liquid_fraction <= transition_start_value) {
        ...
        const ValueType viscosity_start_transition
            = WIOEmulsionViscosity(oil_viscosity, transition_start_value, max_visco_ratio);
        const ValueType viscosity_end_transition
            = OIWEmulsionViscosity(water_viscosity, transition_end_value, max_visco_ratio);
        const ValueType emulsion_viscosity
            = (viscosity_start_transition * (transition_end_value - water_liquid_fraction)
               + viscosity_end_transition * (water_liquid_fraction - transition_start_value))
            / width_transition;
...
#define INSTANTIATE_UMF(T, Dim)                                                                                        \
    template Vec<T, Dim> applyUMFPack(Dune::UMFPack<Mat<T, Dim>>&, Vec<T, Dim>);                                       \
    template Dune::Matrix<typename Mat<T, Dim>::block_type> invertWithUMFPack<Vec<T, Dim>, Mat<T, Dim>>(               \
        const int, const int, Dune::UMFPack<Mat<T, Dim>>&);

#define INSTANTIATE_IMPL(T, ...)                                                                                       \
    template __VA_ARGS__ frictionPressureLoss(                                                                         \
        const T, const T, const T, const T, const __VA_ARGS__&, const __VA_ARGS__&, const __VA_ARGS__&);               \
    template __VA_ARGS__ valveContrictionPressureLoss(                                                                 \
        const __VA_ARGS__& mass_rate, const __VA_ARGS__& density, const T, const T);                                   \
    template __VA_ARGS__ velocityHead(const T, const __VA_ARGS__&, const __VA_ARGS__&);                                \
    template __VA_ARGS__ emulsionViscosity<__VA_ARGS__, T>(                                                            \
        const __VA_ARGS__&, const __VA_ARGS__&, const __VA_ARGS__&, const __VA_ARGS__&, const SICD&);

#define INSTANTIATE_EVAL(T, Dim) INSTANTIATE_IMPL(T, DenseAd::Evaluation<T, Dim>)
