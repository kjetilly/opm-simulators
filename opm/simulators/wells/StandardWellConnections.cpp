...
void
reapportionGasOilMixture(std::string_view well,
                         ...
                         Opm::DeferredLogger& deferred_logger)
{
    ...
    Proceeding as if no dissolution or vaporisation for this connection)",
        ...
        if (rs > Scalar {0}) {
            ...
            phaseMixture[gaspos] = (componentMixture[gaspos] - componentMixture[oilpos] * rs) / denom;
        }

        if (rv > Scalar {0}) {
            // Subtract oil in gas from oil mixture.
            phaseMixture[oilpos] = (componentMixture[oilpos] - componentMixture[gaspos] * rv) / denom;
        }
    }
}
...
template <typename Scalar, typename Properties>
void
reapportionGasWaterMixture(std::string_view well,
                           const typename std::vector<Scalar>::size_type gaspos,
                           ...
                           Opm::DeferredLogger& deferred_logger)
{
    ...
    Proceeding as if no dissolution or vaporisation for this connection)",
        ...
        if (rsw > Scalar {0}) {
            ...
            phaseMixture[gaspos] = (componentMixture[gaspos] - componentMixture[waterpos] * rsw) / d;
        }

        if (rvw > Scalar {0}) {
            // Subtract water in gas from water mixture
            phaseMixture[waterpos] = (componentMixture[waterpos] - componentMixture[gaspos] * rvw) / d;
        }
    }
}
...
StandardWellConnections<FluidSystem, Indices>::StandardWellConnections(
...
template <typename FluidSystem, typename Indices>
void
StandardWellConnections<FluidSystem, Indices>::computePressureDelta()
{
    // Algorithm:
...
template <typename FluidSystem, typename Indices>
void
StandardWellConnections<FluidSystem, Indices>::computeDensities(const std::vector<Scalar>& perfComponentRates,
                                                                const Properties& props,
                                                                DeferredLogger& deferred_logger)
{
    ...
    const auto gaspos = activeGas ? static_cast<Ix>(FluidSystem::canonicalToActiveCompIdx(FluidSystem::gasCompIdx))
                                  : static_cast<Ix>(-1);

    const auto oilpos = activeOil ? static_cast<Ix>(FluidSystem::canonicalToActiveCompIdx(FluidSystem::oilCompIdx))
                                  : static_cast<Ix>(-1);
    ...
    for (int perf = 0; perf < nperf; ++perf) {
        this->initialiseConnectionMixture(num_quantities, perf, q_out_perf, phaseMixture, componentMixture);
        ...
        if (activeGas && activeOil) {
            reapportionGasOilMixture(
                this->well_.name(), gaspos, oilpos, perf, props, componentMixture, phaseMixture, deferred_logger);
        }

        if (activeGas && activeWater) {
            reapportionGasWaterMixture(
                this->well_.name(), gaspos, waterpos, perf, props, componentMixture, phaseMixture, deferred_logger);
        }
...
StandardWellConnections<FluidSystem, Indices>::calculatePerforationOutflow(
    const std::vector<Scalar>& perfComponentRates) const
{
    const int nperf = this->well_.numLocalPerfs();
    const int num_quantities = this->well_.numConservationQuantities();
    ...
    // level.
    const auto& factory = this->well_.parallelWellInfo().getGlobalPerfContainerFactory();

    auto global_q_out_perf = factory.createGlobal(q_out_perf, num_quantities);

    const auto global_perf_comp_rates = factory.createGlobal(perfComponentRates, num_quantities);
    ...
    for (int perf = factory.numGlobalPerfs() - 1; perf >= 0; --perf) {
        for (int component = 0; component < num_quantities; ++component) {
            ...
            // component rate.
            q_out = (perf == factory.numGlobalPerfs() - 1) ? Scalar {0} : global_q_out_perf[index + num_quantities];
...
template <typename FluidSystem, typename Indices>
void
StandardWellConnections<FluidSystem, Indices>::initialiseConnectionMixture(const int num_quantities,
                                                                           const int perf,
...
{
    // Find component mix.
    const auto tot_surf_rate = std::accumulate(
        q_out_perf.begin() + num_quantities * (perf + 0), q_out_perf.begin() + num_quantities * (perf + 1), Scalar {0});
...
template <typename FluidSystem, typename Indices>
void
StandardWellConnections<FluidSystem, Indices>::computeDensitiesForStoppedProducer(
...
StandardWellConnections<FluidSystem, Indices>::computePropertiesForPressures(
    const WellState<Scalar, IndexTraits>& well_state, const PressurePropertyFunctions& prop_func) const
{
    ...
    const auto p_above = this->well_.parallelWellInfo().communicateAboveValues(ws.bhp, perf_press.data(), nperf);
    ...
    for (int perf = 0; perf < nperf; ++perf) {
        ...
        if (waterPresent) {
            ...
            if (FluidSystem::enableDissolvedGasInWater()) {
                ...
                props.rswmax_perf[perf] = FluidSystem::waterPvt().saturatedGasDissolutionFactor(
                    region_idx, temperature, p_avg, saltConcentration);
                if (waterrate > 0) {
                    const int gas_pos = FluidSystem::canonicalToActivePhaseIdx(FluidSystem::gasPhaseIdx);
                    const Scalar gasrate
                        = std::abs(ws.surface_rates[gas_pos]) - (Indices::enableSolvent ? ws.sum_solvent_rates() : 0.0);
                    if (gasrate > 0) {
            ...
            props.b_perf[waterCompIdx + perf * well_.numConservationQuantities()]
                = FluidSystem::waterPvt().inverseFormationVolumeFactor(
                    region_idx, temperature, p_avg, rsw, saltConcentration);
        }
        ...
        if (gasPresent) {
            ...
            if (oilPresent) {
                ...
                props.rvmax_perf[perf]
                    = FluidSystem::gasPvt().saturatedOilVaporizationFactor(region_idx, temperature, p_avg);
            ...
            if (waterPresent) {
                ...
                const int water_pos = FluidSystem::canonicalToActivePhaseIdx(FluidSystem::waterPhaseIdx);
                const Scalar waterrate = std::abs(ws.surface_rates[water_pos]);
                props.rvwmax_perf[perf]
                    = FluidSystem::gasPvt().saturatedWaterVaporizationFactor(region_idx, temperature, p_avg);
            ...
            props.b_perf[gaspos]
                = FluidSystem::gasPvt().inverseFormationVolumeFactor(region_idx, temperature, p_avg, rv, rvw);
        }
        ...
        if (oilPresent) {
            ...
            Scalar rs = 0.0;
            if (gasPresent) {
                props.rsmax_perf[perf]
                    = FluidSystem::oilPvt().saturatedGasDissolutionFactor(region_idx, temperature, p_avg);
            ...
            props.b_perf[oilpos]
                = FluidSystem::oilPvt().inverseFormationVolumeFactor(region_idx, temperature, p_avg, rs);
        }
...
template <typename FluidSystem, typename Indices>
std::vector<typename FluidSystem::Scalar>
StandardWellConnections<FluidSystem, Indices>::copyInPerforationRates(const Properties& props,
                                                                      const PerfData<Scalar>& perf_data) const
{
    ...
    if constexpr (Indices::enableSolvent) {
        for (int perf = 0, ix_s = 0 * nc + Indices::contiSolventEqIdx; perf < nperf; ++perf, ix_s += nc) {
...
template <typename FluidSystem, typename Indices>
void
StandardWellConnections<FluidSystem, Indices>::computeProperties(const bool stopped_or_zero_rate_target,
                                                                 ...
                                                                 DeferredLogger& deferred_logger)
{
    ...
    if (stopped_or_zero_rate_target && this->well_.isProducer()) {
        ...
        const auto perfRates
            = this->copyInPerforationRates(props, well_state.well(this->well_.indexOfWell()).perf_data);
...
StandardWellConnections<FluidSystem, Indices>::connectionRateBrine(
    Scalar& rate,
...
template <typename FluidSystem, typename Indices>
typename StandardWellConnections<FluidSystem, Indices>::Eval
StandardWellConnections<FluidSystem, Indices>::connectionRateFoam(
    const std::vector<EvalWell>& cq_s,
...
typename StandardWellConnections<FluidSystem, Indices>::Eval
StandardWellConnections<FluidSystem, Indices>::connectionRateBioeffects(
    Scalar& rate,
    const Scalar vap_wat_rate,
    const std::vector<EvalWell>& cq_s,
...
std::tuple<typename StandardWellConnections<FluidSystem, Indices>::Eval,
           typename StandardWellConnections<FluidSystem, Indices>::Eval,
           typename StandardWellConnections<FluidSystem, Indices>::Eval>
StandardWellConnections<FluidSystem, Indices>::connectionRatesMICP(
    Scalar& rate_m,
    Scalar& rate_o,
    Scalar& rate_u,
    const std::vector<EvalWell>& cq_s,
    ...
    const std::variant<Scalar, EvalWell>& ureaConcentration) const
{
    ...
    return {well_.restrictEval(cq_s_microbe), well_.restrictEval(cq_s_oxygen), well_.restrictEval(cq_s_urea)};
}

template <typename FluidSystem, typename Indices>
std::tuple<typename StandardWellConnections<FluidSystem, Indices>::Eval,
           typename StandardWellConnections<FluidSystem, Indices>::EvalWell>
StandardWellConnections<FluidSystem, Indices>::connectionRatePolymer(
    Scalar& rate, const std::vector<EvalWell>& cq_s, const std::variant<Scalar, EvalWell>& polymerConcentration) const
{
    // TODO: the application of well efficiency factor has not been tested with an example yet
    const unsigned waterCompIdx = FluidSystem::canonicalToActiveCompIdx(FluidSystem::waterCompIdx);
...
std::tuple<typename StandardWellConnections<FluidSystem, Indices>::Eval,
           typename StandardWellConnections<FluidSystem, Indices>::EvalWell>
StandardWellConnections<FluidSystem, Indices>::connectionRatezFraction(
    Scalar& rate,
