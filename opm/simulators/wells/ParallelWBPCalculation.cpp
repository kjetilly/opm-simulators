...
Opm::ParallelWBPCalculation<Scalar>::LocalConnSet::LocalConnSet(const std::vector<int>& localConnIdx)
...
template <class Scalar>
int
Opm::ParallelWBPCalculation<Scalar>::LocalConnSet::localIndex(const std::size_t connIdx) const
{
    return (connIdx >= this->localConnIdx_.size()) ? -1 : this->localConnIdx_[connIdx];
}

// ---------------------------------------------------------------------------

template <class Scalar>
Opm::ParallelWBPCalculation<Scalar>::SourceData::SourceData(const Parallel::Communication& comm)
    : comm_ {comm}
{
}
...
Opm::ParallelWBPCalculation<Scalar>::SourceData::localIndex(GlobalToLocal localIdx)
{
...
template <class Scalar>
typename Opm::ParallelWBPCalculation<Scalar>::SourceData&
Opm::ParallelWBPCalculation<Scalar>::SourceData::evaluator(Evaluator eval)
{
...
template <class Scalar>
typename Opm::ParallelWBPCalculation<Scalar>::SourceData&
Opm::ParallelWBPCalculation<Scalar>::SourceData::evaluatorFactory(EvaluatorFactory evalFactory)
{
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::SourceData::buildStructure(const std::vector<std::size_t>& sourceLocations)
{
    if (this->srcData_ == nullptr) {
        this->srcData_
            = std::make_unique<ParallelPAvgDynamicSourceData<Scalar>>(this->comm_, sourceLocations, this->localIdx_);
    } else {
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::SourceData::collectDynamicValues()
{
    if (this->srcData_ == nullptr) {
        throw std::logic_error {"Cannot collect dynamic WBP source values "
...
template <class Scalar>
std::vector<int>
Opm::ParallelWBPCalculation<Scalar>::SourceData::getLocalIndex(const std::vector<std::size_t>& globalIndex) const
{
    auto localIdx = std::vector<int>(globalIndex.size());

    std::transform(globalIndex.begin(), globalIndex.end(), localIdx.begin(), [this](const std::size_t globIx) {
...
template <class Scalar>
Opm::ParallelWBPCalculation<Scalar>::ParallelWBPCalculation(const GridDims& cellIndexMap,
...
Opm::ParallelWBPCalculation<Scalar>::localCellIndex(GlobalToLocal localCellIdx)
{
...
template <class Scalar>
Opm::ParallelWBPCalculation<Scalar>&
Opm::ParallelWBPCalculation<Scalar>::evalCellSource(Evaluator evalCellSrc)
{
...
template <class Scalar>
std::size_t
Opm::ParallelWBPCalculation<Scalar>::createCalculator(const Well& well,
                                                      ...
                                                      EvaluatorFactory makeWellSourceEvaluator)
{
    ...
    const auto ix = this->calculators_.setCalculator(
        well.seqIndex(),
        std::make_unique<ParallelPAvgCalculator<Scalar>>(
            parallelWellInfo.communication(), this->cellIndexMap_, well.getConnections()));
    ...
    this->wellConnSrc_[ix]
        ...
        .localIndex([ix = ix, this](const std::size_t connIdx) { return this->localConnSet_[ix].localIndex(connIdx); });
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::defineCommunication()
{
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::collectDynamicValues()
{
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::inferBlockAveragePressures(const std::size_t calcIndex,
                                                                const PAvg& controls,
                                                                const Scalar gravity,
                                                                const Scalar refDepth)
{
    this->calculators_[calcIndex].inferBlockAveragePressures(
        this->makeEvaluationSources(calcIndex), controls, gravity, refDepth);
}

template <class Scalar>
const Opm::PAvgCalculatorResult<Scalar>&
Opm::ParallelWBPCalculation<Scalar>::averagePressures(const std::size_t calcIndex) const
{
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::pruneInactiveWBPCells()
{
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::pruneInactiveWBPCellsSerial()
{
    this->calculators_.pruneInactiveWBPCells([this](const std::vector<std::size_t>& globalWBPCellIdx) {
        ...
        const auto localWBPCellIdx = this->reservoirSrc_.getLocalIndex(globalWBPCellIdx);
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::pruneInactiveWBPCellsParallel()
{
    this->calculators_.pruneInactiveWBPCells([this](const std::vector<std::size_t>& globalWBPCellIdx) {
        auto isActive = std::vector<bool>(globalWBPCellIdx.size(), false);

        // AllWBPCells possibly contains repeated indices.  That's okay here.
        const auto& [allWBPCells, rankStart] = allGatherv(globalWBPCellIdx, this->reservoirSrc_.comm());

        // Recall: localIndex() is negative if input is inactive or not on rank.
        auto localWBPCellIdx = this->reservoirSrc_.getLocalIndex(allWBPCells);

        // The WBP cell is active if it has a non-negative local index on
        // one of the ranks.
        this->reservoirSrc_.comm().max(localWBPCellIdx.data(), localWBPCellIdx.size());
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::defineReservoirCommunication()
{
    auto sourceCells = std::vector<std::size_t> {};

    std::tie(sourceCells, std::ignore) = allGatherv(this->calculators_.allWBPCells(), this->reservoirSrc_.comm());
...
template <class Scalar>
void
Opm::ParallelWBPCalculation<Scalar>::defineWellCommunication(const std::size_t well)
{
    this->wellConnSrc_[well].buildStructure(this->calculators_[well].allWellConnections());
}

template <class Scalar>
typename Opm::PAvgCalculator<Scalar>::Sources
Opm::ParallelWBPCalculation<Scalar>::makeEvaluationSources(const WellID well) const
{
    return
        typename PAvgCalculator<Scalar>::Sources {}.wellBlocks(this->reservoirSrc_).wellConns(this->wellConnSrc_[well]);
}
