...
BlackoilWellModelGeneric<Scalar, IndexTraits>::BlackoilWellModelGeneric(
    Schedule& schedule,
    ...
    , terminal_output_(comm_.rank() == 0 && Parameters::Get<Parameters::EnableTerminalOutput>())
    , guideRate_(schedule)
    , active_wgstate_(pu)
    ...
    , nupcol_wgstate_(pu)
{
    ...
    this->not_on_process_ = [this, numProcs](const std::string& well) {
        ...
        auto candidate = std::lower_bound(this->parallel_well_info_.begin(), this->parallel_well_info_.end(), value);

        return (candidate == this->parallel_well_info_.end()) || (*candidate != value);
    };
...
template <typename Scalar, typename IndexTraits>
int
BlackoilWellModelGeneric<Scalar, IndexTraits>::numLocalWells() const
{
...
template <typename Scalar, typename IndexTraits>
int
BlackoilWellModelGeneric<Scalar, IndexTraits>::numPhases() const
{
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::hasLocalWell(const std::string& wname) const
{
    return std::any_of(
        this->wells_ecl_.begin(), this->wells_ecl_.end(), [&wname](const Well& well) { return well.name() == wname; });
}

template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::hasOpenLocalWell(const std::string& wname) const
{
    return std::any_of(well_container_generic_.begin(),
                       well_container_generic_.end(),
                       [&wname](const auto* elem) -> bool { return elem->name() == wname; });
}

template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::wellsActive() const
{
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::networkActive() const
{
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::anyMSWellOpenLocal() const
{
    return std::any_of(wells_ecl_.begin(), wells_ecl_.end(), [](const auto& well) { return well.isMultiSegment(); });
}

template <typename Scalar, typename IndexTraits>
const Well&
BlackoilWellModelGeneric<Scalar, IndexTraits>::getWellEcl(const std::string& well_name) const
{
    // finding the iterator of the well in wells_ecl
    auto well_ecl = std::find_if(wells_ecl_.begin(), wells_ecl_.end(), [&well_name](const Well& elem) -> bool {
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::initFromRestartFile(const RestartValue& restartValues,
                                                                   ...
                                                                   bool enable_distributed_wells)
{
    ...
    BlackoilWellModelRestart(*this).loadRestartData(
        restartValues.wells, restartValues.grp_nwrk, handle_ms_well, this->wellState(), this->groupState());

    if (config.has_model()) {
        BlackoilWellModelRestart(*this).loadRestartGuideRates(
            report_step, config.model().target(), restartValues.wells, this->guideRate_);

        BlackoilWellModelRestart(*this).loadRestartGuideRates(
            report_step, config, restartValues.grp_nwrk.groupData, this->guideRate_);
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::prepareDeserialize(int report_step,
                                                                  const std::size_t numCells,
                                                                  bool handle_ms_well,
                                                                  bool enable_distributed_wells)
{
...
template <typename Scalar, typename IndexTraits>
std::vector<Well>
BlackoilWellModelGeneric<Scalar, IndexTraits>::getLocalWells(const int timeStepIdx) const
{
    auto w = std::vector<Well> {};

    auto wnames = this->schedule().wellNames(timeStepIdx);
    wnames.erase(std::remove_if(wnames.begin(), wnames.end(), this->not_on_process_), wnames.end());

    w.reserve(wnames.size());

    std::transform(wnames.begin(),
                   wnames.end(),
                   std::back_inserter(w),
                   [&st = this->schedule()[timeStepIdx]](const std::string& wname) { return st.wells(wname); });
...
BlackoilWellModelGeneric<Scalar, IndexTraits>::createLocalParallelWellInfo(const std::vector<Well>& wells)
{
    ...
    for (const auto& well : wells) {
        ...
        auto pwell = std::lower_bound(parallel_well_info_.begin(), parallel_well_info_.end(), wellPair);
        assert(pwell != parallel_well_info_.end() && *pwell == wellPair);
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::initializeWellProdIndCalculators()
{
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::initializeWellPerfData()
{
    ...
    for (const auto& well : wells_ecl_) {
        ...
        auto& connIdxMap = this->conn_idx_map_.emplace_back(well.getConnections().size());

        CheckDistributedWellConnections checker {well, this->local_parallel_well_info_[well_index].get()};
        ...
        for (const auto& connection : well.getConnections()) {
            ...
            if (connIsOpen) {
                if (active_index >= 0) {
                    ...
                    parallelWellInfo.pushBackEclIndex(connection_index_above, connection_index);
                }
                ...
                OPM_THROW(
                    std::runtime_error,
                    fmt::format("Connection state '{}' not handled", Connection::State2String(connection.state())));
            }
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::checkGEconLimits(const Group& group,
                                                                ...
                                                                DeferredLogger& deferred_logger)
{
    ...
    for (const std::string& group_name : group.groups()) {
        checkGEconLimits(
            schedule().getGroup(group_name, report_step_idx), simulation_time, report_step_idx, deferred_logger);
    }
    ...
    if (checker.endRun() && (checker.numProducersOpenInitially() >= 1) && (checker.numProducersOpen() == 0)) {
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::checkGconsaleLimits(const Group& group,
                                                                   ...
                                                                   DeferredLogger& deferred_logger)
{
    ...
    if (sales_rate > gconsale.max_sales_rate) {
        ...
        case GConSale::MaxProcedure::NONE: {
            if (oldProductionControl != Group::ProductionCMode::GRAT
                && oldProductionControl != Group::ProductionCMode::NONE) {
        ...
        case GConSale::MaxProcedure::CON: {
            OPM_DEFLOG_THROW(std::runtime_error,
                             "Group " + group.name() + "GCONSALE exceed limit CON not implemented",
                             deferred_logger);
            break;
        }
        case GConSale::MaxProcedure::CON_P: {
            OPM_DEFLOG_THROW(std::runtime_error,
                             "Group " + group.name() + "GCONSALE exceed limit CON_P not implemented",
                             deferred_logger);
            break;
        }
        case GConSale::MaxProcedure::WELL: {
            OPM_DEFLOG_THROW(std::runtime_error,
                             "Group " + group.name() + "GCONSALE exceed limit WELL not implemented",
                             deferred_logger);
            break;
        }
        case GConSale::MaxProcedure::PLUG: {
            OPM_DEFLOG_THROW(std::runtime_error,
                             "Group " + group.name() + "GCONSALE exceed limit PLUG not implemented",
                             deferred_logger);
            break;
        }
        case GConSale::MaxProcedure::MAXR: {
            OPM_DEFLOG_THROW(std::runtime_error,
                             "Group " + group.name() + "GCONSALE exceed limit MAXR not implemented",
                             deferred_logger);
            break;
        }
        case GConSale::MaxProcedure::END: {
            OPM_DEFLOG_THROW(std::runtime_error,
                             "Group " + group.name() + "GCONSALE exceed limit END not implemented",
                             deferred_logger);
            break;
        }
    ...
    if (sales_rate < gconsale.min_sales_rate) {
        ...
        if (currentProductionControl == Group::ProductionCMode::GRAT) {
            ...
            ss = fmt::format("Group {} has sale rate less then minimum permitted value but cannot increase the group "
                             "production rate \n"
    ...
    if (gconsale.sales_target < 0.0) {
        OPM_DEFLOG_THROW(std::runtime_error,
                         "Group " + group.name() + " has sale rate target less then zero. Not implemented in Flow",
                         deferred_logger);
    }
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::checkGroupHigherConstraints(const Group& group,
                                                                           DeferredLogger& deferred_logger,
                                                                           const int reportStepIdx,
                                                                           ...
                                                                           const bool update_group_switching_log)
{
    ...
    if (comm_.size() > 1) {
        ...
        int firstWellIndex = well_perf_data_.empty() ? std::numeric_limits<int>::max() : wells_ecl_[0].seqIndex();
        ...
        pvtreg = std::min_element(pairs.begin(), pairs.end(), [](const auto& p1, const auto& p2) {
    ...
    if (!isField && group.isInjectionGroup()) {
        ...
        for (Phase phase : all) {
                    ...
                    if (output_first_time) {
                        if (comm_.rank() == 0) {
                            ...
                            const std::string msg = fmt::format(
                                "Group control for {} injector group {} is oscillating. Group control kept at {}.",
                                std::move(os).str(),
                                group.name(),
            ...
            if (currentControl != Group::InjectionCMode::FLD && group.injectionGroupControlAvailable(phase)) {
                ...
                if (is_changed) {
                    auto& group_log
                        = switched_inj_groups_[group.name()][static_cast<std::underlying_type_t<Phase>>(phase)];
                    if (update_group_switching_log || group_log.empty()) {
                        group_log.push_back(currentControl);
                    }
                    BlackoilWellModelConstraints(*this).actionOnBrokenConstraints(
                        group, Group::InjectionCMode::FLD, phase, this->groupState(), deferred_logger);
    ...
    if (!isField && group.isProductionGroup()) {
        ...
        if (auto groupPos = switched_prod_groups_.find(group.name()); groupPos != switched_prod_groups_.end()) {
            ...
            if (group_is_oscillating) {
                ...
                if (output_first_time) {
                    if (comm_.rank() == 0) {
                        const std::string msg = fmt::format(
                            "Group control for production group {} is oscillating. Group control kept at {}.",
                            group.name(),
        ...
        if (currentControl != Group::ProductionCMode::FLD && group.productionGroupControlAvailable()) {
            ...
            if (is_changed) {
                ...
                changed = BlackoilWellModelConstraints(*this).actionOnBrokenConstraints(group,
                                                                                        reportStepIdx,
                                                                                        group_limit_action,
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateEclWells(const int timeStepIdx,
                                                              const SimulatorUpdate& sim_update,
                                                              const SummaryState& st)
{
    this->updateEclWellsConstraints(timeStepIdx, sim_update, st);

    if (!sim_update.well_structure_changed && !this->wellStructureChangedDynamically_) {
...
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::wellUpdateLoop(Iter first, Iter last, const int timeStepIdx, Body&& body)
{
    std::for_each(first, last, [this, timeStepIdx, loopBody = std::forward<Body>(body)](const auto& wname) {
        auto well_iter = std::find_if(this->wells_ecl_.begin(), this->wells_ecl_.end(), [&wname](const auto& well) {
        ...
        const auto wellIdx = std::distance(this->wells_ecl_.begin(), well_iter);

        const auto& well = this->wells_ecl_[wellIdx] = this->schedule_.getWell(wname, timeStepIdx);
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateEclWellsConstraints(const int timeStepIdx,
                                                                         const SimulatorUpdate& sim_update,
                                                                         const SummaryState& st)
{
    this->wellUpdateLoop(sim_update.affected_wells.begin(),
                         ...
                         timeStepIdx,
                         [this, &st](const auto wellIdx, const auto& well) {
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateEclWellsCTFFromAction(const int timeStepIdx,
                                                                           const SimulatorUpdate& sim_update)
{
...
template <typename Scalar, typename IndexTraits>
Scalar
BlackoilWellModelGeneric<Scalar, IndexTraits>::wellPI(const int well_index) const
{
    ...
    default:
        throw std::invalid_argument {"Unsupported preferred phase " + std::to_string(static_cast<int>(preferred))};
    }
}

template <typename Scalar, typename IndexTraits>
Scalar
BlackoilWellModelGeneric<Scalar, IndexTraits>::wellPI(const std::string& well_name) const
{
    auto well_iter = std::find_if(this->wells_ecl_.begin(), this->wells_ecl_.end(), [&well_name](const Well& well) {
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::wasDynamicallyShutThisTimeStep(const int well_index) const
{
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::wasDynamicallyShutThisTimeStep(const std::string& well_name) const
{
    return this->closed_this_step_.find(well_name) != this->closed_this_step_.end();
}

template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateWsolvent(const Group& group,
                                                              const int reportStepIdx,
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::setWsolvent(const Group& group, const int reportStepIdx, Scalar wsolvent)
{
    for (const std::string& groupName : group.groups()) {
        const Group& groupTmp = schedule_.getGroup(groupName, reportStepIdx);
...
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::loopOwnedWells(LoopBody&& loopBody) const
{
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignWellTargets(data::Wells& wsrpt) const
{
    this->loopOwnedWells([this, &wsrpt]([[maybe_unused]] const auto wellIndex, const Well& well) {
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignProductionWellTargets(const Well& well,
                                                                           data::WellControlLimits& limits) const
{
    using Item = data::WellControlLimits::Item;

    const auto ctrl = well.productionControls(this->summaryState());

    limits.set(Item::Bhp, ctrl.bhp_limit)
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignInjectionWellTargets(const Well& well,
                                                                          data::WellControlLimits& limits) const
{
    using Item = data::WellControlLimits::Item;
    ...
    limits.set(Item::Bhp, ctrl.bhp_limit).set(Item::ResVRate, ctrl.reservoir_rate);
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignDynamicWellStatus(data::Wells& wsrpt) const
{
    this->loopOwnedWells([this, &wsrpt](const auto wellID, const Well& well) {
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignShutConnections(data::Wells& wsrpt,
                                                                     const int reportStepIndex) const
{
    this->loopOwnedWells([this, reportStepIndex, &wsrpt]([[maybe_unused]] const auto wellID, const Well& well) {
        const auto wellIsOpen = this->schedule()[reportStepIndex].wells(well.name()).getStatus() == Well::Status::OPEN;

        auto skip
            = [wellIsOpen](const Connection& conn) { return wellIsOpen && (conn.state() != Connection::State::SHUT); };
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignGroupControl(const Group& group, data::GroupData& gdata) const
{
    ...
    cgc.currentGasInjectionConstraint = cgc.currentWaterInjectionConstraint = Group::InjectionCMode::NONE;
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignGroupValues(const int reportStepIdx,
                                                                 std::map<std::string, data::GroupData>& gvalues) const
{
    const auto groupGuideRates = BlackoilWellModelGuideRates(*this).calculateAllGroupGuideRates(reportStepIdx);
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignNodeValues(std::map<std::string, data::NodeData>& nodevalues,
...
BlackoilWellModelGeneric<Scalar, IndexTraits>::groupAndNetworkData(const int reportStepIdx) const
{
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateAndCommunicateGroupData(const int reportStepIdx,
                                                                             ...
                                                                             DeferredLogger& deferred_logger)
{
            ...
            for (Phase phase : all) {
                if (this->groupState().has_injection_control(gr_name, phase)) {
                    if (this->groupState().injection_control(gr_name, phase) == Group::InjectionCMode::VREP
                        ...
                        const bool is_vrep
                            = this->groupState().injection_control(gr_name, phase) == Group::InjectionCMode::VREP;
                        ...
                        Scalar rel_change
                            = denominator > small_rate ? std::abs((gr_rate_nupcol - gr_rate) / denominator) : 0.0;
                        if (rel_change > tol_nupcol) {
                            this->updateNupcolWGState();
                            const std::string control_str = is_vrep ? "VREP" : "REIN";
                            const std::string msg
                                = fmt::format("Group prodution relative change {} larger than tolerance {} "
                                              "at iteration {}. Update {} for Group {} even if iteration is larger "
                                              "than {} given by NUPCOL.",
                                              rel_change,
                                              tol_nupcol,
                                              iterationIdx,
                                              control_str,
    ...
    WellGroupHelpersType::updateREINForGroups(
        fieldGroup, schedule(), reportStepIdx, summaryState_, well_state_nupcol, this->groupState(), comm_.rank() == 0);
    WellGroupHelpersType::updateVREPForGroups(
        fieldGroup, schedule(), reportStepIdx, well_state_nupcol, this->groupState(), this->summaryState_);

    WellGroupHelpersType::updateReservoirRatesInjectionGroups(
        fieldGroup, schedule(), reportStepIdx, well_state_nupcol, this->groupState(), this->summaryState_);
    WellGroupHelpersType::updateSurfaceRatesInjectionGroups(
        fieldGroup, schedule(), reportStepIdx, well_state_nupcol, this->groupState(), this->summaryState_);
    WellGroupHelpersType::updateNetworkLeafNodeProductionRates(
        schedule(), reportStepIdx, well_state_nupcol, this->groupState(), this->summaryState_);

    WellGroupHelpersType::updateGroupProductionRates(
        fieldGroup, schedule(), reportStepIdx, well_state_nupcol, this->groupState(), this->summaryState_);

    WellGroupHelpersType::updateWellRates(fieldGroup, schedule(), reportStepIdx, well_state_nupcol, well_state);
    ...
    if (update_wellgrouptarget) {
        for (const auto& well : well_container_generic_) {
            ...
            calcResvCoeff(fipnum, pvtreg, this->groupState().production_rates(group.name()), resv_coeff);
            const Scalar efficiencyFactor = well->wellEcl().getEfficiencyFactor() * ws.efficiency_scaling_factor;
            // Translate injector type from control to Phase.
            Scalar group_target = std::numeric_limits<Scalar>::max();
            if (well->isProducer()) {
                group_target = WellGroupHelpersType::getWellGroupTargetProducer(well->name(),
                ...
                group_target = WellGroupHelpersType::getWellGroupTargetInjector(well->name(),
                                                                                well->wellEcl().groupName(),
                                                                                group,
                                                                                this->wellState(),
                                                                                this->groupState(),
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateNetworkActiveState(const int report_step)
{
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::needPreStepNetworkRebalance(const int report_step) const
{
    const auto& network = schedule()[report_step].network();
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::forceShutWellByName(const std::string& wellname,
                                                                   const double simulation_time,
                                                                   const bool dont_shut_grup_wells)
{
    ...
    if (terminal_output_ && well_was_shut) {
        const std::string msg = "Well " + wellname + " will be shut because it fails to converge.";
        OpmLog::info(msg);
    }
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::inferLocalShutWells()
{
...
template <typename Scalar, typename IndexTraits>
Scalar
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateNetworkPressures(const int reportStepIdx,
                                                                      ...
                                                                      const Scalar upper_update_bound)
{
    ...
    if (!previous_node_pressures.empty()) {
        for (const auto& [name, new_pressure] : node_pressures_) {
            ...
            // the maximum update is limited (to 5 bar by default, can be changed with
            // --network-max-pressure-update-in-bars).
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::calculateEfficiencyFactors(const int reportStepIdx)
{
    for (auto& well : well_container_generic_) {
        const Well& wellEcl = well->wellEcl();
        Scalar well_efficiency_factor
            = wellEcl.getEfficiencyFactor() * wellState().getGlobalEfficiencyScalingFactor(well->name());
        WellGroupHelpersType::accumulateGroupEfficiencyFactor(
            schedule().getGroup(wellEcl.groupName(), reportStepIdx), schedule(), reportStepIdx, well_efficiency_factor);
...
BlackoilWellModelGeneric<Scalar, IndexTraits>::getGenWell(const std::string& well_name)
{
    // finding the iterator of the well in wells_ecl
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::setRepRadiusPerfLength()
{
    for (const auto& well : well_container_generic_) {
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateWellPotentials(const int reportStepIdx,
                                                                    ...
                                                                    DeferredLogger& deferred_logger)
{
    ...
    for (const auto& well : well_container_generic_) {
        const bool needed_for_summary = ((summaryConfig.hasSummaryKey("WWPI:" + well->name())
                                          ...
                                          || summaryConfig.hasSummaryKey("WGPI:" + well->name()))
                                         && well->isInjector())
            || ((summaryConfig.hasKeyword("GWPI") || summaryConfig.hasKeyword("GOPI")
                 || summaryConfig.hasKeyword("GGPI"))
                && well->isInjector())
            || ((summaryConfig.hasKeyword("FWPI") || summaryConfig.hasKeyword("FOPI")
                 || summaryConfig.hasKeyword("FGPI"))
                && well->isInjector())
            || ((summaryConfig.hasSummaryKey("WWPP:" + well->name())
                 || summaryConfig.hasSummaryKey("WOPP:" + well->name())
                 || summaryConfig.hasSummaryKey("WGPP:" + well->name()))
                && well->isProducer())
            || ((summaryConfig.hasKeyword("GWPP") || summaryConfig.hasKeyword("GOPP")
                 || summaryConfig.hasKeyword("GGPP"))
                && well->isProducer())
            || ((summaryConfig.hasKeyword("FWPP") || summaryConfig.hasKeyword("FOPP")
                 || summaryConfig.hasKeyword("FGPP"))
                && well->isProducer());

        // At the moment, the following events are considered
        // for potentials update
        const uint64_t effective_events_mask = ScheduleEvents::WELL_STATUS_CHANGE + ScheduleEvents::COMPLETION_CHANGE
            + ScheduleEvents::WELL_PRODUCTIVITY_INDEX + ScheduleEvents::WELL_WELSPECS_UPDATE
            + ScheduleEvents::WELLGROUP_EFFICIENCY_UPDATE + ScheduleEvents::NEW_WELL + ScheduleEvents::PRODUCTION_UPDATE
    ...
    logAndCheckForProblemsAndThrow(
        deferred_logger, exc_type, "updateWellPotentials() failed: " + exc_msg, terminal_output_, comm_);
}

template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::runWellPIScaling(const int reportStepIdx,
                                                                DeferredLogger& local_deferredLogger)
{
    ...
    auto hasWellPIEvent = [this, reportStepIdx](const int well_index) -> bool {
        return this->schedule()[reportStepIdx].wellgroup_events().hasEvent(
            this->wells_ecl_[well_index].name(), ScheduleEvents::Events::WELL_PRODUCTIVITY_INDEX);
    };
    ...
    auto rescaleWellPI = [this, reportStepIdx](const int well_index, const Scalar newWellPI) -> void {
        const auto& wname = this->wells_ecl_[well_index].name();
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::shouldBalanceNetwork(const int reportStepIdx,
...
template <typename Scalar, typename IndexTraits>
std::vector<int>
BlackoilWellModelGeneric<Scalar, IndexTraits>::getCellsForConnections(const Well& well) const
{
...
BlackoilWellModelGeneric<Scalar, IndexTraits>::getWellsForTesting(const int timeStepIdx, const double simulationTime)
{
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignMassGasRate(data::Wells& wsrpt, const Scalar gasDensity) const
{
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignWellTracerRates(data::Wells& wsrpt,
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::assignMswTracerRates(data::Wells& wsrpt,
...
BlackoilWellModelGeneric<Scalar, IndexTraits>::getMaxWellConnections() const
{
    auto wellConnections = std::vector<std::vector<int>> {};

    auto schedule_wells = this->schedule().wellNames();
    schedule_wells.erase(std::remove_if(schedule_wells.begin(), schedule_wells.end(), this->not_on_process_),
                         schedule_wells.end());
    ...
    for (const auto& well : schedule_wells) {
        auto& compressed_well_perforations
            = wellConnections.emplace_back(this->getCellsForConnections(this->schedule().back().wells(well)));
        ...
        std::sort(compressed_well_perforations.begin(), compressed_well_perforations.end());
    }
...
template <typename Scalar, typename IndexTraits>
int
BlackoilWellModelGeneric<Scalar, IndexTraits>::numLocalWellsEnd() const
{
    const auto& wnames = schedule().back().well_order().names();

    return std::count_if(
        wnames.begin(), wnames.end(), [this](const std::string& wname) { return !this->not_on_process_(wname); });
}

template <typename Scalar, typename IndexTraits>
int
BlackoilWellModelGeneric<Scalar, IndexTraits>::numLocalNonshutWells() const
{
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::initInjMult()
{
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateFiltrationModelsPostStep(const double dt,
    ...
    for (auto& well : this->well_container_generic_) {
        if (well->isInjector()) {
            ...
            if (conc > 0.) {
                ...
                auto retval = this->filter_cake_.emplace(
                    std::piecewise_construct, std::forward_as_tuple(well->name()), std::tuple {});
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateInjMult(DeferredLogger& deferred_logger)
{
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::updateFiltrationModelsPreStep(DeferredLogger& deferred_logger)
{
    for (auto& well : this->well_container_generic_) {
        if (well->isInjector()) {
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::logPrimaryVars() const
{
    std::ostringstream os;
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelGeneric<Scalar, IndexTraits>::reportGroupSwitching(DeferredLogger& local_deferredLogger) const
{
    for (const auto& [name, ctrls] : this->switched_prod_groups_) {
        ...
        if (ctrls[0] != currentControl) {
            const std::string msg = fmt::format("    Production Group {} control model changed from {} to {}",
                                                name,
    ...
    for (const auto& [grname, grdata] : this->switched_inj_groups_) {
        ...
        for (Phase phase : all) {
            ...
            const Group::InjectionCMode currentControl = this->groupState().injection_control(grname, phase);
            if (ctrls[0] != currentControl) {
...
template <typename Scalar, typename IndexTraits>
bool
BlackoilWellModelGeneric<Scalar, IndexTraits>::operator==(const BlackoilWellModelGeneric& rhs) const
{
    return this->initial_step_ == rhs.initial_step_ && this->report_step_starts_ == rhs.report_step_starts_
        && this->last_run_wellpi_ == rhs.last_run_wellpi_ && this->local_shut_wells_ == rhs.local_shut_wells_
        && this->closed_this_step_ == rhs.closed_this_step_ && this->node_pressures_ == rhs.node_pressures_
        && this->last_valid_node_pressures_ == rhs.last_valid_node_pressures_
        && this->prev_inj_multipliers_ == rhs.prev_inj_multipliers_ && this->active_wgstate_ == rhs.active_wgstate_
        && this->last_valid_wgstate_ == rhs.last_valid_wgstate_ && this->nupcol_wgstate_ == rhs.nupcol_wgstate_
        && this->switched_prod_groups_ == rhs.switched_prod_groups_
        && this->switched_inj_groups_ == rhs.switched_inj_groups_
        && this->closed_offending_wells_ == rhs.closed_offending_wells_ && this->gen_gaslift_ == rhs.gen_gaslift_;
}
