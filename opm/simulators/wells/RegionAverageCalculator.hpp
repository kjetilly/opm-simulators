...
namespace RegionAverageCalculator
{
    ...
    public:
        ...
        void defineState(const Simulator& simulator)
        {
            ...
            int numRegions = std::accumulate(rmap_.activeRegions().begin(),
                                             rmap_.activeRegions().end(),
                                             0,
                                             [](const auto acc, const auto& reg) { return std::max(acc, reg); });
            ...
            for (const auto& elem : elements(gridView, Dune::Partitions::interior)) {
                ...
                const Scalar pv_cell = simulator.model().dofTotalVolume(cellIdx) * intQuants.porosity().value();
