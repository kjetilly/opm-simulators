...
WellInterfaceGeneric<Scalar, IndexTraits>::WellInterfaceGeneric(const Well& well,
    ...
    , ipr_b_(num_conservation_quantities)
{
    ...
    assert(std::is_sorted(perf_data.begin(), perf_data.end(), [](const auto& perf1, const auto& perf2) {
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::adaptRatesForVFP(std::vector<Scalar>& rates) const
{
    const auto& pu = this->phaseUsage();
    if (pu.numActivePhases() == 2) {
        if (pu.phaseIsActive(IndexTraits::waterPhaseIdx) && pu.phaseIsActive(IndexTraits::oilPhaseIdx)
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::isInjector() const
{
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::isProducer() const
{
...
template <typename Scalar, typename IndexTraits>
int
WellInterfaceGeneric<Scalar, IndexTraits>::indexOfWell() const
{
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::getAllowCrossFlow() const
{
...
template <typename Scalar, typename IndexTraits>
const Well&
WellInterfaceGeneric<Scalar, IndexTraits>::wellEcl() const
{
    return well_ecl_;
}
...
template <typename Scalar, typename IndexTraits>
Well&
WellInterfaceGeneric<Scalar, IndexTraits>::wellEcl()
{
    return well_ecl_;
}
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::initInjMult(const std::vector<Scalar>& max_inj_mult)
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::updateInjMult(std::vector<Scalar>& inj_multipliers,
...
template <typename Scalar, typename IndexTraits>
Scalar
WellInterfaceGeneric<Scalar, IndexTraits>::getInjMult(const int local_perf_index,
                                                      const Scalar bhp,
                                                      ...
                                                      DeferredLogger& dlogger) const
{
    ...
    if (active_injmult) {
        const auto& injmult
            = is_wrev ? this->well_ecl_.getWellInjMult() : this->well_ecl_.getConnections()[perf_ecl_index].injmult();
        ...
        const Scalar prev_multiplier
            = this->inj_multiplier_[perf_ecl_index] > 0 ? this->inj_multiplier_[perf_ecl_index] : 1.0;
        if (std::abs(multiplier - prev_multiplier) > osc_threshold) {
            const Scalar prev2_multiplier = this->inj_multiplier_previter_[perf_ecl_index] > 0
                ? this->inj_multiplier_previter_[perf_ecl_index]
                : 1.0;
            ...
            if (oscillating) {
                if (damp_factor > min_damp_factor) {
                    ...
                    const auto msg = fmt::format(
                        "Well {}, perforation {}: Injectivity multiplier dampening factor reached minimum (= {})",
                        this->name(),
                        perf_ecl_index,
                        min_damp_factor);
                    dlogger.warning(msg);
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::wellHasTHPConstraints(const SummaryState& summaryState) const
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::updateWellTestState(const SingleWellState<Scalar, IndexTraits>& ws,
                                                               ...
                                                               DeferredLogger& deferred_logger) const
{
    ...
    if (this->isOperableAndSolvable()) {
        WellTest(*this).updateWellTestStateEconomic(
            ws, simulationTime, writeMessageToOPMLog, wellTestState, zero_group_target, deferred_logger);
    } else {
        // updating well test state based on physical (THP/BHP) limits.
        WellTest(*this).updateWellTestStatePhysical(
            simulationTime, writeMessageToOPMLog, wellTestState, deferred_logger);
    }
...
template <typename Scalar, typename IndexTraits>
Scalar
WellInterfaceGeneric<Scalar, IndexTraits>::getTHPConstraint(const SummaryState& summaryState) const
{
    if (dynamic_thp_limit_) {
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::underPredictionMode() const
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::initCompletions()
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::closeCompletions(const WellTestState& wellTestState)
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::setVFPProperties(
    const VFPProperties<Scalar, IndexTraits>* vfp_properties_arg)
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::setPrevSurfaceRates(
    WellState<Scalar, IndexTraits>& well_state, const WellState<Scalar, IndexTraits>& prev_well_state) const
{
    ...
    const bool zero_rates = std::all_of(ws.surface_rates.begin(), ws.surface_rates.end(), [](Scalar rate) {
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::setGuideRate(const GuideRate* guide_rate_arg)
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::setWellEfficiencyFactor(const Scalar efficiency_factor)
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::setRepRadiusPerfLength()
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::setWsolvent(const Scalar wsolvent)
{
    wsolvent_ = wsolvent;
}
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::setDynamicThpLimit(const Scalar thp_limit)
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::setDynamicThpLimit(const std::optional<Scalar> thp_limit)
{
    dynamic_thp_limit_ = thp_limit;
}
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::updatePerforatedCell(std::vector<bool>& is_cell_perforated)
{
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::isVFPActive(DeferredLogger& deferred_logger) const
{
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::isOperableAndSolvable() const
{
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::useVfpExplicit() const
{
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::thpLimitViolatedButNotSwitched() const
{
...
template <typename Scalar, typename IndexTraits>
Scalar
WellInterfaceGeneric<Scalar, IndexTraits>::getALQ(const WellState<Scalar, IndexTraits>& well_state) const
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::reportWellSwitching(const SingleWellState<Scalar, IndexTraits>& ws,
                                                               DeferredLogger& deferred_logger) const
{
    ...
    if (from != to) {
        deferred_logger.info(fmt::format("    Well {} control mode changed from {} to {}", name(), from, to));
    }
}

template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::isPressureControlled(const WellState<Scalar, IndexTraits>& well_state) const
{
    const auto& ws = well_state.well(this->index_of_well_);
    if (this->isInjector()) {
        const Well::InjectorCMode& current = ws.injection_cmode;
        return current == Well::InjectorCMode::THP || current == Well::InjectorCMode::BHP;
    } else {
        const Well::ProducerCMode& current = ws.production_cmode;
        return current == Well::ProducerCMode::THP || current == Well::ProducerCMode::BHP;
    }
}

template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::wellUnderZeroRateTargetIndividual(
    const SummaryState& summary_state, const WellState<Scalar, IndexTraits>& well_state) const
{
...
template <typename Scalar, typename IndexTraits>
bool
WellInterfaceGeneric<Scalar, IndexTraits>::wellUnderGroupControl(const SingleWellState<Scalar, IndexTraits>& ws) const
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::resetWellOperability()
{
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::addPerforations(const std::vector<RuntimePerforation>& perfs)
{
    for (const auto& perf : perfs) {
        ...
        if (it != this->well_cells_.end()) {
            ...
            this->saturation_table_number_.push_back(this->saturation_table_number_.front());
...
template <typename Scalar, typename IndexTraits>
int
WellInterfaceGeneric<Scalar, IndexTraits>::polymerTable_() const
{
...
template <typename Scalar, typename IndexTraits>
int
WellInterfaceGeneric<Scalar, IndexTraits>::polymerWaterTable_() const
{
...
template <typename Scalar, typename IndexTraits>
int
WellInterfaceGeneric<Scalar, IndexTraits>::polymerInjTable_() const
{
...
template <typename Scalar, typename IndexTraits>
std::pair<bool, bool>
WellInterfaceGeneric<Scalar, IndexTraits>::computeWellPotentials(std::vector<Scalar>& well_potentials,
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::checkNegativeWellPotentials(std::vector<Scalar>& well_potentials,
                                                                       ...
                                                                       DeferredLogger& deferred_logger)
{
    ...
    if (total_potential < 0.0 && checkOperability) {
        ...
        const std::string msg = std::string("well ") + this->name() + ": has negative potentials and is not operable";
...
template <typename Scalar, typename IndexTraits>
void
WellInterfaceGeneric<Scalar, IndexTraits>::prepareForPotentialCalculations(
