...
BlackoilWellModel<TypeTag>::BlackoilWellModel(Simulator& simulator)
    ...
    , gaslift_(this->terminal_output_)
{
    ...
    this->alternative_well_rate_init_ = Parameters::Get<Parameters::AlternativeWellRateInit>();

    using SourceDataSpan = typename PAvgDynamicSourceData<Scalar>::template SourceDataSpan<Scalar>;

    this->wbp_.initializeSources(
        [this](const std::size_t globalIndex) { return this->compressedIndexForInterior(globalIndex); },
        [this](const int localCell, SourceDataSpan sourceTerms) {
            using Item = typename SourceDataSpan::Item;

            const auto* intQuants = this->simulator_.model().cachedIntensiveQuantities(localCell, /*timeIndex = */ 0);
            const auto& fs = intQuants->fluidState();

            sourceTerms
                .set(Item::PoreVol, intQuants->porosity().value() * this->simulator_.model().dofTotalVolume(localCell))
            ...
            auto weightedPhaseDensity
                = [&fs](const auto ip) { return fs.saturation(ip).value() * fs.density(ip).value(); };

            if (haveOil) {
                sourceTerms.set(Item::Pressure, fs.pressure(io).value());
            } else if (haveGas) {
                sourceTerms.set(Item::Pressure, fs.pressure(ig).value());
            } else {
                sourceTerms.set(Item::Pressure, fs.pressure(iw).value());
            }

            // Strictly speaking, assumes SUM(s[p]) == 1.
            auto rho = 0.0;
            if (haveOil) {
                rho += weightedPhaseDensity(io);
            }
            if (haveGas) {
                rho += weightedPhaseDensity(ig);
            }
            if (haveWat) {
                rho += weightedPhaseDensity(iw);
            }
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::init()
{
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::initWellContainer(const int reportStepIdx)
{
    const uint64_t effective_events_mask = ScheduleEvents::WELL_STATUS_CHANGE + ScheduleEvents::NEW_WELL;
    const auto& events = this->schedule()[reportStepIdx].wellgroup_events();
    for (auto& wellPtr : this->well_container_) {
        const bool well_opened_this_step
            = this->report_step_starts_ && events.hasEvent(wellPtr->name(), effective_events_mask);
        wellPtr->init(this->depth_, this->gravity_, this->B_avg_, well_opened_this_step);
    }
}

template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::beginReportStep(const int timeStepIdx)
{
    ...
    this->rateConverter_ = std::make_unique<RateConverterType>(std::vector<int>(this->local_num_cells_, 0));
    ...
    OPM_END_PARALLEL_TRY_CATCH_LOG(local_deferredLogger, "beginReportStep() failed: ", this->terminal_output_, comm)
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::initializeLocalWellStructure(const int reportStepIdx, const bool enableWellPIScaling)
{
    DeferredLogger local_deferredLogger {};
    ...
    this->local_parallel_well_info_ = this->createLocalParallelWellInfo(this->wells_ecl_);
    ...
    OPM_BEGIN_PARALLEL_TRY_CATCH()
    {
        ...
        if (enableWellPIScaling
            && this->schedule()[reportStepIdx].events().hasEvent(ScheduleEvents::Events::WELL_PRODUCTIVITY_INDEX)) {
            this->runWellPIScaling(reportStepIdx, local_deferredLogger);
        }
    }
    OPM_END_PARALLEL_TRY_CATCH_LOG(
        local_deferredLogger, "Failed to initialize local well structure: ", this->terminal_output_, comm)
}
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::initializeGroupStructure(const int reportStepIdx)
{
    DeferredLogger local_deferredLogger {};
    ...
    OPM_BEGIN_PARALLEL_TRY_CATCH()
    {
        const auto& fieldGroup = this->schedule().getGroup("FIELD", reportStepIdx);

        WellGroupHelpersType::setCmodeGroup(
            fieldGroup, this->schedule(), this->summaryState(), reportStepIdx, this->groupState());
    ...
    OPM_END_PARALLEL_TRY_CATCH_LOG(
        local_deferredLogger, "Failed to initialize group structure: ", this->terminal_output_, comm)
}
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::beginTimeStep()
{
    ...
    if (this->wellStructureChangedDynamically_) {
        ...
        // structure.
        const auto reportStepIdx = this->simulator_.episodeIndex();
    ...
    OPM_END_PARALLEL_TRY_CATCH_LOG(
        local_deferredLogger, "beginTimeStep() failed: ", this->terminal_output_, simulator_.vanguard().grid().comm());
...
#endif
    // update guide rates
    this->guide_rate_handler_.updateGuideRates(reportStepIdx, simulationTime, this->wellState(), this->groupState());
#ifdef RESERVOIR_COUPLING_ENABLED
    ...
    try {
        ...
        for (auto& well : well_container_) {
            const uint64_t effective_events_mask = ScheduleEvents::WELL_STATUS_CHANGE
                + ScheduleEvents::INJECTION_TYPE_CHANGED + ScheduleEvents::WELL_SWITCHED_INJECTOR_PRODUCER
                + ScheduleEvents::NEW_WELL;

            const auto& events = this->schedule()[reportStepIdx].wellgroup_events();
            const bool event = this->report_step_starts_ && events.hasEvent(well->name(), effective_events_mask);
            const bool dyn_status_change
                = this->wellState().well(well->name()).status != this->prevWellState().well(well->name()).status;

            if (event || dyn_status_change) {
                try {
                    well->updateWellStateWithTarget(
                        simulator_, this->groupState(), this->wellState(), local_deferredLogger);
                    ...
                    const std::string msg = "Compute initial well solution for new well " + well->name()
    ...
    logAndCheckForExceptionsAndThrow(
        local_deferredLogger, exc_type, "beginTimeStep() failed: " + exc_msg, this->terminal_output_, comm);
}
                                        ...
                                        DeferredLogger& deferred_logger)
{
    for (const std::string& well_name : this->getWellsForTesting(timeStepIdx, simulationTime)) {
        ...
        Scalar well_efficiency_factor
            = wellEcl.getEfficiencyFactor() * this->wellState().getGlobalEfficiencyScalingFactor(well_name);
        ...
        try {
            ...
            const std::string msg
                = fmt::format("Exception during testing of well: {}. The well will not open.\n Exception message: {}",
                              wellEcl.name(),
                              e.what());
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::endReportStep()
{
...
BlackoilWellModel<TypeTag>::lastReport() const
{
    return last_report_;
}
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::timeStepSucceeded(const double simulationTime, const double dt)
{
    ...
    this->checkGEconLimits(fieldGroup, simulationTime, simulator_.episodeIndex(), local_deferredLogger);
    this->checkGconsaleLimits(fieldGroup, this->wellState(), simulator_.episodeIndex(), local_deferredLogger);
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::computeTotalRatesForDof(RateVector& rate, unsigned elemIdx) const
{
    rate = 0;
...
void
BlackoilWellModel<TypeTag>::computeTotalRatesForDof(RateVector& rate,
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::initializeWellState(const int timeStepIdx)
{
    const auto pressIx = []() {
    ...
    this->wellState().init(cellPressures,
                           cellTemperatures,
                           this->schedule(),
                           this->wells_ecl_,
                           ...
                           this->summaryState(),
                           simulator_.vanguard().enableDistributedWells());
}
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::createWellContainer(const int report_step)
{
    DeferredLogger local_deferredLogger;
    ...
    if (nw > 0) {
        ...
        std::for_each(this->wells_ecl_.begin(),
                      this->wells_ecl_.end(),
                      [this, &wg_events = this->report_step_start_events_](const auto& well_ecl) {
                          ...
                          constexpr auto events_mask = ScheduleEvents::WELL_STATUS_CHANGE
                              | ScheduleEvents::REQUEST_OPEN_WELL | ScheduleEvents::REQUEST_SHUT_WELL;
                          const bool well_event
                              = this->report_step_starts_ && wg_events.hasEvent(well_ecl.name(), events_mask);
                          ...
                          if (well_event) {
                              if (well_ecl.getStatus() == WellStatus::OPEN) {
                                  this->well_open_times_.insert_or_assign(well_ecl.name(), this->simulator_.time());
                                  this->well_close_times_.erase(well_ecl.name());
                              } else if (well_ecl.getStatus() == WellStatus::SHUT) {
                                  this->well_close_times_.insert_or_assign(well_ecl.name(), this->simulator_.time());
        ...
        const auto cycle_states
            = wcycle.wellStatus(this->simulator_.time(), wmatcher, this->well_open_times_, this->well_close_times_);
        ...
        for (int w = 0; w < nw; ++w) {
            ...
            const auto well_status = this->schedule().getWell(well_name, report_step).getStatus();
            ...
            if (this->wellTestState().well_is_closed(well_name)) {
                ...
                if (events.hasEvent(ScheduleEvents::REQUEST_OPEN_WELL)) {
                    if (!closed_this_step) {
                        ...
                        this->well_open_times_.insert_or_assign(well_name, this->simulator_.time());
            ...
            if (!well_ecl.getAllowCrossFlow()) {
                ...
                if (any_zero_rate_constraint) {
                    ...
                    local_deferredLogger.debug(
                        fmt::format("  Well {} gets shut due to having zero rate constraint and disallowing crossflow ",
                                    well_ecl.name()));
                    this->wellState().shutWell(w);
                    this->well_close_times_.erase(well_name);
                    this->well_open_times_.erase(well_name);
        ...
        if (!wcycle.empty()) {
            const auto schedule_open = [&wg_events = this->report_step_start_events_](const std::string& name) {
...
BlackoilWellModel<TypeTag>::createWellPointer(const int wellID, const int report_step) const
{
...
BlackoilWellModel<TypeTag>::createTypedWellPointer(const int wellID, const int time_step) const
{
    // Use the pvtRegionIdx from the top cell
    const auto& perf_data = this->well_perf_data_[wellID];

    // Cater for case where local part might have no perforations.
    const auto pvtreg = perf_data.empty() ? 0 : this->pvt_region_idx_[perf_data.front().cell_index];
...
template <typename TypeTag>
typename BlackoilWellModel<TypeTag>::WellInterfacePtr
BlackoilWellModel<TypeTag>::createWellForWellTest(const std::string& well_name,
                                                  const int report_step,
                                                  DeferredLogger& deferred_logger) const
{
    // Finding the location of the well in wells_ecl
    const auto it = std::find_if(this->wells_ecl_.begin(), this->wells_ecl_.end(), [&well_name](const auto& w) {
        return well_name == w.name();
    });
    // It should be able to find in wells_ecl.
    if (it == this->wells_ecl_.end()) {
        OPM_DEFLOG_THROW(
            std::logic_error, fmt::format("Could not find well {} in wells_ecl ", well_name), deferred_logger);
    }
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::doPreStepNetworkRebalance(DeferredLogger& deferred_logger)
{
    OPM_TIMEFUNCTION();
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::assemble(const int iterationIdx, const double dt)
{
    OPM_TIMEFUNCTION();
    DeferredLogger local_deferredLogger;
    ...
    if constexpr (BlackoilWellModelGasLift<TypeTag>::glift_debug) {
        if (gaslift_.terminalOutput()) {
            const std::string msg = fmt::format("assemble() : iteration {}", iterationIdx);
    ...
    if (iterationIdx == 0 && this->wellsActive()) {
        ...
        OPM_END_PARALLEL_TRY_CATCH_LOG(
            local_deferredLogger, "assemble() failed (It=0): ", this->terminal_output_, grid().comm());
    }

    const bool well_group_control_changed = updateWellControlsAndNetwork(false, dt, local_deferredLogger);

    // even when there is no wells active, the network nodal pressure still need to be updated through
    // updateWellControlsAndNetwork() but there is no need to assemble the well equations
    if (!this->wellsActive()) {
        return;
    }
...
template <typename TypeTag>
bool
BlackoilWellModel<TypeTag>::updateWellControlsAndNetwork(const bool mandatory_network_balance,
                                                         const double dt,
                                                         DeferredLogger& local_deferredLogger)
{
    OPM_TIMEFUNCTION();
    ...
    while (do_network_update) {
        if (network_update_iteration >= max_iteration) {
            ...
            if (this->shouldBalanceNetwork(episodeIdx, iterationIdx + 1)) {
                ...
                if (this->terminal_output_) {
                    const std::string msg
                        = fmt::format("Maximum of {:d} network iterations has been used and we stop the update. \n"
                                      "The simulator will continue with unconverged network results (imbalance = "
                                      "{:.2e} bar, ctrl_change = {})",
                                      max_iteration,
                                      network_imbalance * 1.0e-5,
                                      well_group_control_changed);
        ...
        if (this->terminal_output_ && (network_update_iteration == iteration_to_relax)) {
            local_deferredLogger.debug("We begin using relaxed tolerance for network update now after "
                                       + std::to_string(iteration_to_relax) + " iterations ");
        }
        ...
        const bool optimize_gas_lift
            = ((network_update_iteration + 1) < std::max(max_iteration, static_cast<std::size_t>(2)));
        std::tie(well_group_control_changed, do_network_update, network_imbalance)
            = updateWellControlsAndNetworkIteration(
                mandatory_network_balance, relax_network_balance, optimize_gas_lift, dt, local_deferredLogger);
...
BlackoilWellModel<TypeTag>::updateWellControlsAndNetworkIteration(const bool mandatory_network_balance,
                                                                  ...
                                                                  DeferredLogger& local_deferredLogger)
{
    ...
    const auto [more_inner_network_update, network_imbalance]
        = updateNetworks(mandatory_network_balance, local_deferredLogger, relax_network_tolerance);
    ...
    OPM_BEGIN_PARALLEL_TRY_CATCH();
    {
        if (optimize_gas_lift) {
            ...
            const bool updatePotentials
                = (this->shouldBalanceNetwork(reportStepIdx, iterationIdx) || mandatory_network_balance);
    ...
    // update guide rates
    if (alq_updated || BlackoilWellModelGuideRates(*this).guideRateUpdateIsNeeded(reportStepIdx)) {
        const double simulationTime = simulator_.time();
...
template <typename TypeTag>
bool
BlackoilWellModel<TypeTag>::computeWellGroupThp(const double dt, DeferredLogger& local_deferredLogger)
{
    OPM_TIMEFUNCTION();
    ...
    for (const std::string& nodeName : network.node_names()) {
        ...
        if (has_choke) {
            ...
            TargetCalculatorType tcalc(cmode,
                                       FluidSystem::phaseUsage(),
                                       resv_coeff,
                                       gratTargetFromSales,
                                       nodeName,
            ...
            auto mismatch = [&](auto group_thp) {
                ...
                for (auto& well : this->well_container_) {
                    ...
                    if (group.hasWell(well_name)) {
                        ...
                        well->iterateWellEqWithSwitching(this->simulator_,
                                                         dt,
                                                         inj_controls,
                                                         prod_controls,
                                                         well_state,
            ...
            if (!autochoke_thp.has_value()) {
                ...
                WellBhpThpCalculatorType::bruteForceBracketCommonTHP(
                    mismatch, range, low1, high1, appr_sol, 0.0, local_deferredLogger);
                min_thp = low1;
                max_thp = high1;
            ...
            if (!autochoke_thp.has_value() || autochoke_thp.value() > nodal_pressure) {
                ...
                std::array<Scalar, 2> range = autochoke_thp.has_value()
                    ? std::array<Scalar, 2> {Scalar {0.9} * autochoke_thp.value(), Scalar {1.1} * autochoke_thp.value()}
                    : range_initial;
                Scalar low, high;
                ...
                const bool finding_bracket = WellBhpThpCalculatorType::bruteForceBracketCommonTHP(
                    mismatch, range, low, high, approximate_solution, tolerance1, local_deferredLogger);
                ...
                if (approximate_solution.has_value()) {
                    ...
                    int iteration = 0;
                    autochoke_thp = RegulaFalsiBisection<ThrowOnError>::solve(
                        mismatch, low, high, max_iteration_solve, tolerance2, iteration);
            ...
            for (auto& well : this->well_container_) {
                ...
                if (thp_is_limit) {
                    well->prepareWellBeforeAssembling(
                        this->simulator_, dt, this->wellState(), this->groupState(), local_deferredLogger);
                }
            }

            // Use the group THP in computeNetworkPressures().
            const auto& current_well_group_thp
                = group_state.is_autochoke_group(nodeName) ? group_state.well_group_thp(nodeName) : 1e30;
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::assembleWellEq(const double dt, DeferredLogger& deferred_logger)
{
    OPM_TIMEFUNCTION();
    for (auto& well : well_container_) {
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::prepareWellsBeforeAssembling(const double dt, DeferredLogger& deferred_logger)
{
    OPM_TIMEFUNCTION();
    for (auto& well : well_container_) {
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::assembleWellEqWithoutIteration(const double dt, DeferredLogger& deferred_logger)
{
    OPM_TIMEFUNCTION();
    ...
    for (auto& well : well_container_) {
        well->assembleWellEqWithoutIteration(simulator_, dt, this->wellState(), this->groupState(), deferred_logger);
    }
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::getWellContributions(WellContributions<Scalar>& wellContribs) const
{
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::addWellContributions(SparseMatrixAdapter& jacobian) const
{
    for (const auto& well : well_container_) {
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::addWellPressureEquations(PressureMatrix& jacobian,
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::addReservoirSourceTerms(
    GlobalEqVector& residual, const std::vector<typename SparseMatrixAdapter::MatrixBlock*>& diagMatAddress) const
{
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::addWellPressureEquationsStruct(PressureMatrix& jacobian) const
{
    int nw = this->numLocalWellsEnd();
    int rdofs = local_num_cells_;
    for (int i = 0; i < nw; ++i) {
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::recoverWellSolutionAndUpdateWellState(const BVector& x)
{
    DeferredLogger local_deferredLogger;
    OPM_BEGIN_PARALLEL_TRY_CATCH();
    {
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::recoverWellSolutionAndUpdateWellStateDomain(const BVector& x, const int domainIdx)
{
    if (!nldd_) {
...
template <typename TypeTag>
ConvergenceReport
BlackoilWellModel<TypeTag>::getWellConvergence(const std::vector<Scalar>& B_avg,
                                               bool checkWellGroupControlsAndNetwork) const
{
    ...
    for (const auto& well : well_container_) {
        if (well->isOperableAndSolvable() || well->wellIsStopped()) {
            local_report += well->getWellConvergence(simulator_,
                                                     this->wellState(),
                                                     B_avg,
                                                     local_deferredLogger,
    ...
    if (checkWellGroupControlsAndNetwork) {
        ...
        report.setNetworkNotYetBalancedForceAnotherNewtonIteration(
    ...
    if (this->terminal_output_) {
        ...
        for (const auto& f : report.wellFailures()) {
            if (f.severity() == ConvergenceReport::Severity::NotANumber) {
                OpmLog::debug("NaN residual found with phase " + std::to_string(f.phase()) + " for well "
                              + f.wellName());
            } else if (f.severity() == ConvergenceReport::Severity::TooLarge) {
                OpmLog::debug("Too large residual found with phase " + std::to_string(f.phase()) + " for well "
                              + f.wellName());
            }
        }
    }
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::calculateExplicitQuantities(DeferredLogger& deferred_logger) const
{
...
template <typename TypeTag>
bool
BlackoilWellModel<TypeTag>::updateWellControls(DeferredLogger& deferred_logger)
{
    OPM_TIMEFUNCTION();
    if (!this->wellsActive()) {
        return false;
...
BlackoilWellModel<TypeTag>::updateNetworks(const bool mandatory_network_balance,
                                           DeferredLogger& deferred_logger,
                                           const bool relax_network_tolerance)
{
    OPM_TIMEFUNCTION();
    ...
    if (this->shouldBalanceNetwork(episodeIdx, iterationIdx) || mandatory_network_balance) {
        ...
        for (int i = 0; i < max_number_of_sub_iterations; i++) {
            const auto local_network_imbalance = this->updateNetworkPressures(
                episodeIdx, network_pressure_update_damping_factor, network_max_pressure_update);
            ...
            const Scalar tolerance = relax_network_tolerance ? relaxation_factor * balance.pressure_tolerance()
            ...
            for (const auto& well : well_container_) {
                ...
                if (it != this->node_pressures_.end()) {
                    ...
                    if (thp_is_limit) {
                        well->prepareWellBeforeAssembling(
                            this->simulator_, dt, this->wellState(), this->groupState(), deferred_logger);
                    }
                }
            }
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::updateAndCommunicate(const int reportStepIdx,
                                                 ...
                                                 DeferredLogger& deferred_logger)
{
    ...
    OPM_END_PARALLEL_TRY_CATCH("BlackoilWellModel::updateAndCommunicate failed: ", simulator_.gridView().comm())
    this->updateAndCommunicateGroupData(reportStepIdx,
                                        iterationIdx,
                                        param_.nupcol_group_rate_tolerance_,
                                        /*update_wellgrouptarget*/ true,
...
template <typename TypeTag>
bool
BlackoilWellModel<TypeTag>::updateGroupControls(const Group& group,
                                                DeferredLogger& deferred_logger,
                                                ...
                                                const int iterationIdx)
{
    ...
    const bool changed_hc = this->checkGroupHigherConstraints(
        group, deferred_logger, reportStepIdx, max_number_of_group_switches, update_group_switching_log);
    if (changed_hc) {
        changed = true;
    ...
    bool changed_individual
        = BlackoilWellModelConstraints(*this).updateGroupIndividualControl(group,
                                                                           reportStepIdx,
    ...
    for (const std::string& groupName : group.groups()) {
        bool changed_this = updateGroupControls(
            this->schedule().getGroup(groupName, reportStepIdx), deferred_logger, reportStepIdx, iterationIdx);
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::updateWellTestState(const double simulationTime, WellTestState& wellTestState)
{
    OPM_TIMEFUNCTION();
    DeferredLogger local_deferredLogger;
    for (const auto& well : well_container_) {
        ...
        const bool under_zero_target
            = well->wellUnderZeroGroupRateTarget(this->simulator_, this->wellState(), local_deferredLogger);
    ...
    for (const auto& [group_name, to] : this->closed_offending_wells_) {
        if (this->hasOpenLocalWell(to.second) && !this->wasDynamicallyShutThisTimeStep(to.second)) {
            wellTestState.close_well(to.second, WellTestConfig::Reason::GROUP, simulationTime);
            this->updateClosedWellsThisStep(to.second);
            const std::string msg = fmt::format("Procedure on exceeding {} limit is WELL for group {}. "
                                                "Well {} is {}.",
                                                to.first,
                                                group_name,
                                                to.second,
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::calculateProductivityIndexValues(DeferredLogger& deferred_logger)
{
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::calculateProductivityIndexValuesShutWells(const int reportStepIdx,
                                                                      DeferredLogger& deferred_logger)
{
    ...
    for (const auto& shutWell : this->local_shut_wells_) {
        ...
        auto wellPtr = this->template createTypedWellPointer<StandardWell<TypeTag>>(shutWell, reportStepIdx);
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::calculateProductivityIndexValues(const WellInterface<TypeTag>* wellPtr,
                                                             DeferredLogger& deferred_logger)
{
    wellPtr->updateProductivityIndex(
        this->simulator_, this->prod_index_calc_[wellPtr->indexOfWell()], this->wellState(), deferred_logger);
}
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::prepareTimeStep(DeferredLogger& deferred_logger)
{
    ...
    const bool do_prestep_network_rebalance
        = param_.pre_solve_network_ && this->needPreStepNetworkRebalance(episodeIdx);
    ...
    for (const auto& well : well_container_) {
        ...
        if (param_.solve_welleq_initially_ && well->isOperableAndSolvable()) {
            try {
                ...
                const std::string msg = "Compute initial well solution for " + well->name()
    ...
    if (do_prestep_network_rebalance)
        doPreStepNetworkRebalance(deferred_logger);
}

template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::updateAverageFormationFactor()
{
    ...
    std::transform(B_avg.begin(), B_avg.end(), B_avg_.begin(), [gcells = global_num_cells_](const auto bval) {
        return bval / gcells;
    });
}
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::updatePrimaryVariables(DeferredLogger& deferred_logger)
{
    for (const auto& well : well_container_) {
        well->updatePrimaryVariables(simulator_, this->wellState(), deferred_logger);
    }
...
BlackoilWellModel<TypeTag>::extractLegacyCellPvtRegionIndex_()
{
    ...
    for (unsigned cellIdx = 0; cellIdx < numCells; ++cellIdx) {
        this->pvt_region_idx_[cellIdx] = eclProblem.pvtRegionIndex(cellIdx);
    }
}
...
template <typename TypeTag>
typename BlackoilWellModel<TypeTag>::WellInterfacePtr
BlackoilWellModel<TypeTag>::getWell(const std::string& well_name) const
{
    // finding the iterator of the well in wells_ecl
    auto well = std::find_if(well_container_.begin(),
                             well_container_.end(),
                             [&well_name](const WellInterfacePtr& elem) -> bool { return elem->name() == well_name; });
...
template <typename TypeTag>
int
BlackoilWellModel<TypeTag>::reportStepIndex() const
{
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::calcResvCoeff(const int fipnum,
                                          const int pvtreg,
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::calcInjResvCoeff(const int fipnum, const int pvtreg, std::vector<Scalar>& resv_coeff)
{
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::computeWellTemperature()
{
        ...
        for (auto wellID = 0 * nw; wellID < nw; ++wellID) {
            ...
            for (int_type perf = 0, end_perf = this->well_perf_data_[wellID].size(); perf < end_perf; ++perf) {
                ...
                for (unsigned phaseIdx = 0; phaseIdx < FluidSystem::numPhases; ++phaseIdx) {
                    ...
                    cellInternalEnergy
                        = fs.enthalpy(phaseIdx).value() - fs.pressure(phaseIdx).value() / fs.density(phaseIdx).value();
...
template <typename TypeTag>
void
BlackoilWellModel<TypeTag>::assignWellTracerRates(data::Wells& wsrpt) const
{
    ...
    BlackoilWellModelGeneric<Scalar, IndexTraits>::assignWellTracerRates(
        wsrpt, trMod.getWellTracerRates(), reportStepIdx);
    BlackoilWellModelGeneric<Scalar, IndexTraits>::assignWellTracerRates(
        wsrpt, trMod.getWellFreeTracerRates(), reportStepIdx);
    BlackoilWellModelGeneric<Scalar, IndexTraits>::assignWellTracerRates(
        wsrpt, trMod.getWellSolTracerRates(), reportStepIdx);
