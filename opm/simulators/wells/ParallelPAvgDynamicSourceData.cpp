...
Opm::ParallelPAvgDynamicSourceData<Scalar>::ParallelPAvgDynamicSourceData(
    const Parallel::Communication& comm, const std::vector<std::size_t>& sourceLocations, GlobalToLocal localCellIdx)
...
template <class Scalar>
void
Opm::ParallelPAvgDynamicSourceData<Scalar>::setToZero()
{
...
template <class Scalar>
void
Opm::ParallelPAvgDynamicSourceData<Scalar>::reconstruct(const std::vector<std::size_t>& sourceLocations,
...
template <class Scalar>
void
Opm::ParallelPAvgDynamicSourceData<Scalar>::collectLocalSources(Evaluator eval)
{
...
template <class Scalar>
void
Opm::ParallelPAvgDynamicSourceData<Scalar>::synchroniseSources()
{
    this->comm_.get().allgatherv(this->localSrc_.data(), // Input (from)
...
Opm::ParallelPAvgDynamicSourceData<Scalar>::storageIndex(const typename std::vector<Scalar>::size_type elemIndex) const
{
...
template <class Scalar>
void
Opm::ParallelPAvgDynamicSourceData<Scalar>::finaliseConstruction(const std::vector<std::size_t>& sourceLocations,
                                                                 GlobalToLocal localCellIdx)
{
...
template <class Scalar>
void
Opm::ParallelPAvgDynamicSourceData<Scalar>::defineCommunication()
{
    // 1) Determine origins/owning ranks for all source terms.
    auto ixVec = std::vector<std::size_t>(this->locations_.size());
    std::transform(this->locations_.begin(), this->locations_.end(), ixVec.begin(), [](const auto& location) {
        return location.ix;
    });
    ...
    std::transform(allIxStart.begin(), allIxStart.end(), this->startPointers_.begin(), [](const int start) {
    ...
    std::adjacent_difference(this->startPointers_.begin() + 1, this->startPointers_.end(), this->allSizes_.begin());
    ...
    if (auto maxIxPos = std::max_element(allIndices.begin(), allIndices.end()); maxIxPos != allIndices.end()) {
