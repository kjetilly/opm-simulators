...
StandardWell<TypeTag>::StandardWell(const Well& well,
...
template <typename TypeTag>
void
StandardWell<TypeTag>::init(const std::vector<Scalar>& depth_arg,
...
void
StandardWell<TypeTag>::computePerfRate(const IntensiveQuantities& intQuants,
...
template <class Value>
void
StandardWell<TypeTag>::computePerfRate(const std::vector<Value>& mob,
                                       ...
                                       DeferredLogger& deferred_logger) const
{
    ...
    RatioCalculator<Value> ratioCalc {FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)
    ...
    if (drawdown > 0) {
        ...
        if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx)
            ...
            ratioCalc.gasOilPerfRateProd(cq_s,
                                         perf_rates,
                                         rv,
                                         rs,
                                         rvw,
        ...
        if (FluidSystem::enableVaporizedWater() && FluidSystem::enableDissolvedGasInWater()) {
            ratioCalc.disOilVapWatVolumeRatio(
                volumeRatio, rvw, rsw, pressure, cmix_s, b_perfcells_dense, deferred_logger);
            ...
            if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx)
                && FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)) {
                ratioCalc.gasOilVolumeRatio(volumeRatio, rv, rs, pressure, cmix_s, b_perfcells_dense, deferred_logger);
            } else {
                if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx)) {
        ...
        if (this->isProducer()) {
            if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx)
                && FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)) {
                ratioCalc.gasOilPerfRateInj(cq_s,
                                            perf_rates,
                                            rv,
                                            rs,
                                            pressure,
            ...
            if (FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)
                && FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)) {
                // no oil
                ratioCalc.gasWaterPerfRateInj(cq_s, perf_rates, rvw, rsw, pressure, deferred_logger);
            }
        }
    }
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::assembleWellEqWithoutIteration(const Simulator& simulator,
                                                      const double dt,
                                                      ...
                                                      DeferredLogger& deferred_logger)
{
    ...
    assembleWellEqWithoutIterationImpl(
        simulator, dt, inj_controls, prod_controls, well_state, group_state, deferred_logger);
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::assembleWellEqWithoutIterationImpl(const Simulator& simulator,
                                                          const double dt,
                                                          const Well::InjectionControls& inj_controls,
                                                          const Well::ProductionControls& prod_controls,
                                                          WellStateType& well_state,
                                                          ...
                                                          DeferredLogger& deferred_logger)
{
    ...
    for (int perf = 0; perf < this->number_of_local_perforations_; ++perf) {
        ...
        std::vector<EvalWell> cq_s(this->num_conservation_quantities_,
                                   {this->primary_variables_.numWellEq() + Indices::numEq, 0.0});
        ...
        calculateSinglePerf(
            simulator, perf, well_state, connectionRates, cq_s, water_flux_s, cq_s_zfrac_effective, deferred_logger);

        // Equation assembly for this perforation.
        if constexpr (has_polymer && Base::has_polymermw) {
            if (this->isInjector()) {
                handleInjectivityEquations(simulator, well_state, perf, water_flux_s, deferred_logger);
            }
        }
        for (int componentIdx = 0; componentIdx < this->num_conservation_quantities_; ++componentIdx) {
            ...
            StandardWellAssemble<FluidSystem, Indices>(*this).assemblePerforationEq(
                cq_s_effective, componentIdx, perf, this->primary_variables_.numWellEq(), this->linSys_);
            ...
            if (has_solvent && componentIdx == Indices::contiSolventEqIdx) {
                ...
                perf_rates[perf * np + FluidSystem::activeCompToActivePhaseIdx(componentIdx)]
                    = cq_s[componentIdx].value();
            }
        }

        if constexpr (has_zFraction) {
            StandardWellAssemble<FluidSystem, Indices>(*this).assembleZFracEq(
                cq_s_zfrac_effective, perf, this->primary_variables_.numWellEq(), this->linSys_);
        }
    }
    ...
    for (int componentIdx = 0; componentIdx < numWellConservationEq; ++componentIdx) {
        ...
        if (FluidSystem::numActivePhases() > 1) {
            assert(dt > 0);
            resWell_loc += (this->primary_variables_.surfaceVolumeFraction(componentIdx) - this->F0_[componentIdx])
                * volume / dt;
        }
        resWell_loc -= this->primary_variables_.getQs(componentIdx) * this->well_efficiency_factor_;
        StandardWellAssemble<FluidSystem, Indices>(*this).assembleSourceEq(
            resWell_loc, componentIdx, this->primary_variables_.numWellEq(), this->linSys_);
    }
    ...
    StandardWellAssemble<FluidSystem, Indices>(*this).assembleControlEq(well_state,
                                                                        group_state,
                                                                        schedule,
                                                                        summaryState,
                                                                        inj_controls,
    ...
    try {
        ...
        OPM_DEFLOG_PROBLEM(
            NumericalProblem, "Error when inverting local well equations for well " + name(), deferred_logger);
    }
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::calculateSinglePerf(const Simulator& simulator,
                                           const int perf,
                                           WellStateType& well_state,
                                           ...
                                           DeferredLogger& deferred_logger) const
{
    ...
    std::vector<EvalWell> mob(this->num_conservation_quantities_,
                              {this->primary_variables_.numWellEq() + Indices::numEq, 0.});
    ...
    computePerfRate(intQuants, mob, bhp, Tw, perf, allow_cf, cq_s, perf_rates, deferred_logger);
    ...
    if constexpr (has_energy) {
        connectionRates[perf][Indices::contiEnergyEqIdx] = connectionRateEnergy(cq_s, intQuants, deferred_logger);
    }
    ...
    if constexpr (has_polymer) {
        ...
        if (this->isInjector()) {
            ...
            polymerConcentration
                = this->extendEval(intQuants.polymerConcentration() * intQuants.polymerViscosityCorrection());
        }

        [[maybe_unused]] EvalWell cq_s_poly;
        std::tie(connectionRates[perf][Indices::contiPolymerEqIdx], cq_s_poly)
            = this->connections_.connectionRatePolymer(perf_data.polymer_rates[perf], cq_s, polymerConcentration);
    ...
    if constexpr (has_foam) {
        ...
        connectionRates[perf][Indices::contiFoamEqIdx] = this->connections_.connectionRateFoam(
            cq_s, foamConcentration, FoamModule::transportPhase(), deferred_logger);
    }
    ...
    if constexpr (has_zFraction) {
        ...
        if (this->isInjector()) {
            ...
            solventConcentration
                = std::array {this->extendEval(intQuants.xVolume()), this->extendEval(intQuants.yVolume())};
        }
        std::tie(connectionRates[perf][Indices::contiZfracEqIdx], cq_s_zfrac_effective)
    ...
    if constexpr (has_brine) {
        ...
        connectionRates[perf][Indices::contiBrineEqIdx] = this->connections_.connectionRateBrine(
            perf_data.brine_rates[perf], perf_rates.vap_wat, cq_s, saltConcentration);
    }
    ...
    if constexpr (has_bioeffects) {
        ...
        if constexpr (has_micp) {
            ...
            if (this->isProducer()) {
                microbialConcentration = this->extendEval(intQuants.microbialConcentration());
                connectionRates[perf][Indices::contiMicrobialEqIdx] = this->connections_.connectionRateBioeffects(
                    perf_data.microbial_rates[perf], perf_rates.vap_wat, cq_s, microbialConcentration);
            }
        }
    }
...
template <class Value>
void
StandardWell<TypeTag>::getMobility(const Simulator& simulator,
                                   const int perf,
                                   ...
                                   DeferredLogger& deferred_logger) const
{
    ...
    if constexpr (has_polymer) {
        ...
        if constexpr (!Base::has_polymermw) {
            if constexpr (std::is_same_v<Value, Scalar>) {
                std::vector<EvalWell> mob_eval(this->num_conservation_quantities_,
                                               {this->primary_variables_.numWellEq() + Indices::numEq, 0.});
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updateWellState(const Simulator& simulator,
                                       ...
                                       DeferredLogger& deferred_logger)
{
    ...
    Base::calculateReservoirRates(simulator.vanguard().eclState().runspec().co2Storage(),
                                  well_state.well(this->index_of_well_));
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updatePrimaryVariablesNewton(const BVectorWell& dwells,
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updateWellStateFromPrimaryVariables(WellStateType& well_state,
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updateIPR(const Simulator& simulator, DeferredLogger& deferred_logger) const
{
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updateIPRImplicit(const Simulator& simulator,
                                         WellStateType& well_state,
                                         DeferredLogger& deferred_logger)
{
    ...
    if (zero_rates) {
        const auto msg
            = fmt::format("updateIPRImplicit: Well {} has zero rate, IPRs might be problematic", this->name());
...
template <typename TypeTag>
void
StandardWell<TypeTag>::checkOperabilityUnderBHPLimit(const WellStateType& well_state,
...
template <typename TypeTag>
void
StandardWell<TypeTag>::checkOperabilityUnderTHPLimit(const Simulator& simulator,
                                                     const WellStateType& well_state,
                                                     DeferredLogger& deferred_logger)
{
    const auto& summaryState = simulator.vanguard().summaryState();
    const auto obtain_bhp = this->isProducer()
        ? computeBhpAtThpLimitProd(well_state, simulator, summaryState, deferred_logger)
    ...
    if (obtain_bhp) {
        ...
        this->operability_status_.obey_bhp_limit_with_thp_limit
            = this->isProducer() ? *obtain_bhp >= bhp_limit : *obtain_bhp <= bhp_limit;
        ...
        if (this->isProducer() && *obtain_bhp < thp_limit) {
            const std::string msg = " obtained bhp " + std::to_string(unit::convert::to(*obtain_bhp, unit::barsa))
                + " bars is SMALLER than thp limit " + std::to_string(unit::convert::to(thp_limit, unit::barsa))
                + " bars as a producer for well " + name();
            deferred_logger.debug(msg);
        } else if (this->isInjector() && *obtain_bhp > thp_limit) {
            const std::string msg = " obtained bhp " + std::to_string(unit::convert::to(*obtain_bhp, unit::barsa))
                + " bars is LARGER than thp limit " + std::to_string(unit::convert::to(thp_limit, unit::barsa))
        ...
        if (!this->wellIsStopped()) {
            ...
            deferred_logger.debug(" could not find bhp value at thp limit "
                                  + std::to_string(unit::convert::to(thp_limit, unit::barsa)) + " bar for well "
                                  + name() + ", the well might need to be closed ");
        }
    }
}
...
template <typename TypeTag>
bool
StandardWell<TypeTag>::allDrawDownWrongDirection(const Simulator& simulator) const
{
    ...
    for (int perf = 0; perf < this->number_of_local_perforations_; ++perf) {
        ...
        if ((drawdown < 0. && this->isInjector()) || (drawdown > 0. && this->isProducer())) {
    ...
    const auto& comm = this->parallel_well_info_.communication();
    if (comm.size() > 1) {
        all_drawdown_wrong_direction = (comm.min(all_drawdown_wrong_direction ? 1 : 0) == 1);
    }
...
template <typename TypeTag>
bool
StandardWell<TypeTag>::openCrossFlowAvoidSingularity(const Simulator& simulator) const
{
...
StandardWell<TypeTag>::computePropertiesForWellConnectionPressures(const Simulator& simulator,
                                                                   const WellStateType& well_state) const
{
    auto prop_func = typename StdWellEval::StdWellConnections::PressurePropertyFunctions {
        ...
        [&model = simulator.model()](int cell_idx, int phase_idx) {
            return model.intensiveQuantities(cell_idx, /* time_idx = */ 0).fluidState().temperature(phase_idx).value();
        },

        // getSaltConcentration
        [&model = simulator.model()](int cell_idx) {
            return model.intensiveQuantities(cell_idx, /* time_idx = */ 0).fluidState().saltConcentration().value();
        },

        // getPvtRegionIdx
        [&model = simulator.model()](int cell_idx) {
            return model.intensiveQuantities(cell_idx, /* time_idx = */ 0).fluidState().pvtRegionIndex();
        }};

    if constexpr (Indices::enableSolvent) {
        prop_func.solventInverseFormationVolumeFactor = [&model = simulator.model()](int cell_idx) {
        ...
        prop_func.solventRefDensity = [&model = simulator.model()](int cell_idx) {
            return model.intensiveQuantities(cell_idx, /* time_idx = */ 0).solventRefDensity();
        };
    }
...
template <typename TypeTag>
ConvergenceReport
StandardWell<TypeTag>::getWellConvergence(const Simulator& simulator,
                                          const WellStateType& well_state,
                                          const std::vector<Scalar>& B_avg,
                                          DeferredLogger& deferred_logger,
                                          const bool relax_tolerance) const
{
    ...
    assert((int(B_avg.size()) == this->num_conservation_quantities_) || has_polymer || has_energy || has_foam
           || has_brine || has_zFraction || has_bioeffects);
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updateProductivityIndex(const Simulator& simulator,
                                               ...
                                               DeferredLogger& deferred_logger) const
{
    auto fluidState = [&simulator, this](const int perf) {
        ...
        return simulator.model().intensiveQuantities(cell_idx, /*timeIdx=*/0).fluidState();
    };

    const int np = this->number_of_phases_;
    auto setToZero = [np](Scalar* x) -> void { std::fill_n(x, np, 0.0); };

    auto addVector
        = [np](const Scalar* src, Scalar* dest) -> void { std::transform(src, src + np, dest, dest, std::plus<> {}); };
    ...
    for (const auto& perf : *this->perf_data_) {
        ...
        if (this->isInjector()) {
            this->computeConnLevelInjInd(fs, preferred_phase, connPICalc, mob, connPI, deferred_logger);
...
template <typename TypeTag>
void
StandardWell<TypeTag>::computeWellConnectionDensitesPressures(const Simulator& simulator,
                                                              const WellStateType& well_state,
                                                              ...
                                                              DeferredLogger& deferred_logger)
{
    ...
    const auto prop_func = typename StdWellEval::StdWellConnections::DensityPropertyFunctions {
        ...
        [&model = simulator.model()](const int cell, const std::vector<int>& phases, std::vector<Scalar>& mob) {
            const auto& iq = model.intensiveQuantities(cell, /* time_idx = */ 0);

            std::transform(phases.begin(), phases.end(), mob.begin(), [&iq](const int phase) {
                return iq.mobility(phase).value();
            });
        },

        // densityInCell: Reservoir condition phase densities in
        // specified cell.
        [&model = simulator.model()](const int cell, const std::vector<int>& phases, std::vector<Scalar>& rho) {
            const auto& fs = model.intensiveQuantities(cell, /* time_idx = */ 0).fluidState();

            std::transform(phases.begin(), phases.end(), rho.begin(), [&fs](const int phase) {
                return fs.density(phase).value();
            });
        }};

    const auto stopped_or_zero_rate_target = this->stoppedOrZeroRateTarget(simulator, well_state, deferred_logger);

    this->connections_.computeProperties(stopped_or_zero_rate_target, well_state, prop_func, props, deferred_logger);
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::computeWellConnectionPressures(const Simulator& simulator,
                                                      const WellStateType& well_state,
                                                      DeferredLogger& deferred_logger)
{
    const auto props = computePropertiesForWellConnectionPressures(simulator, well_state);

    computeWellConnectionDensitesPressures(simulator, well_state, props, deferred_logger);
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::solveEqAndUpdateWellState(const Simulator& simulator,
                                                 WellStateType& well_state,
                                                 DeferredLogger& deferred_logger)
{
    if (!this->isOperableAndSolvable() && !this->wellIsStopped())
...
template <typename TypeTag>
void
StandardWell<TypeTag>::calculateExplicitQuantities(const Simulator& simulator,
                                                   const WellStateType& well_state,
                                                   DeferredLogger& deferred_logger)
{
...
template <typename TypeTag>
void
StandardWell<TypeTag>::apply(const BVector& x, BVector& Ax) const
{
    if (!this->isOperableAndSolvable() && !this->wellIsStopped())
        return;
...
template <typename TypeTag>
void
StandardWell<TypeTag>::apply(BVector& r) const
{
    if (!this->isOperableAndSolvable() && !this->wellIsStopped())
        return;
...
template <typename TypeTag>
void
StandardWell<TypeTag>::recoverWellSolutionAndUpdateWellState(const Simulator& simulator,
                                                             const BVector& x,
                                                             WellStateType& well_state,
                                                             DeferredLogger& deferred_logger)
{
...
template <typename TypeTag>
void
StandardWell<TypeTag>::computeWellRatesWithBhp(const Simulator& simulator,
                                               const Scalar& bhp,
...
template <typename TypeTag>
void
StandardWell<TypeTag>::computeWellRatesWithBhpIterations(const Simulator& simulator,
                                                         const Scalar& bhp,
                                                         std::vector<Scalar>& well_flux,
                                                         DeferredLogger& deferred_logger) const
{
    ...
    const auto& summary_state = simulator.vanguard().summaryState();
    auto inj_controls = well_copy.well_ecl_.isInjector() ? well_copy.well_ecl_.injectionControls(summary_state)
                                                         : Well::InjectionControls(0);
    auto prod_controls = well_copy.well_ecl_.isProducer() ? well_copy.well_ecl_.productionControls(summary_state)
    ...
    for (int phase = 0; phase < np; ++phase) {
        well_state_copy.wellRates(this->index_of_well_)[phase] = sign * ws.well_potentials[phase];
    }
    ...
    const bool converged = well_copy.iterateWellEqWithControl(
        simulator, dt, inj_controls, prod_controls, well_state_copy, group_state, deferred_logger);
    if (!converged) {
...
StandardWell<TypeTag>::computeWellPotentialWithTHP(const Simulator& simulator,
                                                   DeferredLogger& deferred_logger,
                                                   const WellStateType& well_state) const
{
    ...
    if (well.isInjector()) {
        ...
        if (bhp_at_thp_limit) {
            ...
            deferred_logger.warning("FAILURE_GETTING_CONVERGED_POTENTIAL",
                                    "Failed in getting converged thp based potential calculation for well " + name()
                                        + ". Instead the bhp based value is used");
            const Scalar bhp = controls.bhp_limit;
            computeWellRatesWithBhp(simulator, bhp, potentials, deferred_logger);
        }
    } else {
        computeWellRatesWithThpAlqProd(simulator, summary_state, deferred_logger, potentials, this->getALQ(well_state));
    }
...
template <typename TypeTag>
bool
StandardWell<TypeTag>::computeWellPotentialsImplicit(const Simulator& simulator,
                                                     const WellStateType& well_state,
                                                     ...
                                                     DeferredLogger& deferred_logger) const
{
    ...
    for (int comp_idx = 0; comp_idx < this->num_conservation_quantities_; ++comp_idx) {
        if (has_solvent && comp_idx == Indices::contiSolventEqIdx)
            continue; // we do not store the solvent in the well_potentials
...
StandardWell<TypeTag>::computeWellRatesAndBhpWithThpAlqProd(const Simulator& simulator,
                                                            const SummaryState& summary_state,
                                                            DeferredLogger& deferred_logger,
...
template <typename TypeTag>
void
StandardWell<TypeTag>::computeWellRatesWithThpAlqProd(const Simulator& simulator,
                                                      const SummaryState& summary_state,
                                                      DeferredLogger& deferred_logger,
                                                      std::vector<Scalar>& potentials,
                                                      Scalar alq) const
{
    /*double bhp =*/
    computeWellRatesAndBhpWithThpAlqProd(simulator, summary_state, deferred_logger, potentials, alq);
}

template <typename TypeTag>
void
StandardWell<TypeTag>::computeWellPotentials(const Simulator& simulator,
                                             const WellStateType& well_state,
                                             std::vector<Scalar>& well_potentials,
                                             DeferredLogger& deferred_logger) // const
{
    ...
    if (this->param_.local_well_solver_control_switching_
        && !(this->changed_to_open_this_step_
    ...
    this->checkNegativeWellPotentials(well_potentials, this->param_.check_well_operability_, deferred_logger);
}
...
template <typename TypeTag>
typename StandardWell<TypeTag>::Scalar
StandardWell<TypeTag>::connectionDensity([[maybe_unused]] const int globalConnIdx, const int openConnIdx) const
{
    return (openConnIdx < 0) ? 0.0 : this->connections_.rho(openConnIdx);
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updatePrimaryVariables(const Simulator& simulator,
                                              const WellStateType& well_state,
                                              DeferredLogger& deferred_logger)
{
    if (!this->isOperableAndSolvable() && !this->wellIsStopped())
...
template <typename TypeTag>
typename StandardWell<TypeTag>::Scalar
StandardWell<TypeTag>::getRefDensity() const
{
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updateWaterMobilityWithPolymer(const Simulator& simulator,
                                                      const int perf,
                                                      ...
                                                      DeferredLogger& deferred_logger) const
{
    ...
    if (this->isInjector()) {
        ...
        const unsigned waterCompIdx = FluidSystem::canonicalToActiveCompIdx(FluidSystem::waterCompIdx);
        mob[waterCompIdx] /= (this->extendEval(int_quant.waterViscosityCorrection())
                              * visc_mult_table.eval(polymer_concentration, /*extrapolate=*/true));
    }
    ...
    if (PolymerModule::hasPlyshlog()) {
        ...
        computePerfRate(int_quant, mob, bhp, Tw, perf, allow_cf, cq_s, perf_rates, deferred_logger);
        ...
        const auto& scaled_drainage_info = material_law_manager->oilWaterScaledEpsInfoDrainage(cell_idx);
        ...
        const unsigned waterCompIdx = FluidSystem::canonicalToActiveCompIdx(FluidSystem::waterCompIdx);
        EvalWell water_velocity
            = cq_s[waterCompIdx] / denom * this->extendEval(int_quant.fluidState().invB(FluidSystem::waterPhaseIdx));
        ...
        const EvalWell shear_factor
            = PolymerModule::computeShearFactor(polymer_concentration, int_quant.pvtRegionIndex(), water_velocity);
...
StandardWell<TypeTag>::addWellPressureEquations(PressureMatrix& jacobian,
                                                ...
                                                const WellStateType& well_state) const
{
    this->linSys_.extractCPRPressureMatrix(
        jacobian, weights, pressureVarIndex, use_well_weights, *this, Bhp, well_state);
}
...
StandardWell<TypeTag>::pskinwater(const Scalar throughput,
                                  ...
                                  DeferredLogger& deferred_logger) const
{
    if constexpr (Base::has_polymermw) {
        ...
        if (water_table_id <= 0) {
            OPM_DEFLOG_THROW(
                std::runtime_error, fmt::format("Unused SKPRWAT table id used for well {}", name()), deferred_logger);
        }
        ...
        OPM_DEFLOG_THROW(std::runtime_error,
                         fmt::format("Polymermw is not activated, while injecting "
                                     "skin pressure is requested for well {}",
                                     name()),
                         deferred_logger);
    }
}
...
template <typename TypeTag>
typename StandardWell<TypeTag>::EvalWell
StandardWell<TypeTag>::pskin(const Scalar throughput,
                             const EvalWell& water_velocity,
...
template <typename TypeTag>
typename StandardWell<TypeTag>::EvalWell
StandardWell<TypeTag>::wpolymermw(const Scalar throughput,
                                  const EvalWell& water_velocity,
                                  DeferredLogger& deferred_logger) const
{
    if constexpr (Base::has_polymermw) {
        ...
        OPM_DEFLOG_THROW(std::runtime_error,
                         fmt::format("Polymermw is not activated, while injecting "
                                     "polymer molecular weight is requested for well {}",
                                     name()),
                         deferred_logger);
    }
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updateWaterThroughput([[maybe_unused]] const double dt, WellStateType& well_state) const
{
    if constexpr (Base::has_polymermw) {
        if (!this->isInjector()) {
            return;
        ...
        auto& perf_water_throughput = well_state.well(this->index_of_well_).perf_data.water_throughput;
...
template <typename TypeTag>
void
StandardWell<TypeTag>::handleInjectivityRate(const Simulator& simulator,
                                             const int perf,
...
template <typename TypeTag>
void
StandardWell<TypeTag>::handleInjectivityEquations(const Simulator& simulator,
                                                  const WellStateType& well_state,
                                                  const int perf,
                                                  ...
                                                  DeferredLogger& deferred_logger)
{
    ...
    StandardWellAssemble<FluidSystem, Indices>(*this).assembleInjectivityEq(
        eq_pskin, eq_wat_vel, pskin_index, wat_vel_index, perf, this->primary_variables_.numWellEq(), this->linSys_);
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::checkConvergenceExtraEqs(const std::vector<Scalar>& res, ConvergenceReport& report) const
{
    ...
    if constexpr (Base::has_polymermw) {
        WellConvergence(*this).checkConvergencePolyMW(res, Bhp, this->param_.max_residual_allowed_, report);
    }
}
...
template <typename TypeTag>
void
StandardWell<TypeTag>::updateConnectionRatePolyMW(const EvalWell& cq_s_poly,
...
StandardWell<TypeTag>::computeBhpAtThpLimitProd(const WellStateType& well_state,
                                                const Simulator& simulator,
                                                const SummaryState& summary_state,
                                                DeferredLogger& deferred_logger) const
{
...
template <typename TypeTag>
std::optional<typename StandardWell<TypeTag>::Scalar>
StandardWell<TypeTag>::computeBhpAtThpLimitProdWithAlq(const Simulator& simulator,
                                                       const SummaryState& summary_state,
...
template <typename TypeTag>
std::optional<typename StandardWell<TypeTag>::Scalar>
StandardWell<TypeTag>::computeBhpAtThpLimitInj(const Simulator& simulator,
                                               const SummaryState& summary_state,
                                               DeferredLogger& deferred_logger) const
{
    // Make the frates() function.
    ...
    return WellBhpThpCalculator(*this).computeBhpAtThpLimitInj(
        frates, summary_state, this->connections_.rho(), 1e-6, 50, true, deferred_logger);
}
...
template <typename TypeTag>
bool
StandardWell<TypeTag>::iterateWellEqWithControl(const Simulator& simulator,
                                                const double dt,
                                                const Well::InjectionControls& inj_controls,
                                                const Well::ProductionControls& prod_controls,
                                                WellStateType& well_state,
                                                ...
                                                DeferredLogger& deferred_logger)
{
    ...
    do {
        ...
        // TODO: when this function is used for well testing purposes, will need to check the controls, so that we will
        // obtain convergence under the most restrictive control. Based on this converged results, we can check whether
        // to re-open the well. Either we refactor this function or we use different functions for the well testing
        // purposes. We don't allow for switching well controls while computing well potentials and testing wells
...
template <typename TypeTag>
bool
StandardWell<TypeTag>::iterateWellEqWithSwitching(const Simulator& simulator,
                                                  const double dt,
                                                  const Well::InjectionControls& inj_controls,
                                                  const Well::ProductionControls& prod_controls,
                                                  WellStateType& well_state,
                                                  ...
                                                  const bool fixed_status /*false*/)
{
    ...
    do {
        ...
        if (allow_switching && its_since_last_switch >= min_its_after_switch
            ...
            changed = this->updateWellControlAndStatusLocalIteration(simulator,
                                                                     well_state,
                                                                     group_state,
                                                                     inj_controls,
                                                                     prod_controls,
...
template <typename TypeTag>
std::vector<typename StandardWell<TypeTag>::Scalar>
StandardWell<TypeTag>::computeCurrentWellRates(const Simulator& simulator, DeferredLogger& deferred_logger) const
{
    ...
    for (int perf = 0; perf < this->number_of_local_perforations_; ++perf) {
        ...
        const std::vector<Scalar> Tw = this->wellIndex(perf, intQuants, trans_mult, wellstate_nupcol);
        PerforationRates<Scalar> perf_rates;
        computePerfRate(intQuants, mob, bhp.value(), Tw, perf, allow_cf, cq_s, perf_rates, deferred_logger);
...
template <typename TypeTag>
std::vector<typename StandardWell<TypeTag>::Scalar>
StandardWell<TypeTag>::getPrimaryVars() const
{
...
template <typename TypeTag>
int
StandardWell<TypeTag>::setPrimaryVars(typename std::vector<Scalar>::const_iterator it)
{
    const int num_pri_vars = this->primary_variables_.numWellEq();
    for (int ii = 0; ii < num_pri_vars; ++ii) {
...
StandardWell<TypeTag>::connectionRateEnergy(const std::vector<EvalWell>& cq_s,
                                            ...
                                            DeferredLogger& deferred_logger) const
{
    ...
    for (unsigned phaseIdx = 0; phaseIdx < FluidSystem::numPhases; ++phaseIdx) {
        ...
        if (!both_oil_gas || FluidSystem::waterPhaseIdx == phaseIdx) {
            ...
            if (d <= 0.0) {
                deferred_logger.debug(fmt::format("Problematic d value {} obtained for well {}"
                ...
                if (FluidSystem::gasPhaseIdx == phaseIdx) {
                    cq_r_thermal = (cq_s[gasCompIdx] - this->extendEval(fs.Rs()) * cq_s[oilCompIdx])
                        / (d * this->extendEval(fs.invB(phaseIdx)));
                } else if (FluidSystem::oilPhaseIdx == phaseIdx) {
                    // q_or = 1 / (b_o * d) * (q_os - rv * q_gs)
                    cq_r_thermal = (cq_s[oilCompIdx] - this->extendEval(fs.Rv()) * cq_s[gasCompIdx])
                        / (d * this->extendEval(fs.invB(phaseIdx)));
                }
            }
        }
