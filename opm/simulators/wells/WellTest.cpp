...
bool
WellTest<Scalar, IndexTraits>::checkMaxRatioLimitWell(const SingleWellState<Scalar, IndexTraits>& ws,
...
template <class RatioFunc>
void
WellTest<Scalar, IndexTraits>::checkMaxRatioLimitCompletions(const SingleWellState<Scalar, IndexTraits>& ws,
                                                             const Scalar max_ratio_limit,
...
template <typename Scalar, typename IndexTraits>
void
WellTest<Scalar, IndexTraits>::checkMaxGORLimit(const WellEconProductionLimits& econ_production_limits,
                                                const SingleWellState<Scalar, IndexTraits>& ws,
                                                RatioLimitCheckReport& report) const
{
    // function to calculate gor based on rates
    auto gor = [](const std::vector<Scalar>& rates, const PhaseUsageInfo<IndexTraits>& pu) {
...
template <typename Scalar, typename IndexTraits>
void
WellTest<Scalar, IndexTraits>::checkMaxWGRLimit(const WellEconProductionLimits& econ_production_limits,
                                                const SingleWellState<Scalar, IndexTraits>& ws,
                                                RatioLimitCheckReport& report) const
{
    // function to calculate wgr based on rates
    auto wgr = [](const std::vector<Scalar>& rates, const PhaseUsageInfo<IndexTraits>& pu) {
...
template <typename Scalar, typename IndexTraits>
void
WellTest<Scalar, IndexTraits>::checkMaxWaterCutLimit(const WellEconProductionLimits& econ_production_limits,
                                                     const SingleWellState<Scalar, IndexTraits>& ws,
                                                     RatioLimitCheckReport& report) const
{
    // function to calculate water cut based on rates
    auto waterCut = [](const std::vector<Scalar>& rates, const PhaseUsageInfo<IndexTraits>& pu) {
        const Scalar oil_rate = -rates[pu.canonicalToActivePhaseIdx(IndexTraits::oilPhaseIdx)];
        const Scalar water_rate = -rates[pu.canonicalToActivePhaseIdx(IndexTraits::waterPhaseIdx)];
    ...
    if (watercut_limit_violated) {
        report.ratio_limit_violated = true;
        this->checkMaxRatioLimitCompletions(ws, max_water_cut_limit, waterCut, report);
    }
}

template <typename Scalar, typename IndexTraits>
bool
WellTest<Scalar, IndexTraits>::checkRateEconLimits(const WellEconProductionLimits& econ_production_limits,
                                                   ...
                                                   DeferredLogger& deferred_logger) const
{
    ...
    if (econ_production_limits.onMinReservoirFluidRate()) {
        deferred_logger.warning("NOT_SUPPORTING_MIN_RESERVOIR_FLUID_RATE",
...
WellTest<Scalar, IndexTraits>::checkRatioEconLimits(const WellEconProductionLimits& econ_production_limits,
                                                    const SingleWellState<Scalar, IndexTraits>& ws,
                                                    DeferredLogger& deferred_logger) const
{
    ...
    if (econ_production_limits.onMaxGasLiquidRatio()) {
        deferred_logger.warning("NOT_SUPPORTING_MAX_GLR",
    ...
    if (report.ratio_limit_violated) {
        ...
        // the ratio limit but no completion does.
            ...
            std::string message = "The well ratio limit is violated but no completion ratio limit is violated! "
                + well_.name() + " is kept open";
...
template <typename Scalar, typename IndexTraits>
void
WellTest<Scalar, IndexTraits>::updateWellTestStateEconomic(const SingleWellState<Scalar, IndexTraits>& ws,
                                                           ...
                                                           DeferredLogger& deferred_logger) const
{
    ...
    if (well_.isInjector()) {
        deferred_logger.warning(
            "ECON_LIMITS_INJECTOR_" + well_.name(),
            well_.name() + " is an injector, the production economic limits for this well will be ignored.\n");
        return;
    }
    ...
    if (econ_production_limits.onAnyRateLimit()) {
        if (quantity_limit == WellEconProductionLimits::QuantityLimit::POTN) {
            rate_limit_violated
                = this->checkRateEconLimits(econ_production_limits, ws.well_potentials, deferred_logger);
    ...
    if (rate_limit_violated) {
        if (econ_production_limits.endRun()) {
            const std::string warning_message = std::string("ending run after well closed due to economic limits")
                + std::string("is not supported yet \n") + std::string("the program will keep running after ")
                + well_.name() + std::string(" is closed");
        ...
        if (write_message_to_opmlog) {
            if (well_.wellEcl().getAutomaticShutIn()) {
                const std::string msg
                    = std::string("well ") + well_.name() + std::string(" will be shut due to rate economic limit");
                deferred_logger.info(msg);
            } else {
                const std::string msg
                    = std::string("well ") + well_.name() + std::string(" will be stopped due to rate economic limit");
                deferred_logger.info(msg);
            }
        }
    ...
    RatioLimitCheckReport ratio_report = this->checkRatioEconLimits(econ_production_limits, ws, deferred_logger);
    ...
    if (ratio_report.ratio_limit_violated) {
        ...
        case WellEconProductionLimits::EconWorkover::CON: {
            ...
            if (write_message_to_opmlog) {
                if (worst_offending_completion < 0) {
                    const std::string msg = std::string("Connection ") + std::to_string(-worst_offending_completion)
                        + std::string(" for well ") + well_.name()
            ...
            if (allCompletionsClosed) {
                ...
                if (write_message_to_opmlog) {
                    if (well_.wellEcl().getAutomaticShutIn()) {
                        const std::string msg
                            = well_.name() + std::string(" will be shut due to last completion closed");
                        deferred_logger.info(msg);
                    } else {
                        const std::string msg
                            = well_.name() + std::string(" will be stopped due to last completion closed");
                        deferred_logger.info(msg);
                    }
                }
            }
        ...
        default: {
            deferred_logger.warning("NOT_SUPPORTED_WORKOVER_TYPE",
                                    "not supporting workover type "
                                        + WellEconProductionLimits::EconWorkover2String(workover));
        }
        }
    }
}
...
template <typename Scalar, typename IndexTraits>
void
WellTest<Scalar, IndexTraits>::updateWellTestStatePhysical(const double simulation_time,
                                                           const bool write_message_to_opmlog,
                                                           WellTestState& well_test_state,
                                                           DeferredLogger& deferred_logger) const
{
    if (well_test_state.well_is_closed(well_.name())) {
        ...
        if (write_message_to_opmlog) {
            ...
            const std::string msg = "Well " + well_.name() + " will be " + action
                + " as it can not operate under current reservoir conditions.";
            deferred_logger.info(msg);
        }
    }
}
