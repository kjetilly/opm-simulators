...
dissolvedVaporisedRatio(const int io, const int ig, const Scalar rs, const Scalar rv, const Rates& surface_rates)
{
    if ((io < 0) || (ig < 0)) {
        return {rs, rv};
    }
    ...
    return {std::clamp(static_cast<Scalar>(Rs), Scalar {0.0}, rs),
            std::clamp(static_cast<Scalar>(Rv), Scalar {0.0}, rv)};
}

template <class FluidSystem>
int
phaseIdx(unsigned phase)
{
    return FluidSystem::phaseIsActive(phase) ? FluidSystem::canonicalToActivePhaseIdx(phase) : -1;
}
...
namespace RateConverter
{
    ...
    void
    SurfaceToReservoirVoidage<FluidSystem, Region>::sumRates(std::unordered_map<RegionId, Attributes>& attributes_hpv,
                                                             ...
                                                             Parallel::Communication comm)
    {
        for (const auto& reg : rmap_.activeRegions()) {
            ...
            std::transform(
                ra.data.begin(), ra.data.end(), ra.data.begin(), [pv_sum](const auto d) { return d / pv_sum; });
            ra.pv = pv_sum;
        }
    }
    ...
    void SurfaceToReservoirVoidage<FluidSystem, Region>::calcInjCoeff(const RegionId r,
                                                                      ...
                                                                      Coeff& coeff) const
    {
        ...
        if (FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)) {
            ...
            const Scalar bw = FluidSystem::waterPvt().inverseFormationVolumeFactor(
                pvtRegionIdx, T, p, Scalar {0.0}, saltConcentration);

            coeff[iw] = 1.0 / bw;
        }

        if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx)) {
            const Scalar bo = FluidSystem::oilPvt().inverseFormationVolumeFactor(pvtRegionIdx, T, p, Scalar {0.0});
            coeff[io] += 1.0 / bo;
        }

        if (FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)) {
            const Scalar bg
                = FluidSystem::gasPvt().inverseFormationVolumeFactor(pvtRegionIdx, T, p, Scalar {0.0}, Scalar {0.0});
            coeff[ig] += 1.0 / bg;
        }
    }
    ...
    template <class FluidSystem, class Region>
    template <class Coeff>
    void SurfaceToReservoirVoidage<FluidSystem, Region>::calcCoeff(const RegionId r,
                                                                   const int pvtRegionIdx,
                                                                   Coeff& coeff) const
    {
        const auto& ra = attr_.attributes(r);
                                                                   ...
                                                                   Coeff& coeff) const
    {
        ...
        const int io = phaseIdx<FluidSystem>(FluidSystem::oilPhaseIdx);
        const int ig = phaseIdx<FluidSystem>(FluidSystem::gasPhaseIdx);
        const auto [Rs, Rv] = dissolvedVaporisedRatio(io, ig, ra.rs, ra.rv, surface_rates);

        const auto [Rsw, Rvw] = dissolvedVaporisedRatio(iw, ig, ra.rsw, ra.rvw, surface_rates);
    ...
    template <class FluidSystem, class Region>
    template <class Coeff>
    void SurfaceToReservoirVoidage<FluidSystem, Region>::calcCoeff(const int pvtRegionIdx,
                                                                   const Scalar p,
                                                                   const Scalar Rs,
                                                                   const Scalar Rv,
                                                                   const Scalar Rsw,
                                                                   ...
                                                                   Coeff& coeff) const
    {
        ...
        if (FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)) {
            ...
            const Scalar bw
                = FluidSystem::waterPvt().inverseFormationVolumeFactor(pvtRegionIdx, T, p, Rsw, saltConcentration);
        ...
        if (detR != 1 && detRw != 1) {
            std::string msg = "We currently support either gas in water or gas in oil, not both."
                              "i.e. detR = "
                + std::to_string(detR) + " and detRw " + std::to_string(detRw) + "can not both be different from 1";
    ...
    void SurfaceToReservoirVoidage<FluidSystem, Region>::calcReservoirVoidageRates(const int pvtRegionIdx,
                                                                                   const Scalar p,
                                                                                   const Scalar rs,
                                                                                   const Scalar rv,
                                                                                   const Scalar rsw,
                                                                                   ...
                                                                                   VoidageRates& voidage_rates) const
    {
        ...
        const auto [Rs, Rv] = dissolvedVaporisedRatio(io, ig, rs, rv, surface_rates);

        const auto [Rsw, Rvw] = dissolvedVaporisedRatio(iw, ig, rsw, rvw, surface_rates);
        ...
        if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx)) {
            ...
            const auto bo = FluidSystem::oilPvt().inverseFormationVolumeFactor(pvtRegionIdx, T, p, Rs);
        ...
        if (FluidSystem::phaseIsActive(FluidSystem::gasPhaseIdx)) {
            ...
            const auto bg = FluidSystem::gasPvt().inverseFormationVolumeFactor(pvtRegionIdx, T, p, Rv, Rvw);
    ...
    void SurfaceToReservoirVoidage<FluidSystem, Region>::calcReservoirVoidageRates(const RegionId r,
                                                                                   const int pvtRegionIdx,
                                                                                   const Rates& surface_rates,
    ...
    SurfaceToReservoirVoidage<FluidSystem, Region>::inferDissolvedVaporisedRatio(const Scalar rsMax,
                                                                                 const Scalar rvMax,
...
#define INSTANTIATE_TYPE(T)                                                                                            \
    template void SurfaceToReservoirVoidage<FS<T>, std::vector<int>>::sumRates(                                        \
        std::unordered_map<int, Attributes>&, std::unordered_map<int, Attributes>&, Parallel::Communication);          \
    template void SurfaceToReservoirVoidage<FS<T>, std::vector<int>>::calcInjCoeff(                                    \
        const int, const int, std::vector<T>&) const;                                                                  \
    template void SurfaceToReservoirVoidage<FS<T>, std::vector<int>>::calcCoeff(const int, const int, std::vector<T>&) \
        const;                                                                                                         \
    template void SurfaceToReservoirVoidage<FS<T>, std::vector<int>>::calcCoeff(                                       \
        const int, const int, const std::vector<T>&, std::vector<T>&) const;                                           \
    template void SurfaceToReservoirVoidage<FS<T>, std::vector<int>>::calcReservoirVoidageRates(const int,             \
                                                                                                const T,               \
                                                                                                const T,               \
                                                                                                const T,               \
                                                                                                const T,               \
                                                                                                ...
                                                                                                std::vector<T>&)       \
        const;                                                                                                         \
    template void SurfaceToReservoirVoidage<FS<T>, std::vector<int>>::calcReservoirVoidageRates(                       \
        const int, const T, const T, const T, const T, const T, const T, const T, T const* const&, T*&) const;         \
    template void SurfaceToReservoirVoidage<FS<T>, std::vector<int>>::calcReservoirVoidageRates(                       \
        const int, const int, const std::vector<T>&, std::vector<T>&) const;                                           \
    template std::pair<T, T> SurfaceToReservoirVoidage<FS<T>, std::vector<int>>::inferDissolvedVaporisedRatio(         \
        const T, const T, const std::vector<T>::iterator&) const;
