...
namespace wellhelpers
{
    ...
    ParallelStandardWellB<Scalar>::ParallelStandardWellB(const Matrix& B,
    ...
    void ParallelStandardWellB<Scalar>::mv(const X& x, Y& y) const
    {
        ...
        parallel_well_info_.communication().allgatherv(
            send, cstring_size, cstrings.data(), sizes.data(), offsets.data());
    ...
    template <typename Scalar>
    template <class X, class Y>
    void ParallelStandardWellB<Scalar>::mmv(const X& x, Y& y) const
    {
        if (this->parallel_well_info_.communication().size() == 1) {
    ...
    bool rateControlWithZeroProdTarget(const WellProductionControls& controls, const WellProducerCMode mode)
    {
        switch (mode) {
    ...
    bool rateControlWithZeroInjTarget(const WellInjectionControls& controls, const WellInjectorCMode mode)
    {
        switch (mode) {
...
#define INSTANTIATE(T, Dim)                                                                                            \
    template void ParallelStandardWellB<T>::mv(const Vec<T, Dim>&, DynVec<T>&) const;                                  \
    template void ParallelStandardWellB<T>::mmv(const Vec<T, Dim>&, DynVec<T>&) const;

#define INSTANTIATE_WE(T, Dim)                                                                                         \
    template void sumDistributedWellEntries(                                                                           \
        Dune::FieldMatrix<T, Dim, Dim>& mat, Dune::FieldVector<T, Dim>& vec, const Comm& comm);

#define INSTANTIATE_TYPE(T)                                                                                            \
    template class ParallelStandardWellB<T>;                                                                           \
    template void sumDistributedWellEntries(                                                                           \
        Dune::DynamicMatrix<T>& mat, Dune::DynamicVector<T>& vec, const Comm& comm);                                   \
    template DMatrix<T> transposeDenseDynMatrix(const DMatrix<T>&);                                                    \
    template T computeHydrostaticCorrection(const T, const T, const T, const T);                                       \
    INSTANTIATE(T, 1)                                                                                                  \
    INSTANTIATE(T, 2)                                                                                                  \
    INSTANTIATE(T, 3)                                                                                                  \
    INSTANTIATE(T, 4)                                                                                                  \
