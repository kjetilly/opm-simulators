...
BlackoilWellModelWBP<Scalar, IndexTraits>::BlackoilWellModelWBP(
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelWBP<Scalar, IndexTraits>::initializeSources(
    typename ParallelWBPCalculation<Scalar>::GlobalToLocal index,
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelWBP<Scalar, IndexTraits>::registerOpenWellsForWBPCalculation()
{
    assert(this->wbpCalcMap_.size() == static_cast<std::size_t>(well_model_.numLocalWells()));
...
template <typename Scalar, typename IndexTraits>
void
BlackoilWellModelWBP<Scalar, IndexTraits>::initializeWBPCalculationService()
{
    ...
    for (const auto& well : well_model_.eclWells()) {
        this->wbpCalcMap_[wellID].wbpCalcIdx_
            = this->wbpCalculationService_.createCalculator(well,
...
BlackoilWellModelWBP<Scalar, IndexTraits>::computeWellBlockAveragePressures(const Scalar gravity) const
{
    ...
    for (auto wellID = 0 * numWells; wellID < numWells; ++wellID) {
        ...
        this->wbpCalculationService_.inferBlockAveragePressures(calcIdx, well.pavg(), gravity, well.getWPaveRefDepth());

        const auto& result = this->wbpCalculationService_.averagePressures(calcIdx);
...
BlackoilWellModelWBP<Scalar, IndexTraits>::makeWellSourceEvaluatorFactory(
    const std::vector<Well>::size_type wellIdx) const
{
    ...
    return [wellIdx, this]() -> typename ParallelWBPCalculation<Scalar>::Evaluator {
        if (!this->wbpCalcMap_[wellIdx].openWellIdx_.has_value()) {
            ...
            return []([[maybe_unused]] const int connIdx, Span sourceTerm) {
                ...
                sourceTerm.set(Item::Pressure, 0.0)
        ...
        return [this, wellPtr = well_model_.genericWells()[*this->wbpCalcMap_[wellIdx].openWellIdx_]](const int connIdx,
                                                                                                      Span sourceTerm) {
            ...
            const auto& connIdxMap = well_model_.connectionIndexMap(wellPtr->indexOfWell());

            const auto rho = wellPtr->connectionDensity(connIdxMap.global(connIdx), connIdxMap.open(connIdx));
