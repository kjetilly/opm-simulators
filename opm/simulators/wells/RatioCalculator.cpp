...
RatioCalculator<Value>::RatioCalculator(int gasCompIdx, int oilCompIdx, int waterCompIdx, std::string_view name)
    : gasComp_ {gasCompIdx}
    , oilComp_(oilCompIdx)
...
template <class Value>
void
RatioCalculator<Value>::disOilVapWatVolumeRatio(Value& volumeRatio,
                                                const Value& rvw,
                                                const Value& rsw,
                                                const Value& pressure,
                                                ...
                                                DeferredLogger& deferred_logger) const
{
    ...
    if (d <= 0.0) {
        deferred_logger.debug(dValueError(d, name_, "disOilVapWatVolumeRatio", rsw, rvw, pressure));
    }
    const Value tmp_wat = d > 0.0 ? (cmix_s[waterComp_] - rvw * cmix_s[gasComp_]) / d : cmix_s[waterComp_];
    volumeRatio += tmp_wat / b_perfcells_dense[waterComp_];

    const Value tmp_gas = d > 0.0 ? (cmix_s[gasComp_] - rsw * cmix_s[waterComp_]) / d : cmix_s[gasComp_];
    volumeRatio += tmp_gas / b_perfcells_dense[gasComp_];
}

template <class Value>
void
RatioCalculator<Value>::gasOilPerfRateInj(const std::vector<Value>& cq_s,
                                          ...
                                          DeferredLogger& deferred_logger) const
{
    ...
    if (d <= 0.0) {
        deferred_logger.debug(dValueError(d, name_, "gasOilPerfRateInj", rs, rv, pressure));
    } else {
        // vaporized oil into gas
        // rv * q_gr * b_g = rv * (q_gs - rs * q_os) / d
        perf_rates.vap_oil = getValue(rv) * (getValue(cq_s[gasComp_]) - getValue(rs) * getValue(cq_s[oilComp_])) / d;
        // dissolved of gas in oil
        // rs * q_or * b_o = rs * (q_os - rv * q_gs) / d
        perf_rates.dis_gas = getValue(rs) * (getValue(cq_s[oilComp_]) - getValue(rv) * getValue(cq_s[gasComp_])) / d;
    }
    ...
    if (waterActive) {
        ...
        perf_rates.vap_wat = getValue(rvw) * (getValue(cq_s[gasComp_]) - getValue(rs) * getValue(cq_s[oilComp_])) / d;
    }
}

template <class Value>
void
RatioCalculator<Value>::gasOilPerfRateProd(std::vector<Value>& cq_s,
                                           PerforationRates<Scalar>& perf_rates,
                                           const Value& rv,
                                           const Value& rs,
                                           const Value& rvw,
...
template <class Value>
void
RatioCalculator<Value>::gasOilVolumeRatio(Value& volumeRatio,
                                          const Value& rv,
                                          const Value& rs,
                                          const Value& pressure,
                                          const std::vector<Value>& cmix_s,
                                          ...
                                          DeferredLogger& deferred_logger) const
{
    ...
    if (d <= 0.0) {
        deferred_logger.debug(dValueError(d, name_, "gasOilVolumeRatio", rs, rv, pressure));
    }
    const Value tmp_oil = d > 0.0 ? (cmix_s[oilComp_] - rv * cmix_s[gasComp_]) / d : cmix_s[oilComp_];
    volumeRatio += tmp_oil / b_perfcells_dense[oilComp_];

    const Value tmp_gas = d > 0.0 ? (cmix_s[gasComp_] - rs * cmix_s[oilComp_]) / d : cmix_s[gasComp_];
    volumeRatio += tmp_gas / b_perfcells_dense[gasComp_];
}

template <class Value>
void
RatioCalculator<Value>::gasWaterPerfRateInj(const std::vector<Value>& cq_s,
                                            PerforationRates<Scalar>& perf_rates,
                                            const Value& rvw,
                                            const Value& rsw,
                                            const Value& pressure,
                                            DeferredLogger& deferred_logger) const
{
    ...
    if (dw <= 0.0) {
        deferred_logger.debug(dValueError(dw, name_, "gasWaterPerfRateInj", rsw, rvw, pressure));
    } else {
        // vaporized water into gas
        // rvw * q_gr * b_g = rvw * (q_gs - rsw * q_ws) / dw
        perf_rates.vap_wat
            = getValue(rvw) * (getValue(cq_s[gasComp_]) - getValue(rsw) * getValue(cq_s[waterComp_])) / dw;
        // dissolved gas in water
        // rsw * q_wr * b_w = rsw * (q_ws - rvw * q_gs) / dw
        perf_rates.dis_gas_in_water
            = getValue(rsw) * (getValue(cq_s[waterComp_]) - getValue(rvw) * getValue(cq_s[gasComp_])) / dw;
    }
}

template <class Value>
void
RatioCalculator<Value>::gasWaterPerfRateProd(std::vector<Value>& cq_s,
                                             PerforationRates<Scalar>& perf_rates,
                                             const Value& rvw,
                                             const Value& rsw,
                                             const bool isProducer) const
