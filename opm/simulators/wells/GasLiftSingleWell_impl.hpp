...
GasLiftSingleWell<TypeTag>::GasLiftSingleWell(const WellInterface<TypeTag>& well,
...
GasLiftSingleWell<TypeTag>::computeWellRates_(Scalar bhp, bool bhp_is_limited, bool debug_output) const
{
    std::vector<Scalar> potentials(this->NUM_PHASES, 0.0);
    this->well_.computeWellRatesWithBhp(this->simulator_, bhp, potentials, this->deferred_logger_);
    if (debug_output) {
    ...
    std::transform(potentials.begin(), potentials.end(), potentials.begin(), [](const auto& potential) {
        return std::min(Scalar {0}, potential);
    });
    return {-potentials[this->oil_pos_], -potentials[this->gas_pos_], -potentials[this->water_pos_], bhp_is_limited};
}

template <typename TypeTag>
std::optional<typename GasLiftSingleWell<TypeTag>::Scalar>
GasLiftSingleWell<TypeTag>::computeBhpAtThpLimit_(Scalar alq, bool debug_output) const
{
    OPM_TIMEFUNCTION();
    auto bhp_at_thp_limit = this->well_.computeBhpAtThpLimitProdWithAlq(this->simulator_,
                                                                        this->summary_state_,
                                                                        alq,
    ...
    if (bhp_at_thp_limit) {
        if (*bhp_at_thp_limit < this->controls_.bhp_limit) {
            if (debug_output && this->debug) {
                const std::string msg
                    = fmt::format("Computed bhp ({}) from thp limit is below bhp limit ({}), (ALQ = {})."
                                  " Using bhp limit instead",
                                  *bhp_at_thp_limit,
                                  this->controls_.bhp_limit,
                                  alq);
                this->displayDebugMessage_(msg);
            }
        ...
        const std::string msg = fmt::format("Failed in getting converged bhp potential from thp limit (ALQ = {})", alq);
        this->displayDebugMessage_(msg);
    }
...
template <typename TypeTag>
void
GasLiftSingleWell<TypeTag>::setupPhaseVariables_()
{
#ifndef NDEBUG
...
template <typename TypeTag>
void
GasLiftSingleWell<TypeTag>::setAlqMaxRate_(const GasLiftWell& well)
{
    ...
    if (max_alq_optional) {
        ...
        const auto& table = well_.vfpProperties()->getProd()->getTable(this->controls_.vfp_table_number);
...
template <typename TypeTag>
bool
GasLiftSingleWell<TypeTag>::checkThpControl_() const
{
