...
GuideRateHandler<Scalar, IndexTraits>::GuideRateHandler(BlackoilWellModelGeneric<Scalar, IndexTraits>& well_model,
...
template <typename Scalar, typename IndexTraits>
DeferredLogger&
GuideRateHandler<Scalar, IndexTraits>::deferredLogger()
{
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::debugDumpGuideRates(const int report_step_idx, const double sim_time)
{
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::receiveMasterGroupPotentialsFromSlaves()
{
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::sendSlaveGroupPotentialsToMaster(const GroupState<Scalar>& group_state)
{
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::updateGuideRates(const int report_step_idx,
                                                        ...
                                                        GroupState<Scalar>& group_state)
{
    ...
    UpdateGuideRates updater {*this, report_step_idx, sim_time, well_state, group_state, num_phases};
    updater.update();
}
...
template <typename Scalar, typename IndexTraits>
GuideRateHandler<Scalar, IndexTraits>::GuideRateDumper::GuideRateDumper(GuideRateHandler<Scalar, IndexTraits>& parent,
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::GuideRateDumper::dumpGuideRates()
{
    if (this->comm_.rank() == 0) {
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::GuideRateDumper::dumpGuideRatesRecursive_(const Group& group, int level)
{
    if (group.name() != "FIELD") {
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::GuideRateDumper::getGroupGuideRatesInjection_(
    const Group& group, const data::GroupGuideRates& group_guide_rate, std::vector<std::string>& msg_items) const
{
    ...
    for (const auto& [phase, value, phase_str] : items) {
        if (wm_guide_rate.has(name, phase)) {
            if (guide_rate_value.has(value)) {
                msg_items.push_back(fmt::format("{}(Group Inj, {}): {}", name, phase_str, guide_rate_value.get(value)));
            }
        }
    }
}
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::GuideRateDumper::getGroupGuideRatesProduction_(
    const Group& group, const data::GroupGuideRates& group_guide_rate, std::vector<std::string>& msg_items) const
{
    ...
    if (wm_guide_rate.has(name)) { // Check if group has production guiderates
        ...
        static const std::array<std::tuple<Value, std::string_view>, 4> value_types
            = {{{Value::Oil, "oil"}, {Value::Gas, "gas"}, {Value::Water, "water"}, {Value::ResV, "resv"}}};
        const auto& guide_rate_value = group_guide_rate.production;
        for (const auto& [value_type, phase_str] : value_types) {
            if (guide_rate_value.has(value_type)) {
                msg_items.push_back(
                    fmt::format("{}(Group Prod, {}): {}", name, phase_str, guide_rate_value.get(value_type)));
            }
        }
    }
}
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::GuideRateDumper::printGroupGuideRates_(const Group& group, int level)
{
    const auto& name = group.name();
    assert(level >= 1);
    ...
    if (gr_itr == this->group_guide_rates_.end()) {
        this->deferredLogger().debug(fmt::format("{}{}: <NO GUIDERATE>", indent, name));
        return;
    }
    ...
    if (msg_items.empty()) {
        this->deferredLogger().debug(fmt::format("{}{}(Group): <NO GUIDERATES FOUND>", indent, name));
        return;
    }
    this->deferredLogger().debug(fmt::format("{}{}", indent, fmt::join(msg_items, ", ")));
}

template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::GuideRateDumper::printHeader_()
{
    this->deferredLogger().debug(
        fmt::format("\n---- GUIDE RATE REPORT: step {}, simtime {} ----", this->report_step_idx_, this->sim_time_));
}

template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::GuideRateDumper::printFooter_()
{
    this->deferredLogger().debug(
        fmt::format("---- END GUIDE RATE REPORT: step {}, simtime {} ----\n", this->report_step_idx_, this->sim_time_));
}

template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::GuideRateDumper::printWellGuideRates_(const Well& well, int level)
{
    ...
    static const std::array<std::tuple<Value, std::string_view>, 3> value_types
        = {{{Value::Oil, "oil"}, {Value::Gas, "gas"}, {Value::Water, "water"}}};
    const std::string well_type = well.isInjector() ? "Inj" : "Prod";
    for (const auto& [value_type, phase_str] : value_types) {
        if (guide_rate_value.has(value_type)) {
            msg_items.push_back(
                fmt::format("{}(Well {}, {}): {}", name, well_type, phase_str, guide_rate_value.get(value_type)));
        }
    }
    if (msg_items.empty()) {
        this->deferredLogger().debug(fmt::format("{}{}(Well {}): <NO GUIDERATES FOUND>", indent, name, well_type));
        return;
    }
    this->deferredLogger().debug(fmt::format("{}{}", indent, fmt::join(msg_items, ", ")));
}
...
template <typename Scalar, typename IndexTraits>
GuideRateHandler<Scalar, IndexTraits>::UpdateGuideRates::UpdateGuideRates(
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::UpdateGuideRates::update()
{
...
template <typename Scalar, typename IndexTraits>
bool
GuideRateHandler<Scalar, IndexTraits>::UpdateGuideRates::isMasterGroup_(const Group& group)
{
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::UpdateGuideRates::updateGuideRatesForInjectionGroups_(const Group& group)
{
    OPM_TIMEFUNCTION();
    ...
    for (Phase phase : all) {
        ...
        case Group::GuideRateInjTarget::VOID: {
            guide_rate_value = std::max(Scalar(0.0), this->group_state_.injection_vrep_rate(group.name()));
            break;
        }
        case Group::GuideRateInjTarget::NETV: {
            ...
            const std::vector<Scalar>& injRES = this->group_state_.injection_reservoir_rates(group.name());
        ...
        case Group::GuideRateInjTarget::RESV:
            OPM_DEFLOG_THROW(
                std::runtime_error, "GUIDE PHASE RESV not implemented. Group " + group.name(), this->deferredLogger());
        ...
        if (guide_rate_value) {
            guide_rate_value = this->unit_system_.from_si(UnitSystem::measure::rate, *guide_rate_value);
        }
        this->guideRate().compute(group.name(), phase, this->report_step_idx_, guide_rate_value);
    }
}

template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::UpdateGuideRates::updateGuideRatesForProductionGroups_(const Group& group,
                                                                                              std::vector<Scalar>& pot)
{
    ...
    this->guideRate().compute(group.name(), this->report_step_idx_, this->sim_time_, oil_pot, gas_pot, water_pot);
    ...
    this->group_state_.update_group_production_potential(group.name(), oil_pot, gas_pot, water_pot);
}

template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::UpdateGuideRates::updateGuideRatesForWells_()
{
    OPM_TIMEFUNCTION();
    ...
    const auto o_pos
        = pu.phaseIsActive(IndexTraits::oilPhaseIdx) ? pu.canonicalToActivePhaseIdx(IndexTraits::oilPhaseIdx) : -1;

    const auto g_pos
        = pu.phaseIsActive(IndexTraits::gasPhaseIdx) ? pu.canonicalToActivePhaseIdx(IndexTraits::gasPhaseIdx) : -1;

    const auto w_pos
        = pu.phaseIsActive(IndexTraits::waterPhaseIdx) ? pu.canonicalToActivePhaseIdx(IndexTraits::waterPhaseIdx) : -1;
    ...
    for (const auto& wname : wnames) {
        ...
        if (well_index.has_value() && this->well_state_.wellIsOwned(*well_index, wname)) {
            ...
            if (o_pos >= 0) {
                well_pot[o_ix] = static_cast<double>(wpot[o_pos]);
            }
            if (g_pos >= 0) {
                well_pot[g_ix] = static_cast<double>(wpot[g_pos]);
            }
            if (w_pos >= 0) {
                well_pot[w_ix] = static_cast<double>(wpot[w_pos]);
            }
        }
    ...
    for (const auto& wname : wnames) {
        ...
        this->guideRate().compute(wname, this->report_step_idx_, this->sim_time_, o_pot, g_pot, w_pot);
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::UpdateGuideRates::updateProductionGroupPotentialFromSlaveGroup_(
...
template <typename Scalar, typename IndexTraits>
void
GuideRateHandler<Scalar, IndexTraits>::UpdateGuideRates::updateProductionGroupPotentialFromSubGroups(
    const Group& group, std::vector<Scalar>& pot)
{
    for (const std::string& group_name : group.groups()) {
