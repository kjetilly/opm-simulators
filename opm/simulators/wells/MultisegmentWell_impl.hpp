...
MultisegmentWell<TypeTag>::MultisegmentWell(const Well& well,
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::init(const std::vector<Scalar>& depth_arg,
                                ...
                                const bool changed_to_open_this_step)
{
    ...
    for (int local_perf_index = 0; local_perf_index < this->number_of_local_perforations_; ++local_perf_index) {
        ...
        this->cell_perforation_depth_diffs_[local_perf_index]
            = depth_arg[cell_idx] - this->perf_depth_[this->pw_info_.localPerfToActivePerf(local_perf_index)];
    }
}
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::updatePrimaryVariables(const Simulator& simulator,
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::updateWellStateWithTarget(const Simulator& simulator,
                                                     ...
                                                     DeferredLogger& deferred_logger) const
{
    ...
    this->scaleSegmentRatesWithWellRates(this->segments_.inlets(), this->segments_.perforations(), well_state);
...
template <typename TypeTag>
ConvergenceReport
MultisegmentWell<TypeTag>::getWellConvergence(const Simulator& /* simulator */,
                                              const WellStateType& well_state,
                                              const std::vector<Scalar>& B_avg,
                                              DeferredLogger& deferred_logger,
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::apply(const BVector& x, BVector& Ax) const
{
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::apply(BVector& r) const
{
    if (!this->isOperableAndSolvable() && !this->wellIsStopped()) {
        return;
    }
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::recoverWellSolutionAndUpdateWellState(const Simulator& simulator,
                                                                 const BVector& x,
                                                                 WellStateType& well_state,
                                                                 DeferredLogger& deferred_logger)
{
    ...
    try {
        ...
        deferred_logger.problem("In MultisegmentWell::recoverWellSolutionAndUpdateWellState for well " + this->name()
                                + ": " + exp.what());
        throw;
    }
}
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::computeWellPotentials(const Simulator& simulator,
                                                 const WellStateType& well_state,
                                                 ...
                                                 DeferredLogger& deferred_logger)
{
    ...
    if (this->param_.local_well_solver_control_switching_) {
        ...
        if (!converged_implicit) {
            deferred_logger.debug("Implicit potential calculations failed for well " + this->name()
    ...
    if (!converged_implicit) {
        ...
        if (!Base::wellHasTHPConstraints(summaryState) || bhp_controlled_well) {
            ...
            well_potentials = computeWellPotentialWithTHP(well_state, simulator, deferred_logger);
        }
    }
    deferred_logger.debug("Cost in iterations of finding well potential for well " + this->name() + ": "
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::computeWellRatesAtBhpLimit(const Simulator& simulator,
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::computeWellRatesWithBhp(const Simulator& simulator,
                                                   const Scalar& bhp,
                                                   std::vector<Scalar>& well_flux,
                                                   DeferredLogger& deferred_logger) const
{
    ...
    for (int seg = 0; seg < nseg; ++seg) {
        for (const int perf : this->segments_.perforations()[seg]) {
            ...
            computePerfRate(
                intQuants, mob, Tw, seg, perf, seg_pressure, allow_cf, cq_s, perf_press, perf_rates, deferred_logger);
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::computeWellRatesWithBhpIterations(const Simulator& simulator,
                                                             const Scalar& bhp,
                                                             std::vector<Scalar>& well_flux,
                                                             DeferredLogger& deferred_logger) const
{
    ...
    well_copy.scaleSegmentRatesWithWellRates(this->segments_.inlets(), this->segments_.perforations(), well_state_copy);

    well_copy.calculateExplicitQuantities(simulator, well_state_copy, deferred_logger);
    const double dt = simulator.timeStepSize();
    // iterate to get a solution at the given bhp.
    well_copy.iterateWellEqWithControl(
...
MultisegmentWell<TypeTag>::computeWellPotentialWithTHP(const WellStateType& well_state,
                                                       ...
                                                       DeferredLogger& deferred_logger) const
{
    ...
    if (well.isInjector()) {
        ...
        if (bhp_at_thp_limit) {
            ...
            deferred_logger.debug("Converged thp based potential calculation for well " + this->name()
        ...
        auto bhp_at_thp_limit = computeBhpAtThpLimitProd(well_state, simulator, summary_state, deferred_logger);
        if (bhp_at_thp_limit) {
...
template <typename TypeTag>
bool
MultisegmentWell<TypeTag>::computeWellPotentialsImplicit(const Simulator& simulator,
                                                         const WellStateType& well_state,
                                                         std::vector<Scalar>& well_potentials,
                                                         DeferredLogger& deferred_logger) const
{
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::solveEqAndUpdateWellState(const Simulator& simulator,
                                                     WellStateType& well_state,
                                                     DeferredLogger& deferred_logger)
{
    if (!this->isOperableAndSolvable() && !this->wellIsStopped())
    ...
    try {
        ...
        // this is the process with rank zero)
        deferred_logger.problem("In MultisegmentWell::solveEqAndUpdateWellState for well " + this->name() + ": "
                                + exp.what());
        throw;
    }
}
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::computePerfCellPressDiffs(const Simulator& simulator)
{
    ...
    for (int local_perf_index = 0; local_perf_index < this->number_of_local_perforations_; ++local_perf_index) {
        ...
        this->cell_perforation_pressure_diffs_[local_perf_index]
            = this->gravity_ * average_density * this->cell_perforation_depth_diffs_[local_perf_index];
    }
}
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::computeInitialSegmentFluids(const Simulator& simulator)
{
    for (int seg = 0; seg < this->numberOfSegments(); ++seg) {
        ...
        for (int comp_idx = 0; comp_idx < this->num_conservation_quantities_; ++comp_idx) {
            segment_fluid_initial_[seg][comp_idx]
                = surface_volume * this->primary_variables_.surfaceVolumeFraction(seg, comp_idx).value();
        }
    }
}
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::updateWellState(const Simulator& simulator,
                                           ...
                                           const Scalar relaxation_factor)
{
    ...
    const bool stop_or_zero_rate_target = this->stoppedOrZeroRateTarget(simulator, well_state, deferred_logger);
    this->primary_variables_.updateNewton(
        dwells, relaxation_factor, dFLimit, stop_or_zero_rate_target, max_pressure_change);

    const auto& summary_state = simulator.vanguard().summaryState();
    this->primary_variables_.copyToWellState(*this, getRefDensity(), well_state, summary_state, deferred_logger);
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::calculateExplicitQuantities(const Simulator& simulator,
                                                       const WellStateType& well_state,
                                                       DeferredLogger& deferred_logger)
{
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::updateProductivityIndex(const Simulator& simulator,
...
MultisegmentWell<TypeTag>::connectionDensity(const int globalConnIdx, [[maybe_unused]] const int openConnIdx) const
{
    // Simple approximation: Mixture density at reservoir connection is
    // mixture density at connection's segment.

    const auto segNum = this->wellEcl().getConnections()[globalConnIdx].segment();

    const auto segIdx = this->wellEcl().getSegments().segmentNumberToIndex(segNum);
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::addWellContributions(SparseMatrixAdapter& jacobian) const
{
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::addWellPressureEquations(PressureMatrix& jacobian,
                                                    ...
                                                    const WellStateType& well_state) const
{
    ...
    this->linSys_.extractCPRPressureMatrix(
        jacobian, weights, pressureVarIndex, use_well_weights, *this, this->SPres, well_state);
}
...
void
MultisegmentWell<TypeTag>::computePerfRate(const Value& pressure_cell,
                                           const Value& rs,
                                           const Value& rv,
                                           ...
                                           DeferredLogger& deferred_logger) const
{
    ...
    const Value perf_seg_press_diff
        = this->gravity() * segment_density * this->segments_.local_perforation_depth_diff(local_perf_index);
...
template <class Value>
void
MultisegmentWell<TypeTag>::computePerfRate(const IntensiveQuantities& int_quants,
                                           const std::vector<Value>& mob_perfcells,
                                           const std::vector<Scalar>& Tw,
                                           const int seg,
                                           const int perf,
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::computeSegmentFluidProperties(const Simulator& simulator, DeferredLogger& deferred_logger)
{
...
template <class Value>
void
MultisegmentWell<TypeTag>::getMobility(const Simulator& simulator,
...
template <typename TypeTag>
typename MultisegmentWell<TypeTag>::Scalar
MultisegmentWell<TypeTag>::getRefDensity() const
{
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::checkOperabilityUnderBHPLimit(const WellStateType& /*well_state*/,
                                                         const Simulator& simulator,
                                                         DeferredLogger& deferred_logger)
{
    const auto& summaryState = simulator.vanguard().summaryState();
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::updateIPR(const Simulator& simulator, DeferredLogger& deferred_logger) const
{
    ...
    for (int seg = 0; seg < nseg; ++seg) {
        ...
        const Scalar dp = this->getSegmentDp(seg, this->segments_.density(seg).value(), seg_dp);
        seg_dp[seg] = dp;
        for (const int perf : this->segments_.perforations()[seg]) {
            const int local_perf_index = this->pw_info_.activePerfToLocalPerf(perf);
            if (local_perf_index < 0) // then the perforation is not on this process
            ...
            // TODO: maybe we should store the mobility somewhere, so that we only need to calculate it one per
            // iteration
            getMobility(simulator, local_perf_index, mob, deferred_logger);
            ...
            for (std::size_t phase = 0; phase < FluidSystem::numPhases; ++phase) {
                ...
                const unsigned comp_idx
                    = FluidSystem::canonicalToActiveCompIdx(FluidSystem::solventComponentIndex(phase));
            ...
            if ((this->isProducer() && pressure_diff < 0.) || (this->isInjector() && pressure_diff > 0.)) {
                deferred_logger.debug("CROSSFLOW_IPR", "cross flow found when updateIPR for well " + this->name());
            }

            // the well index associated with the connection
            const Scalar trans_mult
                = simulator.problem().template wellTransMultiplier<Scalar>(int_quantities, cell_idx);
            const auto& wellstate_nupcol = simulator.problem().wellModel().nupcolWellState().well(this->index_of_well_);
            const std::vector<Scalar> tw_perf
                = this->wellIndex(local_perf_index, int_quantities, trans_mult, wellstate_nupcol);
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::updateIPRImplicit(const Simulator& simulator,
                                             WellStateType& well_state,
                                             DeferredLogger& deferred_logger)
{
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::checkOperabilityUnderTHPLimit(const Simulator& simulator,
                                                         const WellStateType& well_state,
                                                         DeferredLogger& deferred_logger)
{
    const auto& summaryState = simulator.vanguard().summaryState();
    ...
    if (obtain_bhp) {
        ...
        if (!this->wellIsStopped()) {
            ...
            deferred_logger.debug(" could not find bhp value at thp limit "
                                  ...
                                  + this->name() + ", the well might need to be closed ");
        }
    }
}
...
template <typename TypeTag>
bool
MultisegmentWell<TypeTag>::iterateWellEqWithControl(const Simulator& simulator,
                                                    const double dt,
                                                    ...
                                                    DeferredLogger& deferred_logger)
{
    ...
    for (; it < max_iter_number; ++it, ++debug_cost_counter_) {
        ...
        if (min_relaxation_reached || this->repeatedStagnation(measure_history, this->regularize_, deferred_logger)) {
            ...
            if (reportStag.converged()) {
                converged = true;
                std::string message = fmt::format("Well stagnates/oscillates but {} manages to get converged with "
                                                  "relaxed tolerances in {} inner iterations.",
                                                  this->name(),
                                                  it);
        ...
        try {
            ...
            // (Logging done inside of solve() method will only be seen if
            // this is the process with rank zero)
            deferred_logger.problem("In MultisegmentWell::iterateWellEqWithControl for well " + this->name() + ": "
                                    + exp.what());
            throw;
        }
    }
    ...
    if (converged) {
        ...
        if (relax_convergence)
            sstr << "      (A relaxed tolerance was used after " << this->param_.strict_inner_iter_wells_
                 << " iterations)";
...
template <typename TypeTag>
bool
MultisegmentWell<TypeTag>::iterateWellEqWithSwitching(const Simulator& simulator,
                                                      const double dt,
                                                      const Well::InjectionControls& inj_controls,
                                                      const Well::ProductionControls& prod_controls,
                                                      WellStateType& well_state,
                                                      ...
                                                      const bool fixed_status /*false*/)
{
    ...
    for (; it < max_iter_number; ++it, ++debug_cost_counter_) {
        ...
        if (allow_switching && its_since_last_switch >= min_its_after_switch
            ...
            bool changed = this->updateWellControlAndStatusLocalIteration(simulator,
                                                                          well_state,
                                                                          group_state,
                                                                          inj_controls,
                                                                          prod_controls,
        ...
        if (this->parallel_well_info_.communication().size() > 1
            && this->parallel_well_info_.communication().max(converged)
                != this->parallel_well_info_.communication().min(converged)) {
            OPM_THROW(std::runtime_error,
                      fmt::format("Misalignment of the parallel simulation run in iterateWellEqWithSwitching - the "
                                  "well calculation for well {} succeeded some ranks but failed on other ranks.",
                                  this->name()));
        }
        if (converged) {
        ...
        if (!converged) {
            measure_history.push_back(this->getResidualMeasureValue(well_state,
                                                                    ...
                                                                    this->param_.tolerance_pressure_ms_wells_,
                                                                    deferred_logger));
            bool min_relaxation_reached = this->update_relaxation_factor(
                measure_history, relaxation_factor, this->regularize_, deferred_logger);
            if (min_relaxation_reached
                || this->repeatedStagnation(measure_history, this->regularize_, deferred_logger)) {
                converged = false;
                break;
            }
        }
        try {
            ...
            // (Logging done inside of solve() method will only be seen if
            // this is the process with rank zero)
            deferred_logger.problem("In MultisegmentWell::iterateWellEqWithSwitching for well " + this->name() + ": "
                                    + exp.what());
            throw;
        }
    }
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::assembleWellEqWithoutIteration(const Simulator& simulator,
                                                          const double dt,
                                                          const Well::InjectionControls& inj_controls,
                                                          const Well::ProductionControls& prod_controls,
                                                          WellStateType& well_state,
                                                          ...
                                                          DeferredLogger& deferred_logger)
{
    ...
    //
    // but for the top segment, the pressure equation will be the well control equation, and the other three will be the
    // same.
    ...
    const Scalar rhow = FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)
        ? FluidSystem::referenceDensity(FluidSystem::waterPhaseIdx, Base::pvtRegionIdx())
        : 0.0;
    ...
    for (int seg = 0; seg < nseg; ++seg) {
        ...
        for (const int perf : this->segments_.perforations()[seg]) {
            ...
            computePerfRate(
                int_quants, mob, Tw, seg, perf, seg_pressure, allow_cf, cq_s, perf_press, perfRates, deferred_logger);
            ...
            for (int comp_idx = 0; comp_idx < this->num_conservation_quantities_; ++comp_idx) {
                perf_rates[local_perf_index * this->number_of_phases_
                           + FluidSystem::activeCompToActivePhaseIdx(comp_idx)]
                    = cq_s[comp_idx].value();
            }
            ...
            for (int comp_idx = 0; comp_idx < this->num_conservation_quantities_; ++comp_idx) {
                ...
                MultisegmentWellAssemble(*this).assemblePerforationEq(
                    seg, local_perf_index, comp_idx, cq_s_effective, this->linSys_);
            }
        }
    }
    ...
    for (int seg = 0; seg < nseg; ++seg) {
        ...
        // TODO: without considering the efficiency factor for now
        {
            ...
            // for each component
            for (int comp_idx = 0; comp_idx < this->num_conservation_quantities_; ++comp_idx) {
                const EvalWell accumulation_term = regularization_factor
                    * (segment_surface_volume * this->primary_variables_.surfaceVolumeFraction(seg, comp_idx)
                       - segment_fluid_initial_[seg][comp_idx])
                    / dt;
                MultisegmentWellAssemble(*this).assembleAccumulationTerm(
                    seg, comp_idx, accumulation_term, this->linSys_);
            }
        }
        // considering the contributions due to flowing out from the segment
        {
            ...
            for (int comp_idx = 0; comp_idx < this->num_conservation_quantities_; ++comp_idx) {
                const EvalWell segment_rate
                    = this->primary_variables_.getSegmentRateUpwinding(seg, seg_upwind, comp_idx)
                    * this->well_efficiency_factor_;
                MultisegmentWellAssemble(*this).assembleOutflowTerm(
                    seg, seg_upwind, comp_idx, segment_rate, this->linSys_);
            }
        }
            ...
            for (const int inlet : this->segments_.inlets()[seg]) {
                ...
                for (int comp_idx = 0; comp_idx < this->num_conservation_quantities_; ++comp_idx) {
                    const EvalWell inlet_rate
                        = this->primary_variables_.getSegmentRateUpwinding(inlet, inlet_upwind, comp_idx)
                        * this->well_efficiency_factor_;
                    MultisegmentWellAssemble(*this).assembleInflowTerm(
                        seg, inlet, inlet_upwind, comp_idx, inlet_rate, this->linSys_);
                }
            }
        }
        ...
        if (seg == 0) { // top segment, pressure equation is the control equation
            ...
            MultisegmentWellAssemble(*this).assembleControlEq(well_state,
                                                              group_state,
                                                              schedule,
                                                              summaryState,
                                                              inj_controls,
            ...
            const auto& summary_state = simulator.vanguard().summaryState();
            this->assemblePressureEq(seg,
                                     unit_system,
                                     well_state,
                                     summary_state,
                                     this->param_.use_average_density_ms_wells_,
                                     deferred_logger);
        }
    }
...
template <typename TypeTag>
bool
MultisegmentWell<TypeTag>::openCrossFlowAvoidSingularity(const Simulator& simulator) const
{
...
template <typename TypeTag>
bool
MultisegmentWell<TypeTag>::allDrawDownWrongDirection(const Simulator& simulator) const
{
...
template <typename TypeTag>
void
MultisegmentWell<TypeTag>::updateWaterThroughput(const double /*dt*/, WellStateType& /*well_state*/) const
{
}
...
MultisegmentWell<TypeTag>::getSegmentSurfaceVolume(const Simulator& simulator, const int seg_idx) const
{
    EvalWell temperature;
    EvalWell saltConcentration;
...
MultisegmentWell<TypeTag>::computeBhpAtThpLimitProd(const WellStateType& well_state,
                                                    const Simulator& simulator,
                                                    const SummaryState& summary_state,
                                                    DeferredLogger& deferred_logger) const
{
    return this->MultisegmentWell<TypeTag>::computeBhpAtThpLimitProdWithAlq(simulator,
                                                                            summary_state,
...
template <typename TypeTag>
std::optional<typename MultisegmentWell<TypeTag>::Scalar>
MultisegmentWell<TypeTag>::computeBhpAtThpLimitProdWithAlq(const Simulator& simulator,
                                                           const SummaryState& summary_state,
                                                           ...
                                                           bool iterate_if_no_solution) const
{
    ...
    return WellBhpThpCalculator(*this).computeBhpAtThpLimitProd(fratesIter,
                                                                summary_state,
                                                                this->maxPerfPress(simulator),
                                                                this->getRefDensity(),
                                                                alq_value,
...
template <typename TypeTag>
std::optional<typename MultisegmentWell<TypeTag>::Scalar>
MultisegmentWell<TypeTag>::computeBhpAtThpLimitInj(const Simulator& simulator,
                                                   const SummaryState& summary_state,
                                                   DeferredLogger& deferred_logger) const
{
    // Make the frates() function.
    ...
    auto bhpAtLimit = WellBhpThpCalculator(*this).computeBhpAtThpLimitInj(
        frates, summary_state, this->getRefDensity(), 0.05, 100, false, deferred_logger);
    ...
    return WellBhpThpCalculator(*this).computeBhpAtThpLimitInj(
        fratesIter, summary_state, this->getRefDensity(), 0.05, 100, false, deferred_logger);
}
...
template <typename TypeTag>
typename MultisegmentWell<TypeTag>::Scalar
MultisegmentWell<TypeTag>::maxPerfPress(const Simulator& simulator) const
{
...
template <typename TypeTag>
std::vector<typename MultisegmentWell<TypeTag>::Scalar>
MultisegmentWell<TypeTag>::computeCurrentWellRates(const Simulator& simulator, DeferredLogger& deferred_logger) const
{
    ...
    for (int seg = 0; seg < nseg; ++seg) {
        ...
        for (const int perf : this->segments_.perforations()[seg]) {
            ...
            PerforationRates<Scalar> perf_rates;
            computePerfRate(
                int_quants, mob, Tw, seg, perf, seg_pressure, allow_cf, cq_s, perf_press, perf_rates, deferred_logger);
...
template <typename TypeTag>
std::vector<typename MultisegmentWell<TypeTag>::Scalar>
MultisegmentWell<TypeTag>::getPrimaryVars() const
{
...
template <typename TypeTag>
int
MultisegmentWell<TypeTag>::setPrimaryVars(typename std::vector<Scalar>::const_iterator it)
{
    const int num_seg = this->numberOfSegments();
    constexpr int num_eq = MSWEval::numWellEq;
...
MultisegmentWell<TypeTag>::getFirstPerforationFluidStateInfo(const Simulator& simulator) const
{
    Scalar fsTemperature = 0.0;
    using SaltConcType =
        typename std::decay<decltype(std::declval<decltype(simulator.model().intensiveQuantities(0, 0).fluidState())>()
    ...
    return this->parallel_well_info_.communication().size() == 1 ? info
