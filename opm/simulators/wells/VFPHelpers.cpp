...
Scalar
findX(const Scalar x0, const Scalar x1, const Scalar y0, const Scalar y1, const Scalar y)
{
...
template <typename T>
static T
chopNegativeValues(const T& value)
{
...
template <class Scalar>
detail::InterpData<Scalar>
VFPHelpers<Scalar>::findInterpData(const Scalar value_in, const std::vector<double>& values)
{
...
template <class Scalar>
detail::VFPEvaluation<Scalar>
VFPHelpers<Scalar>::interpolate(const VFPProdTable& table,
...
template <class Scalar>
detail::VFPEvaluation<Scalar>
VFPHelpers<Scalar>::interpolate(const VFPInjTable& table,
                                const detail::InterpData<Scalar>& flo_i,
                                const detail::InterpData<Scalar>& thp_i)
{
...
template <class Scalar>
detail::VFPEvaluation<Scalar>
VFPHelpers<Scalar>::bhp(const VFPProdTable& table,
                        const Scalar aqua,
                        const Scalar liquid,
                        const Scalar vapour,
                        const Scalar thp,
...
template <class Scalar>
detail::VFPEvaluation<Scalar>
VFPHelpers<Scalar>::bhp(
    const VFPInjTable& table, const Scalar aqua, const Scalar liquid, const Scalar vapour, const Scalar thp)
{
    // Find interpolation variables
    Scalar flo = detail::getFlo(table, aqua, liquid, vapour);
...
template <class Scalar>
Scalar
VFPHelpers<Scalar>::findTHP(const std::vector<Scalar>& bhp_array,
                            ...
                            const bool find_largest)
{
    ...
    // bhp_array not sorted, raw search.
        ...
        const bool valid_low
            = (bhp < bhp_array[0] && first_slope_positive) || (bhp >= bhp_array[0] && !first_slope_positive);
        const bool last_slope_positive = bhp_array[nthp - 1] >= bhp_array[nthp - 2];
        const bool valid_high = (bhp > bhp_array[nthp - 1] && last_slope_positive)
            || (bhp <= bhp_array[nthp - 1] && !last_slope_positive);
...
VFPHelpers<Scalar>::getMinimumBHPCoordinate(
...
VFPHelpers<Scalar>::intersectWithIPR(const VFPProdTable& table,
                                     const Scalar thp,
                                     const Scalar wfr,
                                     const Scalar gfr,
                                     const Scalar alq,
...
namespace detail
{
    ...
    const T& getTable(const std::map<int, std::reference_wrapper<const T>>& tables, int table_id)
    {
        ...
        if (entry == tables.end()) {
            OPM_THROW(std::invalid_argument, "Nonexistent VFP table " + std::to_string(table_id) + " referenced.");
        } else {
