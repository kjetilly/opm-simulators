...
SimulatorTimer
SimulatorTimer::serializationTestObject()
{
    SimulatorTimer res;
...
void
SimulatorTimer::init(const ParameterGroup& param)
{
...
void
SimulatorTimer::init(const Schedule& schedule, std::size_t report_step)
{
...
bool
SimulatorTimer::initialStep() const
{
...
/// Current step number.
int
SimulatorTimer::currentStepNum() const
{
...
void
SimulatorTimer::setCurrentStepNum(int step)
{
    current_step_ = step;
...
/// Current step length.
double
SimulatorTimer::currentStepLength() const
{
    assert(!done());
...
double
SimulatorTimer::stepLengthTaken() const
{
...
double
SimulatorTimer::simulationTimeElapsed() const
{
    return current_time_;
}

boost::posix_time::ptime
SimulatorTimer::startDateTime() const
{
...
/// Total time.
double
SimulatorTimer::totalTime() const
{
    return total_time_;
}
...
void
SimulatorTimer::setTotalTime(double time)
{
    total_time_ = time;
}
...
void
SimulatorTimer::report(std::ostream& os) const
{
...
bool
SimulatorTimer::operator==(const SimulatorTimer& rhs) const
{
    return this->timesteps_ == rhs.timesteps_ && this->current_step_ == rhs.current_step_
        && this->current_time_ == rhs.current_time_ && this->total_time_ == rhs.total_time_
