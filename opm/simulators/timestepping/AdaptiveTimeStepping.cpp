...
namespace detail
{
    ...
    std::set<std::string> consistentlyFailingWells(const std::vector<StepReport>& sr, bool requireRepeatedFailures)
    {
        ...
        msg << "    Excessive chopping detected in report step " << sr.back().report_step << ", substep "
    ...
    void registerAdaptiveParameters()
    {
        ...
        Parameters::Register<Parameters::SolverVerbosity>("Specify the \"chattiness\" of the non-linear solver itself");
        Parameters::Register<Parameters::TimeStepVerbosity>("Specify the \"chattiness\" during the time integration");
        Parameters::Register<Parameters::InitialTimeStepInDays>("The size of the initial time step in days");
        Parameters::Register<Parameters::FullTimeStepInitially>(
            "Always attempt to finish a report step using a single substep");
        Parameters::Register<Parameters::TimeStepControl>("The algorithm used to determine time-step sizes. "
                                                          "Valid options are: "
                                                          "'pid' (default), "
                                                          "'pid+iteration', "
                                                          "'pid+newtoniteration', "
    ...
    createController(const UnitSystem& unitSystem)
    {
        ...
        const auto creators = std::unordered_map<std::string, Func> {
            {"pid",
             [tol, verbose]() {
                 return RetVal {TimeStepControlType::PID, std::make_unique<PIDTimeStepControl>(tol, verbose), false};
             }},
            {"pid+iteration",
             [tol, verbose]() {
                 ...
                 return RetVal {TimeStepControlType::PIDAndIterationCount,
            ...
            {"pid+newtoniteration",
             [tol, verbose, &unitSystem]() {
                 ...
                 const double decayDampingFactor = Parameters::Get<Parameters::TimeStepControlDecayDampingFactor>();
                 const double growthDampingFactor = Parameters::Get<Parameters::TimeStepControlGrowthDampingFactor>();
                 const double nonDimensionalMinTimeStepIterations
                     = Parameters::Get<Parameters::MinTimeStepBasedOnNewtonIterations>();
                 // the min time step can be reduced by the newton iteration numbers
                 double minTimeStepReducedByIterations
                     = unitSystem.to_si(UnitSystem::measure::time, nonDimensionalMinTimeStepIterations);
                 return RetVal {TimeStepControlType::PIDAndIterationCount,
            ...
            {"iterationcount",
             [verbose]() {
                 ...
                 return RetVal {
                     ...
                     std::make_unique<SimpleIterationCountTimeStepControl>(iterations, decayrate, growthrate, verbose),
                     false};
             }},
            {"newtoniterationcount",
             [verbose]() {
            ...
            {"general3rdorder",
             [tol, verbose]() {
                 ...
                 const std::string toleranceTestVersion
                     = Parameters::Get<Parameters::TimeStepControlToleranceTestVersion>();
                 ...
                 return RetVal {TimeStepControlType::General3rdOrder,
            ...
            {"hardcoded",
             []() {
                 ...
                 return RetVal {TimeStepControlType::HardCodedTimeStep,
