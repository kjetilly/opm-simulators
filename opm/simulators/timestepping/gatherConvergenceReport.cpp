...
public:
    ...
    explicit CollectConvReports(const Opm::Mpi::Packer& packer, const Opm::Parallel::Communication comm)
    ...
    /// rank order.
    std::vector<Opm::ConvergenceReport> operator()(const Opm::ConvergenceReport& report);
...
private:
    ...
    void deserialise(const std::vector<int>::size_type rank, Opm::ConvergenceReport& report);
};
...
CollectConvReports::operator()(const Opm::ConvergenceReport& report)
{
    ...
    std::tie(this->rankBuffers_, this->rankStart_) = Opm::allGatherv(this->m_buffer, this->comm_);
...
void
CollectConvReports::deserialise(const std::vector<int>::size_type rank, Opm::ConvergenceReport& report)
{
...
/// reports.
ConvergenceReport
gatherConvergenceReport(const ConvergenceReport& local_report, Parallel::Communication mpi_communicator)
{
    ...
    const auto rankReports = CollectConvReports {packer, mpi_communicator}(local_report);

    std::for_each(rankReports.begin(), rankReports.end(), [&combinedReport](const auto& rankReport) {
