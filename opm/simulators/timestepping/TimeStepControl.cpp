...
SimpleIterationCountTimeStepControl::SimpleIterationCountTimeStepControl(const int target_iterations,
    ...
    , verbose_(verbose)
{
    if (decayrate_ > 1.0) {
        OPM_THROW(std::runtime_error,
                  ...
                  "decay should be <= 1 "
                      + std::to_string(decayrate_));
    }
    if (growthrate_ < 1.0) {
        OPM_THROW(std::runtime_error,
                  "SimpleIterationCountTimeStepControl: "
                  "growth should be >= 1 "
                      + std::to_string(growthrate_));
    }
}
...
bool
SimpleIterationCountTimeStepControl::operator==(const SimpleIterationCountTimeStepControl& ctrl) const
{
    return this->target_iterations_ == ctrl.target_iterations_ && this->decayrate_ == ctrl.decayrate_
        && this->growthrate_ == ctrl.growthrate_ && this->verbose_ == ctrl.verbose_;
}
...
HardcodedTimeStepControl::HardcodedTimeStepControl(const std::string& filename)
{
...
HardcodedTimeStepControl::serializationTestObject()
{
...
bool
HardcodedTimeStepControl::operator==(const HardcodedTimeStepControl& ctrl) const
{
                                        ...
                                        const AdaptiveSimulatorTimer& /* substepTimer */) const
{
    ...
    if (errors_[2] > tol_) {
        ...
        const double newDt = (dt * std::pow(errors_[1] / errors_[2], kP) * std::pow(tol_ / errors_[2], kI)
...
bool
PIDTimeStepControl::operator==(const PIDTimeStepControl& ctrl) const
{
    return this->tol_ == ctrl.tol_ && this->errors_ == ctrl.errors_ && this->verbose_ == ctrl.verbose_;
}
...
PIDAndIterationCountTimeStepControl::PIDAndIterationCountTimeStepControl(const int target_iterations,
...
bool
PIDAndIterationCountTimeStepControl::operator==(const PIDAndIterationCountTimeStepControl& ctrl) const
{
    return static_cast<const PIDTimeStepControl&>(*this) == ctrl && this->target_iterations_ == ctrl.target_iterations_
        && this->decayDampingFactor_ == ctrl.decayDampingFactor_
        && this->growthDampingFactor_ == ctrl.growthDampingFactor_
        && this->minTimeStepBasedOnIterations_ == ctrl.minTimeStepBasedOnIterations_ && this->verbose_ == ctrl.verbose_;
}
...
double
General3rdOrderController::timeStepFactor(const std::array<double, 3>& errors,
                                          const std::array<double, 3>& timeSteps) const
{
    if (controllerVersion_ == InternalControlVersions::General3rdOrder) {
        return std::pow(safetyFactor_ * tolerance_ / errors[2], beta_[0])
            ...
            * std::pow(timeSteps[2] / timeSteps[1], -alpha_[0]) * std::pow(timeSteps[1] / timeSteps[0], -alpha_[1]);
    }
    // controllerVersion_ == InternalControlVersions::IController
...
bool
General3rdOrderController::timeStepAccepted(const double error, const double timeStepJustCompleted) const
{
...
bool
General3rdOrderController::operator==(const General3rdOrderController& ctrl) const
{
    return this->tolerance_ == ctrl.tolerance_ && this->safetyFactor_ == ctrl.safetyFactor_
        && this->rejectCompletedStep_ == ctrl.rejectCompletedStep_ && this->errors_ == ctrl.errors_
        && this->timeSteps_ == ctrl.timeSteps_ && this->beta_ == ctrl.beta_ && this->alpha_ == ctrl.alpha_
        && this->controllerVersion_ == ctrl.controllerVersion_
        && this->toleranceTestVersion_ == ctrl.toleranceTestVersion_
        && this->maxReductionTimeStep_ == ctrl.maxReductionTimeStep_ && this->verbose_ == ctrl.verbose_;
}
