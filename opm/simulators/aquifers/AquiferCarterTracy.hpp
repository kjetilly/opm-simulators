...
public:
    ...
    data::AquiferData aquiferData() const override
    {
        ...
        if (!this->co2store_or_h2store_()) {
            ...
            const auto x = this->aquct_data_.porosity * this->aquct_data_.total_compr * this->aquct_data_.inner_radius
    ...
    {
        return static_cast<const AquiferAnalytical<TypeTag>&>(*this) == rhs && this->fluxValue_ == rhs.fluxValue_
...
protected:
    ...
    std::pair<Scalar, Scalar> getInfluenceTableValues(const Scalar td_plus_dt)
    {
        ...
        this->dimensionless_pressure_ = linearInterpolation(
            this->aquct_data_.dimensionless_time, this->aquct_data_.dimensionless_pressure, this->dimensionless_time_);

        const auto PItd = linearInterpolation(
            this->aquct_data_.dimensionless_time, this->aquct_data_.dimensionless_pressure, td_plus_dt);

        const auto PItdprime = linearInterpolationDerivative(
            this->aquct_data_.dimensionless_time, this->aquct_data_.dimensionless_pressure, td_plus_dt);
    ...
    {
        const auto gdz = this->gravity_() * (this->cell_depth_.at(idx) - this->aquiferDepth());

        const auto dp = this->pa0_ + this->rhow_ * gdz - this->pressure_previous_.at(idx);
    ...
    {
        this->Tc_ = this->co2store_or_h2store_() ? this->timeConstantCO2Store() : this->aquct_data_.timeConstant();
    ...
    void calculateAquiferCondition() override
    {
        ...
        if (!this->aquct_data_.initial_pressure.has_value()) {
            this->aquct_data_.initial_pressure = this->calculateReservoirEquilibrium();
        ...
        this->rhow_ = this->co2store_or_h2store_() ? this->waterDensityCO2Store() : this->aquct_data_.waterDensity();
    }
...
private:
    ...
    Scalar waterDensityCO2Store() const
    {
        ...
        if (FluidSystem::phaseIsActive(FluidSystem::oilPhaseIdx)) {
            ...
            const auto rs = Scalar {0}; // no dissolved CO2
            return pvt.inverseFormationVolumeFactor(reg, temp, press, rs) * pvt.oilReferenceDensity(reg);
        } else if (FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)) {
            ...
            return pvt.inverseFormationVolumeFactor(reg, temp, press, rsw, salinity) * pvt.waterReferenceDensity(reg);
        } else {
            OPM_THROW(std::runtime_error, "water or oil phase is needed to run CO2Store.");
        }
    }
    ...
    {
        return this->aquct_data_.initial_temperature.has_value() ? this->aquct_data_.initial_temperature.value()
